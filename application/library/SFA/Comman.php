...
class SFA_Comman
{
    ...
    public function ociConnect() {
       // echo str_repeat(" ",1024*1024*4) ."ociConnect!<br>";ob_flush();
        if($this->ociConn == null) {
			...
			//echo "test";
           // $this->ociConn = oci_connect('SYSTEM', 'system', '//192.168.0.2/XE');
			...
			//$this->ociConn = oci_connect('ENHANCE', 'ENHANCE', '(DESCRIPTION = (ADDRESS = (PROTOCOL = TCP)(HOST = SFDSA-DT0001.safatdairy.com)(PORT = 1521))(CONNECT_DATA = (SERVER = DEDICATED)(SERVICE_NAME = TST.safatdairy.com)))');
			//$this->ociConn = oci_connect('STAGING', 'STAGING', '192.168.1.39');
			//echo "testsfss";
		///	print_r($this->ociConn);
			$m = oci_error();
		//	print_r($m);
            if (!$this->ociConn) {
				$m = oci_error();
   ...
   // mysqli_set_charset('utf8');
	...
	//$sSQL= 'SET CHARACTER SET utf8'; 
        ...
        $path   = str_replace('//','/',$_SERVER['DOCUMENT_ROOT'].'/sfa_enhance/log/');
        $path1     = str_replace('//','/',$baseUrl.'/');        
        
        $filename = $path.'/cs_log_'.date('YmdH').'.txt';
     ...
     //  echo $call_sp; //exit;
        ...
        // commented by Shine to disable writing to log file.
	...
	*/
	public function passDecrypt($strq){
	    $cryptKey  = '4QcTlzuaNUcX289Z9D0ovPCzb';
    $qDecoded      = rtrim( mcrypt_decrypt( MCRYPT_RIJNDAEL_256, md5( $cryptKey ), base64_decode( $strq ), MCRYPT_MODE_CBC, md5( md5( $cryptKey ) ) ), "\0");
    return( $qDecoded );
	...
	mysqli_query('SET CHARACTER SET utf8');
     ...
     /*   $path   = str_replace('//','/',$_SERVER['DOCUMENT_ROOT'].'/sfa/enhance/log/');
        //$path1     = str_replace('//','/',$baseUrl.'/');        
        
        $filename = $path.'/cs_log_'.date('Ymd').'.txt';*/
