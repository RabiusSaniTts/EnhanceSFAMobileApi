    ...
    private function insertCustomerInvoice() {
        ...
        while($count > 0) {
            ...
            foreach($result as $row) {
                ...
                $array = array($row[0]['INVOICENUMBER'], $row[0]['SALESMANCODE'], $row[0]['CUSTOMERCODE'], $transactionDate, $dueDate, $row[0]['TOTALINVOICEAMOUNT'], $row[0]['INVOICEBALANCE'], "'" .$row[0]['ORACLEREFERENCENUMBER'] . "'", $row[0]['STATUS'], $row[0]['RECORDMODE'], '0', '0');
                $sqlArray = implode(",", $array);
                $sql = $sql ."(" .$sqlArray ."),";
            }
            $sql = rtrim($sql, ",");
