   ...
   public function updateOracleDBWithExportData($dataArray) 
   {
	...
	{             
        ...
        foreach($queryResult[0] as $row) 
		{
            ...
            $ociSql = $ociSql ."(SEQUENCENUMBER,INVOICENUMBER, ROUTEKEY, TRANSACTIONDATE, CUSTOMERREFERENCENO, CUSTOMERCODE,ROUTECODE,SALESMANCODE, TOTALINVOICEAMOUNT, INVOICEDISCOUNT, INVOICEBALANCE, STATUS)";
            $ociSql = $ociSql ." VALUES ";
            $ociSql = $ociSql ."(INVOICEHEADER_SEQ.NEXTVAL,'$row[invoiceno]', $row[routekey], TO_DATE('$row[transactiondate]','YYYY-MM-DD'),'$row[custrefno]', '$row[custcode]', '$row[routecode]', '$row[salesmancode]', $row[totalinvoiceamt], '$row[invoicediscount]', '$row[invoicebalance]', 1)";
	...
	{        
        ...
        foreach($queryResult[0] as $row) 
		{
            ...
            $ociSql = $ociSql ."(SEQUENCENUMBER, INVOICENUMBER, ROUTEKEY, LINENO, ITEMCODE, QUANTITY, REFERENCEINVOICENO, LINETYPE,EACHPRICE, ITEMDISCOUNT, EXPIRYDATE, STATUS, ATTRIBUTE1, ATTRIBUTE2, ATTRIBUTE3)";
            $ociSql = $ociSql ." VALUES ";
			if($row['expirydate'] == "")
			{
			 $ociSql = $ociSql ."(INVOICEDETAIL_SEQ.NEXTVAL, $row[invoiceno], $row[routekey], $row[lineno],'$row[itemcode]', $row[quantity],'$row[refinvoiceno]', '$row[linetype]', $row[eachprice], '$row[itemdiscount]',NULL, 1, '$row[unit]', '$row[attribute2]', $row[attribute3])";		
			}	else
			{
			$ociSql = $ociSql ."(INVOICEDETAIL_SEQ.NEXTVAL, $row[invoiceno], $row[routekey], $row[lineno],'$row[itemcode]', $row[quantity],'$row[refinvoiceno]', '$row[linetype]', $row[eachprice], '$row[itemdiscount]',TO_DATE('$row[expirydate]','YYYY-MM-DD'), 1, '$row[unit]', '$row[attribute2]', $row[attribute3])";
			}			
	...
	{ 
		...
		{        	
			...
			$ociSql = $ociSql ."(SEQUENCENUMBER, ORDERNUMBER, ROUTEKEY, LINENO, ITEMCODE, QUANTITY, LINETYPE, EACHPRICE, ITEMDISCOUNT, EXPIRYDATE, STATUS, ATTRIBUTE1, ATTRIBUTE2, ATTRIBUTE3, ATTRIBUTE4)";
            $ociSql = $ociSql ." VALUES ";

			if($row['expirydate'] == "")
			{
			$ociSql = $ociSql ."(ORDERDETAIL_SEQ.NEXTVAL, $row[invoiceno], $row[routekey], $row[lineno],'$row[itemcode]', $row[quantity], '$row[linetype]', $row[eachprice], '$row[itemdiscount]',NULL, 1, '$row[unit]', '$row[attribute2]', $row[attribute3], '$row[attribute4]')";	
			}	else
			{
			$ociSql = $ociSql ."(ORDERDETAIL_SEQ.NEXTVAL, $row[invoiceno], $row[routekey], $row[lineno],'$row[itemcode]', $row[quantity], '$row[linetype]', $row[eachprice], '$row[itemdiscount]',TO_DATE('$row[expirydate]','YYYY-MM-DD'), 1, '$row[unit]', '$row[attribute2]', $row[attribute3], '$row[attribute4]')";
			}	
