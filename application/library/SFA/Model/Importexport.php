    ...
    */
    public function saveXlsTabledata($sheets = array(), $tablename)
    {
        ...
        if(!empty($sheets))
        {
			...
			if($result[1][0]["counter"] > 0){
				$arrres = explode("=",$result[0][0]["result"]);
				 $count = $arrres[1];//exit;
				 $counter = $result[1][0]["counter"];
				if($counter > 0){
					if($counter == $count){
						return true;
					}else{
						$msg = "Error: ".$count." lines updated into database. Some data Miss Match Occured";
					}
				
				}else{
					$msg = "Error: Failed to upload Or uploaded file didnt have any data";
				}
				return $msg;
			}else{
            $msg = "Error: Uploaded file didnt have any data or File format is different.";
            return $msg;
			}
        }
        else
