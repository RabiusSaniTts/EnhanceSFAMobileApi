...
*/
    ...
    */
    ...
    public function dailysalesmanloadgridAction()
    {
		...
		$param_array[12] = $params['rtstatus'];
    ...
    * @param
    *
    * This action is use for give list of customer item group
    */
	function customeritemgroupAction () {
	...
	if($formdata["hdDelete"]==1)
	{
	    ...
	    $result 	= $this->SFA_Comman->executequery('CALL sp_delete_inventory_transaction_customeritemgroup(?,?)',$param_array,'');
	...
	$cols_array = array('customeritemgrpcode','description','transferstatus');
	$columns_show =  array($this->translate->_('Code'),$this->translate->_('Description'),$this->translate->_('Status'));
	...
	$pagingparams = array(
				"show_grid_heading" => true,
				"grid_heading_message" => $this->translate->_('Overview'),
				"pagename" => $this->translate->_('Customer Item Group'),
				"show_records_per_page" => isset($formdata["show_records_per_page"])?$formdata["show_records_per_page"]:10,
				"show_searchbox" => true,
				"searchbox_value" => isset($formdata["grid_search_text"])?$formdata["grid_search_text"]:"",
				"show_selectbox" => true,
				...
				"editlink" => array("/inventory/transaction/addcustomeritemgroup/id/#pattern#/edit/yes/","#pattern#"),
				"nodata_message" => $this->translate->_('No Record(s) Found'),
				"fetch_columns_inquery" => $cols_array,
				"show_columns" => $columns_show			 
			    );
	...
	$printData = (isset($formdata['printData'])) ? $formdata['printData'] : $params['printData'];
	// called stored procedure for counter
	$result = $this->SFA_Comman->executequery('CALL sp_get_inventory_transaction_customeritemgroup(?,?,?,?,?,?,?,?)',$param_array,'',$downloadCSV,$printData,2,$pagingparams);
	...
	$this->view->addScriptPath(APPLICATION_PATH."/theme/default/datagrid/");
	 ...
	 /**
    * @name       addcustomeritemgroupAction
    * @since      6-2-2012
    * @version    Release: 1
    * @author     PM <pankit@elantechnologies.com>
    * @copyright  Elan Technologies
    * @param
    *
    * This action is use for add customer category item
    */
    public function  addcustomeritemgroupAction()
    {        
		$this->view->params = $params = $this->getRequest()->getParams();
		$this->view->formdata = $formdata = $this->_request->getPost();
//	print_r($formdata);
		...
		if($formdata['txtdescription'] !='')
		{
			...
			$param_array[1] = $formdata['txtdescription'];
			$param_array[2] = $formdata['ddlitem_group'];
			$param_array[3] = $formdata['ddlcustomer_group'];
			$param_array[4] = $formdata['ddlstatus'];
			$param_array[5]	= $selectedval;	//selected question
			$param_array[6]	= $formdata['hdncount_items'];			//get count for add record in child table
			$param_array[7]	= $difference;
			$param_array[8]	= $diff_cnt;
			$param_array[9]	= $this->currentUser->username;			
			
			if($formdata['hdnid'] > 0)
			{
				$param_array[10] = $formdata['hdnid'];
				$result = $this->SFA_Comman->executequery("CALL sp_edit_inventory_transaction_addcustomeritemgroup(?,?,?,?,?,?,?,?,?)",$param_array,'');
				SFA_Message::setMsg($this->translate->_('Update Record'));
			}
			else
			{
				$result = $this->SFA_Comman->executequery("CALL sp_add_inventory_transaction_addcustomeritemgroup(?,?,?,?,?,?,?,?)",$param_array,'');
				SFA_Message::setMsg($this->translate->_('New Record'));
			}
			$this->_helper->redirector('customeritemgroup', 'transaction', 'inventory');
		}
		elseif($params['id'] > 0)
		{
			$result = $this->SFA_Comman->executequery("CALL sp_get_inventory_transaction_addcustomeritemgroup('?')",$params['id'],'');
			$this->view->items_data 		= $result[0];
			$this->view->item_group_data	= $result[1];
			$this->view->formdata 		= $result[2][0];
			$this->view->selected_items		= $result[3];
			$customer = $this->SFA_Comman->executequery("call sp_getcombobox_account_customer_addcustomer()",'','');
			$this->view->customercat_data 			= $customer[3];
		}
		else
		{
			$result = $this->SFA_Comman->executequery("CALL sp_get_inventory_transaction_addcustomeritemgroup('?')",'0','');
			$this->view->items_data 			= $result[0];
			$this->view->item_group_data 		= $result[1];
			$customer = $this->SFA_Comman->executequery("call sp_getcombobox_account_customer_addcustomer()",'','');
			$this->view->customercat_data 			= $customer[3];
			$this->view->formdata['customeritemgrpcode'] 	= ($result[2][0]['Auto_increment'] == '' ) ? '1' : $result[2][0]['Auto_increment'];
		}
    }
	/**
    * @name       getitemfromitemgrpforcustomerAction
    * @since      10-07-2012
    * @version    Release: 1
    * @author     HD <hiren.d@elantechnologies.com>
    * @copyright  Elan Technologies
    ...
    */
    public function getitemfromitemgrpforcustomerAction()
    {
		//view variable declaration
		$params = $this->getRequest()->getParams();
		$param_array[1] = $params['item_grp'];
		$param_array[2] = ($params['customer_cat'] <> "" ? $params['customer_cat']:0);
		
		$result = $this->SFA_Comman->executequery('CALL sp_get_items_customergroupcode(?,?)',$param_array,'');
	//	print_r($result[0]);
