...
*/
class Reports_OtpgenerationController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      15-02-2012
    ...
    */
    ...
    public function otpgenerationAction()
    {
		...
		$result_arr = $this->SFA_Comman->executequery('CALL sp_combo_customermaster()','','');
        $this->view->cust_list = $result_arr[0];
		$result_arr = $this->SFA_Comman->executequery('CALL sp_combo_customercategory()','','');
        $this->view->category_list = $result_arr[0];
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "OTP Generation Report";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/otpgeneration/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/otpgeneration/exportcsv";
    }
    /**
    ...
    *
    */
    public function otpgenerationdataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        {
            $extra_where .= ' AND rm.divisioncode =  "'.$this->report_session->post['ddldivision'].'"';
        }
		if(isset($this->report_session->post['ddlroute'] ) && $this->report_session->post['ddlroute']  != "")
        {
            $extra_where .= ' AND otld.routecode =  "'.$this->report_session->post['ddlroute'].'"';
        }
		if(isset($this->report_session->post['ddlcustomer'] ) && $this->report_session->post['ddlcustomer']  != "")
        {
            $extra_where .= ' AND cm.customercode =  "'.$this->report_session->post['ddlcustomer'].'"';
        }
		if(isset($this->report_session->post['ddlcategory'] ) && $this->report_session->post['ddlcategory']  != "")
        {
            $extra_where .= ' AND cm.transaction_anals3 =  "'.$this->report_session->post['ddlcategory'].'"';
        }
        if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        {
            $extra_where  .= ' AND otld.otpdate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        }
        ...
        if(empty($sidx)) {  $sidx  = "otld.customercode";  }
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_otp_generation_list(?,?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['division'],$row['route'],$row['salesman'],$row['customercategory'],$row['customer'],$row['customername'],$row['otptype'],$row['username'],$row['otpdate'],$row['otptime'],$row['fixedlatitude'],$row['fixedlongitude'],$row['comments']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Division'),$this->translate->_('Route code'),$this->translate->_('Salesman Name'),$this->translate->_('Customer Category'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Override Reason for'),$this->translate->_('Password Provided by'),$this->translate->_('Date'),$this->translate->_('Time'),$this->translate->_('Latitude'),$this->translate->_('Longitude'),$this->translate->_('Comments'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['division'],$data[$i]['route'],$data[$i]['salesman'],$data[$i]['customercategory'],$data[$i]['customer'],$data[$i]['customername'],$data[$i]['otptype'],$data[$i]['username'],$data[$i]['otpdate'],$data[$i]['otptime'],$data[$i]['fixedlatitude'],$data[$i]['fixedlongitude'],$data[$i]['comments']);
			// array($data[$i]['itemmustkey'],$data[$i]['itemmustdescription'],$data[$i]['alternatecode'],$data[$i]['categoryname'],$data[$i]['channelname'],$data[$i]['customername'],$data[$i]['itemcode'],$data[$i]['quantity'],$data[$i]['max_quantity'],$data[$i]['quantity']);$row['comments']
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('OTP Generation Report').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "OTP Generation Report";
