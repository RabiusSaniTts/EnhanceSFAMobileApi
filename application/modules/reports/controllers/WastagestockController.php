...
*/
class Reports_WastagestockController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      05-10-2012
    ...
    */
    ...
    public function init()
    {
        ...
        $this->report_session = new Zend_Session_Namespace('Re_wastagestock');
  ...
  /**
    ...
    *
    */
    public function wastagestockAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Wastage Stock";
	...
	if($formdata['ddlfilterby'] != "") {
       ...
       $this->view->searchParams  =  array(
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/wastagestock/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/wastagestock/exportcsv";
    }
    /**
      ...
      *
      */
    public function wastagestockdataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $param_array[5] = $page;
        $param_array[6] = 0;
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_dailyreport_wastagestock(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell'] = array($row['routecode'].' - '.$row['routename'],$row['VisitDate'],$row['VisitTime'],$row['CustomerCode'],$row['Customer'],
                                                 $row['ItemCode'],$row['Item'],$row['UPC'],$row['CasePrice'],$row['UnitPrice'],$row['Location1Qty'],$row['Location2Qty'],$row['Location3Qty'],$row['Expiry']);
                $i++;
    ...
    public function exportAction()
    {
       ...
       $report_title_height = 15;
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Visit Date'),$this->translate->_('Visit Time'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Item Code'),$this->translate->_('Item'),$this->translate->_('UPC'),$this->translate->_('CasePrice'),$this->translate->_('UnitPrice'),$this->translate->_('Location1Qty'),$this->translate->_('Location2Qty'),$this->translate->_('Location3Qty'),$this->translate->_('Expiry'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode'].' - '.$result_arr[0][$i]['routename']][] =  array($result_arr[0][$i]['VisitDate'],
                                        $result_arr[0][$i]['VisitTime'],$result_arr[0][$i]['CustomerCode'],$result_arr[0][$i]['Customer'],$result_arr[0][$i]['ItemCode'], $result_arr[0][$i]['Item'],
                                        $result_arr[0][$i]['UPC'], $result_arr[0][$i]['CasePrice'], $result_arr[0][$i]['UnitPrice'], $result_arr[0][$i]['Location1Qty'],
                                        $result_arr[0][$i]['Location2Qty'], $result_arr[0][$i]['Location3Qty'], $result_arr[0][$i]['Expiry']);
        }
        
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_("Wastage Stock").$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "Wastage_Stock";
