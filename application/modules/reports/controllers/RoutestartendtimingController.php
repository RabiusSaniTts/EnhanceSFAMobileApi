...
*/
class Reports_RoutestartendtimingController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function routestartendtimingsAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Route Start End Timing";
       ...
       //  print_r($this->view->searchParams);
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/routestartendtiming/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/routestartendtiming/exportcsv";
    }
    /**
      ...
      *
      */
    public function routestartendtimingdataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
       ...
       if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        ...
        $param_array[4] = $limit;
        $param_array[5] = $page;
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_route_start_end_timings(?,?,?,?,?)',$param_array,'');
        
        $count  = count($result_arr[1]);//$result_arr[0][0]['counter'];
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['alternateroutecode'],$routename,$row['alternatesalesmancode'],$salesmanname,$row['transactiondate'],$row['firstvisit'],$row['firstinvoice'],$row['lastvisit'],$row['lastinvoice'],$row['validinvoice'],$row['cancelledinvoice']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Salesman Code'),$this->translate->_('Salesman Name'),$this->translate->_('Transaction Date'),$this->translate->_('First Visit Time'),$this->translate->_('First Invoice No'),$this->translate->_('Last Visit Time'),$this->translate->_('Last Invoice No'),$this->translate->_('Valid Invoices'),$this->translate->_('Cancelled Invoices'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['alternateroutecode'],$routename,$data[$i]['alternatesalesmancode'],$salesmanname,$data[$i]['transactiondate'],$data[$i]['firstvisit'],$data[$i]['firstinvoice'],$data[$i]['lastvisit'],$data[$i]['lastinvoice'],$data[$i]['validinvoice'],$data[$i]['cancelledinvoice']);
			
		
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('Route Start End Timing').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "Route Start End Timing";
