...
*/
class Reports_CustomershelfstockController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function customershelfstockAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Customer Shelf Stock";
       ...
       //  print_r($this->view->searchParams);
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/customershelfstock/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/customershelfstock/exportcsv";
    }
    /**
      ...
      *
      */
    public function customershelfstockdataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_customer_shelf_stock(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['salesmancode'],$salesmanname,$row['routecode'],$routename,$row['customercode'],$customername,$row['itemcode'],$itemshortdescription,$row['entrydate'],$row['visitstarttime'],$row['shelfstockcase'],$row['shelfstockeach'],$row['channelcode'],$row['Grouping_Code'],$row['Outlet_Quality']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Salesman Code'),$this->translate->_('Salesman Name'),$this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Customer Code'),$this->translate->_('Customer'),$this->translate->_('Item Code'),$this->translate->_('Item Description'),$this->translate->_('Date Entered'),$this->translate->_('Visit Time'),$this->translate->_('Case Qty'),$this->translate->_('Outer Qty'),$this->translate->_('Channel'),$this->translate->_('Grouping Code'),$this->translate->_('Outlet quality'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['salesmancode'],$salesmanname,$data[$i]['routecode'],$routename,$data[$i]['customercode'], $customername,$data[$i]['itemcode'],$itemshortdescription,$data[$i]['entrydate'],$data[$i]['visitstarttime'],$data[$i]['shelfstockcase'],$data[$i]['shelfstockeach'],$data[$i]['channelcode'],$data[$i]['Grouping_Code'],$data[$i]['Outlet_Quality']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('Customer Shelf Stock').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "Customer Shelf Stock";
