...
*/
class Reports_ReturnsummaryController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      15-02-2012
    ...
    */
    ...
    public function init()
    {
        ...
        $this->report_session		= new Zend_Session_Namespace('Re_returnsummary');
    }
	/**
    * @name       preDispatch
    * @since      26- sep-2012
    ...
    *
    */
    public function returnsummaryAction()
    {
        $this->view->params 	= $params = $this->getRequest()->getParams();
        $this->view->formdata  = $formdata = $this->_request->getPost();
        
        //$result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_returnsummery_detail()','','');
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Return Summary";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/returnsummary/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/returnsummary/exportcsv";
    ...
    /**
      ...
      *
      */
    public function returnsummarydataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_returnsummery(?,?,?,?,?)',$param_array,'');
        ...
        $i=0;
        $total_upc = $total_returnprice = $total_quantity = $total_value = 0;
        if(!empty($result_arr[2])){
            foreach($result_arr[2] as $row) {
                $total_upc += $row['upc'];
				
				$damage_quantity += $row['damagedqty'];
				$damage_value += $row['damagevalue'];
				
				$exp_quantity += $row['expiredqty'];
				$exp_value += $row['expvalue'];
				
				$other_damage_quantity += $row['otherdamagedqty'];
				$other_damage_value += $row['otherdamagevalue'];
				
				$tot_damage_quantity += $row['totaldamage'];
				$tot_damage_value += $row['totaldamagevalue'];


				//  $total_returnprice += $row['returnprice'];
				// $total_quantity += $row['damagedqty'];
                //$total_value += $row['totaldamagevalue'];
                ...
                $responce->rows[$i]['cell']=array($row['transactiondate'],$row['routecode']." - ".$routename,
                                                $row['majorcategorycode']." - ".$row['majorcategroy'],/*$row['description'],*/$row['itemcode'],$row['itemdescription'],
                                                $row['upc'], $row['damagedqty'],$row['damagevalue'],
												$row['expiredqty'], $row['expvalue'],$row['otherdamagedqty'],$row['otherdamagevalue'],
												$row['totaldamage'],$row['totaldamagevalue']
                                                );
		...
		$responce->userdata['upc'] = $total_upc;
		
		$responce->userdata['damagedqty'] = $damage_quantity;
		$responce->userdata['damagevalue'] = $damage_value;
		
		$responce->userdata['expiredqty'] = $exp_quantity;
		$responce->userdata['expvalue'] = $exp_value;
		
		$responce->userdata['otherdamagedqty'] = $other_damage_quantity;
		$responce->userdata['otherdamagevalue'] = $other_damage_value;
		
		$responce->userdata['totaldamage'] = $tot_damage_quantity;
		$responce->userdata['totaldamagevalue'] = $tot_damage_value;
		
       // $responce->userdata['returnprice'] = $total_returnprice;
       // $responce->userdata['quantity'] = $total_quantity;
       // $responce->userdata['value'] = $total_value;
    ...
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Transaction Date'),$this->translate->_('Route Code'),$this->translate->_('Item Group'),$this->translate->_( 'Item Code'),$this->translate->_('Item Description'),$this->translate->_('UPC'),$this->translate->_('Damaged Qty.'),$this->translate->_('Damaged Value'),$this->translate->_('Expired Qty.'), $this->translate->_('Expired Value'),$this->translate->_('Other Damaged Qty.'),$this->translate->_('Other Damaged Value'),$this->translate->_('Total Damaged Qty.'),$this->translate->_('Total Damaged Value'));//$this->translate->_('Return Price (Pcs)'),$this->translate->_('Qty in PCS'),$this->translate->_('Value'));
        $data_arr["columns_config"] =   array(
                                            ...
                                            array("width"=>11),
                                           /* array("width"=>15),*/
                                            array("width"=>45,"toaltext"=>$this->translate->_('Total'),"group_total_text"=>$this->translate->_('Group Total')),
        ...
        for($i = 0; $i < count($result_arr[1]); $i++)
        {
            ...
            $column_model_arr[$result_arr[1][$i]['transactiondate']][$result_arr[1][$i]['routecode']." - ".$routename][$result_arr[1][$i]['majorcategorycode']." - ".$result_arr[1][$i]['majorcategroy']][] =			
            array($result_arr[1][$i]['itemcode'],$result_arr[1][$i]['itemdescription'],$result_arr[1][$i]['upc'],$result_arr[1][$i]['damagedqty'],$result_arr[1][$i]['damagevalue'],
						$result_arr[1][$i]['expiredqty'],$result_arr[1][$i]['expvalue'],$result_arr[1][$i]['otherdamagedqty'],$result_arr[1][$i]['otherdamagevalue'],$result_arr[1][$i]['totaldamage'],$result_arr[1][$i]['totaldamagevalue']	); //$result_arr[0][$i]['returnprice'],$result_arr[0][$i]['damagedqty'],$result_arr[0][$i]['totaldamagevalue']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_('Return Summary').$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "ReturnSummary";
