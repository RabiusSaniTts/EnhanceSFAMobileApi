    ...
    */
	public function useraccessgridAction()
	{
		...
		if(isset($params["key"]) && $params["key"]>0) {
			...
			elseif($params['key'] == 8)
            {
                $table_name = 'divisionmaster';                
                $columns_array 	= array('divisionname AS id','CONCAT(divisioncode," -- ",divisionname) AS val','CONCAT(divisioncode,"$$",divisionname) as edit_del_primary_id');
            }
		}
        ...
        $result = $this->SFA_Comman->executequery('CALL sp_get_admin_security_adduseraccesscode_with_permission_check(?,?,?,?,?,?,?,?,?)',$param_array,'');
	...
	public function itemgridAction()
	{
		...
		// called stored procedure for counter		
      // $result = $this->SFA_Comman->executequery('CALL sp_get_admin_security_adduseraccesscode(?,?,?,?,?,?,?,?)',$param_array,'');		$data_arr["count"] 	    = $result[0][0]['counter'];	
		$data_arr["data"][0] 	= $result[1];
