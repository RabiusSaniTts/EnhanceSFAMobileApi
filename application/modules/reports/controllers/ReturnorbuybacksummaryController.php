<?php
/**
* @name       ReturnorbuybacksummaryController
* @since      10-10-2012
* @version    Release: 1
...
*/
class Reports_ReturnorbuybacksummaryController extends Reports_Library_Controller_Action_Abstract
{
    /**
    * @name       init
    * @since      10-10-2012
    ...
    *
    */
    public function returnorbuybacksummaryAction()
    {
        $this->view->params 	= $params = $this->getRequest()->getParams();
        $this->view->formdata  = $formdata = $this->_request->getPost();
		
        //$result_arr = $this->SFA_Comman->executequery('CALL sp_report_tabletreport_returnorbuybacksummary_detail()','','');
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Return/Buy Back Summary";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/returnorbuybacksummary/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/returnorbuybacksummary/exportcsv";
    }
    /**
      * @name       pdcdataAction
      * @since      10-10-2012
      ...
      *
      */
    public function returnorbuybacksummarydataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_tabletreport_returnorbuybacksummary(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
				...
				$description		= ($this->css == 'ar_') ? $row['arbdescription'] 		: $row['description'];
				
                $responce->rows[$i]['cell'] = array($row['routecode'].' - '.$routename,$row['itemcode'],$itemdescription,$row['quantity'],
      ...
      * @param
      *
      * This action is to export xls file
      *
      */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route Code'),$this->translate->_('ItemCode'),$this->translate->_('Description'),$this->translate->_('Qty (InPcs)'),$this->translate->_('Value'),$this->translate->_('Reason Description'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
			...
			$description		= ($this->css == 'ar_') ? $result_arr[0][$i]['arbdescription'] 		: $result_arr[0][$i]['description'];
				
            $column_model_arr[$result_arr[0][$i]['routecode'].' - '.$routename][] = array($result_arr[0][$i]['itemcode'],$itemdescription,
																							$result_arr[0][$i]['quantity'],
																							$result_arr[0][$i]['value'], $description
																						);
        }
        
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_("Return/Buy Back Summary").$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "ReturnOrBuyBackSummary";
