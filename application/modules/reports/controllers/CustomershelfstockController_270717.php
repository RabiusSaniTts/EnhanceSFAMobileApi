...
class Reports_CustomershelfstockController extends Reports_Library_Controller_Action_Abstract
{
    ...
    public function customershelfstockdataAction()
    {
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routecode'],$routename,$row['entrydate'],$row['visitstarttime'],$row['customercode'], $customername,$row['itemcode'],$row['shelfstockcase'],$row['shelfstockeach'],$itemshortdescription,$row['salesmancode'],$salesmanname,$row['channelcode'],$row['Grouping_Code'],$row['Outlet_Quality']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Date Entered'),$this->translate->_('Visit Time'),$this->translate->_('Customer Code'),$this->translate->_('Customer'),$this->translate->_('Item Code'),$this->translate->_('Case Qty'),$this->translate->_('Outer Qty'),$this->translate->_('Item Description'),$this->translate->_('Salesman Code'),$this->translate->_('Salesman Name'),$this->translate->_('Channel'),$this->translate->_('Grouping Code'),$this->translate->_('Outlet quality'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[$routename][] = array($data[$i]['routecode'],$routename,$data[$i]['entrydate'],$data[$i]['visitstarttime'],$data[$i]['customercode'], $customername,$data[$i]['itemcode'],$data[$i]['shelfstockcase'],$data[$i]['shelfstockeach'],$itemshortdescription,$data[$i]['salesmancode'],$salesmanname,$data[$i]['channelcode'],$data[$i]['Grouping_Code'],$data[$i]['Outlet_Quality']);
        }
