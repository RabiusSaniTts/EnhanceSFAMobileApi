...
*/
class Reports_PenaltysalessummaryController extends Reports_Library_Controller_Action_Abstract
{
    /**
    * @name       init
    * @since      15-02-2012
    ...
    */
    ...
    public function penaltysalessummaryAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Penalty Sales Summary";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/penaltysalessummary/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/penaltysalessummary/exportcsv";
    }
    ...
    /**
      ...
      *
      */
    public function penaltysalessummarydataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_penaltysalessummary_with_permission(?,?,?,?,?,?)',$param_array,'');
        ...
        $i=0;
        $totalsalesamount = $totalreturnamount = $totaldamagedamount =$totalexpiryamount = $totalfreesampleamount = $invoicebalance = $totalpromoamount = $amountpaid = 0;
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
               // print_r($row);
                $responce->rows[$i]['id']=$i;
             //   $netamount = $row['invoiceamount']-$row['discountamount1'];
                $netamount = $row['salesqty'] * $row['salesprice'];
             ...
             //   $invoicebalance	       += $row['invoiceamount'];
              //  $totaldiscount	       += $row['discountamount1'];
			//	$totallinediscount	       += $row['totallinediscount'];
                $totalnetamount	       += $netamount;
              //  $totalimmediatepaid	   += $row['immediatepaid'];
             //   $totalinvoicebalance   += $row['invoicebalance'];
                ...
                $responce->rows[$i]['cell'] = array($row['routecode']." - ".$routename,$row['transactiondate'],
                                                    $row['transactiontime'],$row['invoicenumber'],$row['salesmancode'],$row['customercode'],$customername,
													$row['itemcode'],$row['itemdescription'],$row['salesqty'],$row['salesprice'],
                                                    /*$row['mop'],$row['salesamount'],$row['goodreturnamount'],$row['totaldamagedamount'],
                                                    /*$row['totalexpiryamount'],$row['freeamount'],$row['invoiceamount'],$row['discountamount1'],*/
                                                    $netamount,/*$row['immediatepaid'],/*$row['invoicebalance'],$row['totallinediscount']*/
                                                    );
                $i++;
            }
        }
     ...
     //   $responce->userdata['totalexpiryamount'] = $totalexpiryamount;
        ...
        $responce->userdata['invoiceamount'] = $invoicebalance;
        $responce->userdata['discountamount1'] = $totaldiscount;
        $responce->userdata['netamount'] = $totalnetamount;
        $responce->userdata['immediatepaid'] = $totalimmediatepaid;
        $responce->userdata['invoicebalance'] = $totalinvoicebalance;
	//	$responce->userdata['itemcode'] = $itemcode;
	//	$responce->userdata['itemdescription'] = $itemdescription;
	//	$responce->userdata['salesqty'] = $salesqty;
	//	$responce->userdata['salesprice'] = $salesprice;
		//$responce->userdata['totallinediscount'] = $totallinediscount;
    ...
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route Code'),$this->translate->_( 'Transaction Date'),$this->translate->_('Transaction Time'),$this->translate->_('Invoice Number'),$this->translate->_('Salesman Code'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Payment Type'),$this->translate->_('Sales Amount'),$this->translate->_('Good Return Amount'),$this->translate->_('Bad Return Amount'),/*$this->translate->_('Expiry Return Amount'),*/$this->translate->_('Free Amount'),$this->translate->_('Invoice Amount'),$this->translate->_('Discount Amount'),$this->translate->_('Net Amount'),$this->translate->_('Immediate Paid'),$this->translate->_('Invoice Balance'),$this->translate->_('Total Line Discount'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
         ...
         /*   $column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][$result_arr[0][$i]['transactiondate']][] =
			...
			$column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][] = array($result_arr[0][$i]['transactiondate'],$result_arr[0][$i]['transactiontime'],$result_arr[0][$i]['invoicenumber'],$result_arr[0][$i]['salesmancode'],$result_arr[0][$i]['customercode'],$customername,
                                                    $result_arr[0][$i]['mop'],$result_arr[0][$i]['salesamount'],$result_arr[0][$i]['goodreturnamount'],$result_arr[0][$i]['totaldamagedamount'],
                                                    /*$result_arr[0][$i]['totalexpiryamount'],*/$result_arr[0][$i]['freeamount'],$result_arr[0][$i]['invoiceamount'],$result_arr[0][$i]['discountamount1'],
                                                    $netamount,$result_arr[0][$i]['immediatepaid'],$result_arr[0][$i]['invoicebalance'],$result_arr[0][$i]['totallinediscount']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_("Penalty Sales Summary").$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "PenaltySalesSummary";
