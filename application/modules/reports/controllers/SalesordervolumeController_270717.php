    ...
    public function salesordervolumedataAction()
    {
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routecode'],$routename,$row['salesmancode'],$salesmanname,$row['customercode'],$customername,$row['channel'],$row['invoicenumber'],$row['invoicedate'],$row['invoicepayment'],$row['itemcode'],$itemshortdescription,$row['salesqty'],$row['freeqty'],$row['netqty'],$row['alternateitemgroupcode'],$row['Outlet_Quality']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['routecode'],$routename,$data[$i]['salesmancode'],$salesmanname,$data[$i]['customercode'],$customername,$data[$i]['channel'],$data[$i]['invoicenumber'],$data[$i]['invoicedate'],$data[$i]['invoicepayment'],$data[$i]['itemcode'],$itemshortdescription,$data[$i]['salesqty'],$data[$i]['freeqty'],$data[$i]['netqty'],$data[$i]['alternateitemgroupcode'],$data[$i]['Outlet_Quality']);
        }
