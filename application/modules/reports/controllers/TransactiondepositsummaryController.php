       ...
       // $param_array[6] = $extra_where1;
        ...
        if(!empty($result_arr[2])){
            foreach($result_arr[2] as $row) {
                ...
                $responce->rows[$i]['cell'] = array($row['routecode']." - ".$routename,$row['transactiondate'],$row['trantype'],$row['immediatecash'],$row['immediatecheck'],$row['total']);
                $i++;
            }
        }
        else
    ...
    public function exportAction()
    {
      ...
      /*  if(isset($this->report_session->routecode_str) && $this->report_session->routecode_str != "")
       ...
       // $param_array[6] = $extra_where1;
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Type'),$this->translate->_('Transaction Date'),$this->translate->_('Immediate Cash'),$this->translate->_('Immediate Cheque'),$this->translate->_('Total'));
        ...
        for($i = 0; $i < count($result_arr[1]); $i++)
        {
            ...
            $column_model_arr[$result_arr[1][$i]['routecode']." - ".$routename][$result_arr[1][$i]['trantype']][] = array( $result_arr[1][$i]['transactiondate'],$result_arr[1][$i]['immediatecash'],$result_arr[1][$i]['immediatecheck'],$result_arr[1][$i]['total']);
        }
