...
*/
class Reports_RoutedailytrackController extends Reports_Library_Controller_Action_Abstract
{
    /**
    * @name       init
    * @since      15-02-2012
    ...
    */
    ...
    public function routedailytrackAction()
    {
        ...
        $salesman_arr = $this->SFA_Comman->executequery('CALL sp_get_salesman(?,?)',$param_array,'');        
        
        $this->view->itemgrid    = $this->view->BaseUrl("/".$params['module']."/ajaxdata/useraccessgrid");
        $this->report_session->post = array();

        $this->view->salesmandata = $salesman_arr[0]; 
   }
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "RouteTrack - Daily Report";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/aandpreport/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/aandpreport/exportcsv";
    }
    ...
    /**
      ...
      *
      */
    public function routedailytrackdataAction()
    {
        ...
        {
            $extra_where .= ' AND str.routecode IN ('.$this->report_session->routecode_str.')';
        }
        // if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        // {
        //     $extra_where  .= ' AND str.routestartdate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        // }
        // elseif($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] == "")
        // {
        //     $extra_where  .= ' AND str.routestartdate >= "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'"';
        // }
        // elseif($this->report_session->post['txt_route_start_date'] == "" && $this->report_session->post['txt_route_end_date'] != "")
        // {
        //     $extra_where  .= ' AND str.routestartdate <= "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        // }

        if(isset($this->report_session->post['ddlsalesman']) )//; exit;
        {
               // print_r('Set');
               // var_dump($this->report_session->post['ddlsalesman']);
            $extra_where  .= ' AND str.salesmancode IN('. implode(",", $this->report_session->post['ddlsalesman']) .') ';
        }
        else
        {
              //  print_r('Not Set');
        ...
        $extra_where  .= ' AND YEAR(str.routestartdate) ='. $this->report_session->post['ddlyear'] ;
        $extra_where  .= ' AND MONTH(str.routestartdate) ='. $this->report_session->post['ddlmonth'];
        ...
        //$result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_routedailytrack(?,?,?,?,?)',$param_array,'');
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_routedailytrack(?,?,?,?,?)',$param_array,'');
		...
		$this->view->dtparam = $this->report_session->post['ddlyear'] ." - ". $montharray[$this->report_session->post['ddlmonth']]; //json_encode($result_arr); 
