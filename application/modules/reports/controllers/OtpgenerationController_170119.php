      ...
      /*  if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['route'],$row['salesman'],$row['customercategory'],$row['customer'],$row['customername'],$row['otptype'],$row['username'],$row['otpdate'],$row['otptime'],$row['fixedlatitude'],$row['fixedlongitude']);
                $i++;
    ...
    */
    public function exportAction()
    {
     ...
     /*   if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        ...
        $data_arr["columns"] = array($this->translate->_('Route code'),$this->translate->_('Salesman Name'),$this->translate->_('Customer Category'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Override Reason for'),$this->translate->_('Password Provided by'),$this->translate->_('Date'),$this->translate->_('Time'),$this->translate->_('Latitude'),$this->translate->_('Longitude'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['route'],$data[$i]['salesman'],$data[$i]['customercategory'],$data[$i]['customer'],$data[$i]['customername'],$data[$i]['otptype'],$data[$i]['username'],$data[$i]['otpdate'],$data[$i]['otptime'],$data[$i]['fixedlatitude'],$data[$i]['fixedlongitude']);
			// array($data[$i]['itemmustkey'],$data[$i]['itemmustdescription'],$data[$i]['alternatecode'],$data[$i]['categoryname'],$data[$i]['channelname'],$data[$i]['customername'],$data[$i]['itemcode'],$data[$i]['quantity'],$data[$i]['max_quantity'],$data[$i]['quantity']);
        }
