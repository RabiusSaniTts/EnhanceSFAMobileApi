    ...
    public function exportAction()
    {
       ...
       // $result_arr = $this->SFA_Comman->executequery('CALL sp_report_dataanalysis_itemgroupwisesales(?,?,?,?,?)',$param_array,'');
        ...
        for($i = 0; $i < count($result_arr[1]); $i++)
        {
            ...
            $itemgroup = ($this->css == 'ar_') ? $row['arbitemgroup'] : $result_arr[1][$i]['itemgroupname'];
            $total = $result_arr[1][$i]['salesinvprice'] - ($result_arr[1][$i]['goodretuninvprice']+$result_arr[1][$i]['damageinvprice']+$result_arr[1][$i]['itempromodiscount']);
            
            $column_model_arr[$result_arr[1][$i]['transactiondate']][$result_arr[1][$i]['routecode'].' - '.$routename][$result_arr[1][$i]['itemgroupcode'].' - '.$itemgroup][]
                = array(
                        $result_arr[1][$i]['itemcode'], $itemdescription,
                        $result_arr[1][$i]['upc'],$result_arr[1][$i]['salesqty'], $result_arr[1][$i]['salesinvprice'],$result_arr[1][$i]['returnqty'], $result_arr[1][$i]['goodretuninvprice'],
                        $result_arr[1][$i]['damagedqty'], $result_arr[1][$i]['damageinvprice'], $result_arr[1][$i]['expiryqty'], $result_arr[1][$i]['expiryinvprice'],
                        $result_arr[1][$i]['freesampleqty'], $result_arr[1][$i]['freegodsstdprice'], $result_arr[1][$i]['itempromodiscount'],$result_arr[1][$i]['netsalesinvprice']);
        }
