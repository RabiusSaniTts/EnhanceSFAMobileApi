...
*/
class Reports_SalesordervalueController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function salesordervalueAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Value Sales Order Report";
       ...
       //  print_r($this->view->searchParams);
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/salesordervalue/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/salesordervalue/exportcsv";
    }
    /**
      ...
      *
      */
    public function salesordervaluedataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_value_sales_order(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routecode'],$routename,$row['salesmancode'],$salesmanname,$row['customercode'],$customername,$row['channel'],$row['invoicenumber'],$row['invoicedate'],$row['invoicepayment'],$row['itemcode'],$itemshortdescription,$row['salesqty'],$row['salesvalue'],$row['alternateitemgroupcode'],$row['Outlet_Quality']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Salesman code'),$this->translate->_('Salesman'),$this->translate->_('Customer Code'),$this->translate->_('Customer'),$this->translate->_('channel'),$this->translate->_('Invoice Number'),$this->translate->_('Invoice Date'),$this->translate->_('Invoice Type'),$this->translate->_('Item Code'),$this->translate->_('Item Description'),$this->translate->_('Sales Qty'),$this->translate->_('Sales Value'),$this->translate->_('Grouping Code'),$this->translate->_('Outlet Quality'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['routecode'],$routename,$data[$i]['salesmancode'],$salesmanname,$data[$i]['customercode'],$customername,$data[$i]['channel'],$data[$i]['invoicenumber'],$data[$i]['invoicedate'],$data[$i]['invoicepayment'],$data[$i]['itemcode'],$itemshortdescription,$data[$i]['salesqty'],$data[$i]['salesvalue'],$data[$i]['alternateitemgroupcode'],$data[$i]['Outlet_Quality']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('Value Sales Order Report').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "Value Sales Order Report";
