...
*/
class Reports_CustomervisitdetController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function customervisitAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Customer Visit Detail";
       ...
       //  print_r($this->view->searchParams);
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/customervisitdet/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/customervisitdet/exportcsv";
    }
    /**
      ...
      *
      */
    public function customervisitdetAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
     ...
     //   echo "here"; exit;
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_customer_visit_details(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routekey'],$row['transactiondate'],$row['alternateroutecode'],$routename,$row['alternatesalesmancode'],$salesmanname,$row['customeraltcode'],$customername,$row['channelname'],($row['schelduledflag'] == 1 ? "YES":"NO"),($row['servicedflag'] == 1 ? "YES":"NO"));
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route code'),$this->translate->_('Trip ID'),$this->translate->_('Transaction Date'),$this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Salesman Code'),$this->translate->_('Salesman Name'),$this->translate->_('Customer Code'),$this->translate->_('Customer'),$this->translate->_('Channel'),$this->translate->_('Visit Scheduled'),$this->translate->_('Serviced'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[$routename][] = array($data[$i]['routekey'],$data[$i]['transactiondate'],$data[$i]['alternateroutecode'],$routename,$data[$i]['alternatesalesmancode'],$salesmanname,$data[$i]['customeraltcode'],$customername,$data[$i]['channelname'],($data[$i]['schelduledflag'] == 1 ? "YES":"NO"),($data[$i]['servicedflag'] == 1 ? "YES":"NO"));
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('Customer Visit Detail').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "Customer Visit Detail";
