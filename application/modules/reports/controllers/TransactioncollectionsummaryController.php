...
*/
class Reports_TransactioncollectionsummaryController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function init()
    {
		...
		$this->report_session	= new Zend_Session_Namespace('Re_collectionsummary');
    }
    ...
    */
    ...
    public function collectionsummaryAction()
    {
        $this->view->params 	= $params = $this->getRequest()->getParams();
        $this->view->formdata  = $formdata = $this->_request->getPost();
        
        //$result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_collectionsummary_detail()','','');
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Collection Summary";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/transactioncollectionsummary/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/transactioncollectionsummary/exportcsv";
    ...
    /**
      ...
      *
      */
    public function collectionsumAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        {
            $extra_where .= ' AND arh.routecode IN ('.$this->report_session->routecode_str.')';
        }
        if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        {
            $extra_where  .= ' AND arh.transactiondate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        }
        elseif($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] == "")
        {
            $extra_where  .= ' AND arh.transactiondate >= "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'"';
        }
        elseif($this->report_session->post['txt_route_start_date'] == "" && $this->report_session->post['txt_route_end_date'] != "")
        {
            $extra_where  .= ' AND arh.transactiondate <= "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        }
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_collectionsummary(?,?,?,?,?)',$param_array,'');
        ...
        $i=0;
        $total_arcash = $total_archeck = $total_totalinvoiceamount = 0;
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                $total_arcash += $row['arcash'];
                $total_archeck += $row['archeck'];
                $total_totalinvoiceamount += $row['totalinvoiceamount'];
                
				$routename 		= ($this->css == 'ar_') ? $row['arbroutename'] 		: $row['routename'];
				$customername	= ($this->css == 'ar_') ? $row['arbcustomername'] 	: $row['customername'];
				
                $responce->rows[$i]['id']=$i;
                $responce->rows[$i]['cell']=array($row['routecode']." - ".$routename,$row['documentnumber'],$row['transactiondate'],
                                                $row['customercode']." - ".$customername, $row['arcash'], $row['archeck'], $row['totalinvoiceamount'],
                                                $row['alternateinvoicenumber'], $row['invoicedby'], $row['invoicedate'], $row['amountpaid']);
                $i++;
            }
        }
        else
        ...
        $responce->userdata['totalinvoiceamount'] = $total_totalinvoiceamount;
    ...
    */
    public function exportAction()
    {
        ...
        $param_array = array();
        $param_array[1] = $extra_where;
        $param_array[2] = "routecode,documentnumber,".$sidx;
        $param_array[3] = $sord;
        $param_array[4] = $limit;
        $param_array[5] = $page;
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Receipt Number'),$this->translate->_('Transaction Date'),$this->translate->_('Customer'),$this->translate->_( 'Amount Paid in CASH'),$this->translate->_( 'Amount Paid in CHEQUE'),$this->translate->_('Total Amount'),$this->translate->_('Against Invoice'),$this->translate->_('Invoiced By'),$this->translate->_('Invoiced Date'),$this->translate->_('Paid Amount'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][$result_arr[0][$i]['documentnumber']][] = array($result_arr[0][$i]['transactiondate'],
                                        $result_arr[0][$i]['customercode']." - ".$customername, $result_arr[0][$i]['arcash'], $result_arr[0][$i]['archeck'], $result_arr[0][$i]['totalinvoiceamount'],
                                        $result_arr[0][$i]['alternateinvoicenumber'], $result_arr[0][$i]['invoicedby'], $result_arr[0][$i]['invoicedate'], $result_arr[0][$i]['amountpaid']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_('Collection Summary').$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "CollectionSummary";
