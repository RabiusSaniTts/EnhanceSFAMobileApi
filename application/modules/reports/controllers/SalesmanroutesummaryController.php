    ...
    */
    ...
    public function salesmanroutesummaryAction()
    {
        ...
        if(isset($this->report_session->routecode_str) && $this->report_session->routecode_str != "")
        {
			...
			$temp_where = $this->report_session->routecode_str;//' AND sed.routecode IN ('.$this->report_session->routecode_str.')';
			//$temp_where1 .= ' AND sed.routecode IN ('.$this->report_session->routecode_str.')';
        }
        if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        {
            $extra_where  .= ' AND sed.routeenddate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
				// $temp_where  .= ' AND sed.routeenddate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'"  AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
			// $temp_where1  .= ' AND sed.routeenddate BETWEEN DATE_SUB("'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'", INTERVAL 2 MONTH)  AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
			// $temp_where1  .= ' AND routeenddate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
       ...
       /* elseif($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] == "")
        {
			...
			// $temp_where  .= ' AND sed.routeenddate BETWEEN DATE_SUB("'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'", INTERVAL 2 MONTH)  AND "'.date("Y-m-d")).'"';
        }
        elseif($this->report_session->post['txt_route_start_date'] == "" && $this->report_session->post['txt_route_end_date'] != "")
        {
            $extra_where  .= ' AND routeenddate <= "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
			// $temp_where  .= ' AND sed.routeenddate BETWEEN DATE_SUB("'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'", INTERVAL 2 MONTH)  AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        }
        */
        ...
        if(empty($sord)) {  $sord  = "asc"; }
		 $startdate = date("Y-m-d", strtotime($this->report_session->post['txt_route_start_date']));
		 $enddate = date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date']));
        $param_array = array();
        ...
        $param_array[6] = $limit;
        $param_array[7] = $page;
		$param_array[8] = $temp_where;
		//$param_array[9] = $temp_where1;
      //  print_r($param_array);
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_salesman_route_summary_modified(?,?,?,?,?,?,?)',$param_array,'');
       ...
       /* if(isset($this->report_session->routecode_str) && $this->report_session->routecode_str != "")
        {
			...
			$temp_where .= ' AND sed.routecode IN ('.$this->report_session->routecode_str.')';
			$temp_where1 .= ' AND sed.routecode IN ('.$this->report_session->routecode_str.')';
        }
        if($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] != "")
        {
            $extra_where  .= ' AND routeenddate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
			 $temp_where  .= ' AND sed.routeenddate BETWEEN DATE_SUB("'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'", INTERVAL 2 MONTH)  AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
			  $temp_where1  .= ' AND routeenddate BETWEEN "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_start_date'])).'" AND "'.date("Y-m-d",strtotime($this->report_session->post['txt_route_end_date'])).'"';
        }
        elseif($this->report_session->post['txt_route_start_date'] != "" && $this->report_session->post['txt_route_end_date'] == "")
        {
