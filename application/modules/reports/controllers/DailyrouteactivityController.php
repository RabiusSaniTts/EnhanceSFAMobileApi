...
*/
class Reports_DailyrouteactivityController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      15-02-2012
  ...
  /**
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Daily Route Activity";
        $this->view->pageHeaderTitle  = $this->translate->_('Date');
        $this->view->pageHeadervalue  =  date("m/d/Y h:i:s");
        
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/dailyrouteactivity/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/dailyrouteactivity/exportcsv";
    ...
    public function dailyrouteactivitydataAction()
    {
       ...
       /* if($this->report_session->post['txt_route_end_date'] != "" )
        ...
        //    $result_arr = $this->SFA_Comman->executequery('CALL sp_report_dailyreport_dailyrouteactivity(?,?,?,?,?)',$param_array,'');
			$result_arr = $this->SFA_Comman->executequery('CALL sp_report_dailyreport_dailyrouteactivity_new(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[0])) {
            ...
            {
                $address="";$customer="";
                $total_tranamount += $result_arr[0][$j]['tranamount'];
                $total_salePaid += $result_arr[0][$j]['salePaid'];
				...
				$result_arr[0][$j]['trandate'],
				$result_arr[0][$j]['routecode'].'-'.$routename,
				$result_arr[0][$j]['documentno'],
				$customer,
				$address,
				$result_arr[0][$j]['trandate'],
				$result_arr[0][$j]['logstarttime'],
				$result_arr[0][$j]['logendtime'], 
				$this->gettimediff($result_arr[0][$i]['logstarttime'] , $result_arr[0][$i]['logendtime']) ,
				$result_arr[0][$j]['visitstarttime'],
				$result_arr[0][$j]['visitendtime'], 
				$this->gettimediff($result_arr[0][$j]['visitstarttime'] , $result_arr[0][$j]['visitendtime']) ,
				$result_arr[0][$j]['trantype'],
				$result_arr[0][$j]['tranamount'],
				$result_arr[0][$j]['salePaid'],
				$result_arr[0][$j]['receiptPaid']
				);
            }
        }
        $responce->userdata['trantype'] = $this->translate->_("Total");
        $responce->userdata['tranamount'] = $total_tranamount;
        $responce->userdata['salePaid'] = $total_salePaid;
    ...
    public function exportAction()
    {
      ...
      /*  if($this->report_session->post['txt_route_end_date'] != "" )
		...
		$this->translate->_('Transaction Date'),
		$this->translate->_('Document Number'),
		$this->translate->_('Customer'),
		$this->translate->_('Address'),
		$this->translate->_('Transaction Date'),
		$this->translate->_('Visit Start Time'),
		$this->translate->_('Visit End Time'),
		$this->translate->_('Visit Duration hh:mm'),
		$this->translate->_('Activity Start Time'),
		$this->translate->_('Activity End Time'),
		$this->translate->_('Duration hh:mm'),
		$this->translate->_('Transaction'),
		$this->translate->_('Total Transaction Amount'),
		$this->translate->_('Invoice Amount Collected'),
		$this->translate->_('Receipt Amount Collected'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode'].'-'.$routename][$result_arr[0][$i]['trandate']][] = array(
			$result_arr[0][$i]['documentno'],
			$customer,$address,$result_arr[0][$i]['trandate'],
			$result_arr[0][$i]['logstarttime'],
			$result_arr[0][$i]['logendtime'], 
			$this->gettimediff($result_arr[0][$i]['logstarttime'] , $result_arr[0][$i]['logendtime']) ,
			$result_arr[0][$i]['visitstarttime'],
			$result_arr[0][$i]['visitendtime'], 
			$this->gettimediff($result_arr[0][$i]['visitstarttime'] , $result_arr[0][$i]['visitendtime']) ,
			$result_arr[0][$i]['trantype'],
			$result_arr[0][$i]['tranamount'],
			$result_arr[0][$i]['salePaid'],
			$result_arr[0][$i]['receiptPaid']);
        }
        
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_("Daily Route Activity").$report_title;
        $data_arr["config"]["report_title_height"] = 30;
        $data_arr["config"]["file_name"]    = "DailyRouteActivity";
