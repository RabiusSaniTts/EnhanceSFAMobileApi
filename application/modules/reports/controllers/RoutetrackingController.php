        ...
        $this->report_session->post = $formdata;
		 if($formdata['ddlfilterby'] != "") {
                ...
                if($formdata['ddlfilterby'] != 6) {
					...
					$param_array[3] = 1;//$formdata["showcustomer"];	
					$this->view->showcustomer = 1;					
					$result_arr = $this->SFA_Comman->executequery('CALL sp_get_report_routetracking(?,?,?)',$param_array,'');
					$this->view->mapdata =  $result_arr[0];
					$this->view->startmapdata =  $result_arr[1];
					foreach ($result_arr[0] as $row){
						$routepath  .= '"'.$row["latitude"].",".$row["longitude"].'",';
					}
					$this->view->routepath =  substr($routepath, 0, strlen($routepath)-1);
		 }
