...
*/
class Reports_TransactionfocController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function init()
    {
        ...
        $this->report_session		= new Zend_Session_Namespace('Re_focsummary');
    }
    ...
    */
    ...
    public function focsummaryAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "FOC Summary";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/transactionfoc/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/transactionfoc/exportcsv";
    }
    /**
    ...
    public function focAction()
    {
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_foc(?,?,?,?,?)',$param_array,'');        
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routecode']." - ".$routename,date("d M Y",strtotime($row['actualtransactiondate'])),$row['transactiontime'],$row['invoicenumber'],$row['salesmancode'],$row['customercode'],$customername,$row['totalsalesamount'], $row['totalfreesampleamount'], $row['totalinvoiceamount'],$row['totaldiscountamount'],$row['netamount']);
                $i++;
            }
        }
        else
    ...
    */
    public function exportAction()
    {
        ...
        $param_array = array();
        $param_array[1] = $extra_where;
        $param_array[2] = "routecode,actualtransactiondate,".$sidx;
        $param_array[3] = $sord;
        $param_array[4] = $limit;
        $param_array[5] = $page;
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Transaction Date'),$this->translate->_('Transaction Time'),$this->translate->_('Invoice Number'),$this->translate->_('Salesman Code'),$this->translate->_('Customer Code'),$this->translate->_( 'Customer Name'),$this->translate->_('Sales Amount'),$this->translate->_('Free Amount'),$this->translate->_('Invoice Amount'),$this->translate->_('Discount Amount'),$this->translate->_('Net Amount'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][date("d M Y",strtotime($result_arr[0][$i]['actualtransactiondate']))][] = array($result_arr[0][$i]['transactiontime'],$result_arr[0][$i]['invoicenumber'],$result_arr[0][$i]['salesmancode'],$result_arr[0][$i]['customercode'],$customername,$result_arr[0][$i]['totalsalesamount'], $result_arr[0][$i]['totalfreesampleamount'], $result_arr[0][$i]['totalinvoiceamount'],$result_arr[0][$i]['totaldiscountamount'],$result_arr[0][$i]['netamount']);
        }
        
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('FOC').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "FOC";
