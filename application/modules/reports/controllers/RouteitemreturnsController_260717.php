...
*/
class Reports_RouteitemreturnsController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function routeitemreturnsAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Route Item Returns";
       ...
       //  print_r($this->view->searchParams);
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/routeitemreturns/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/routeitemreturns/exportcsv";
    }
    /**
      ...
      *
      */
    public function routeitemreturndataAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
     ...
     //   echo "here"; exit;
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_route_item_returns(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['salesmancode'],$salesmanname,$row['routecode'],$routename,$row['description'],$row['transactiondate'],$row['invoicenumber'],$row['customercode'],$customername,$row['itemcode'],$itemshortdescription,$row['unitspercase'],$row['quantity'],$row['returncaseprice'],$row['returnprice'],$row['returnamount'],$row['alternateitemgroupcode'],$row['Outlet_Quality']);
                $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Salesman code'),$this->translate->_('Salesman'),$this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Invoice Type'),$this->translate->_('Date'),$this->translate->_('Invoice Number'),$this->translate->_('Customer Code'),$this->translate->_('Customer'),$this->translate->_('Item Code'),$this->translate->_('Item Description'),$this->translate->_('UPC'),$this->translate->_('Return Qty'),$this->translate->_('Case Price'),$this->translate->_('Unit Price'),$this->translate->_('Return Amount'),$this->translate->_('Grouping Code'),$this->translate->_('Outlet Quality'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
            ...
            $column_model_arr[] = array($data[$i]['salesmancode'],$salesmanname,$data[$i]['routecode'],$routename,$data[$i]['description'],$data[$i]['transactiondate'],$data[$i]['invoicenumber'],$data[$i]['customercode'],$customername,$data[$i]['itemcode'],$itemshortdescription,$data[$i]['unitspercase'],$data[$i]['quantity'],$data[$i]['returncaseprice'],$data[$i]['returnprice'],$data[$i]['returnamount'],$data[$i]['alternateitemgroupcode'],$data[$i]['Outlet_Quality']);/*array($data[$i]['routekey'],$data[$i]['transactiondate'],$data[$i]['alternateroutecode'],$routename,$data[$i]['alternatesalesmancode'],$salesmanname,$data[$i]['customeraltcode'],$customername,$data[$i]['channelname'],($data[$i]['schelduledflag'] == 1 ? "YES":"NO"),($data[$i]['servicedflag'] == 1 ? "YES":"NO"));*/
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('Route Item Returns').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "Route Item Returns";
