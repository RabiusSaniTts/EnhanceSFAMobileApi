...
class Hhctransaction_InventoryController extends Hhctransaction_Library_Controller_Action_Abstract
{
    ...
    */
    public function inventorysummaryAction()
    {
		...
		$cols_array 	= array('sed.routecode','rm.routename', 'sed.salesmancode', 'DATE_FORMAT(sed.routestartdate,"%d-%m-%Y") AS routestartdate', 'DATE_FORMAT(sed.routeenddate,"%d-%m-%Y") AS routeenddate','sed.versionno','IF(sed.routeclosed = 0,"No","Yes") AS routeclosed','sed.routekey AS edit_del_primary_id');
		$columns_show 	= array($this->translate->_('Route Code'),$this->translate->_('Route Name'),$this->translate->_('Salesman Code'),$this->translate->_('Route Startdate'),$this->translate->_('Route Enddate'),$this->translate->_('VersionNo'),$this->translate->_('Route Close'));
    ...
    */
    public function viewcustomerinventorygridAction()
    {
		...
		$columns_show  	= array($this->translate->_('Item Code'),$this->translate->_('Item Description'),$this->translate->_('UPC'),
								$this->translate->_('Location1'),$this->translate->_('Location2'),$this->translate->_('Location3'),$this->translate->_('Expiry Date'));
		
		$cpanel				= $this->SFA_Comman->getaltcodestatus();
		$altcode_status		= $cpanel["Use Alternate Code"]['status'];
		
		$columns_array 	= array('im.alternatecode','im.itemdescription','im.unitspercase','CONCAT(cid.qtyloc1case,"/",cid.qtyloc1each) location1','CONCAT(cid.qtyloc2case,"/",cid.qtyloc2each) location2','CONCAT(cid.qtyloc3case,"/",cid.qtyloc3each) location3','DATE_FORMAT(cid.expirydate,"%Y-%m-%d") expirydate');
