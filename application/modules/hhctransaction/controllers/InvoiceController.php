...
class Hhctransaction_InvoiceController extends Hhctransaction_Library_Controller_Action_Abstract
{
    ...
    */
	...
	if(isset($params["key"]) && $params["key"]>0)
		...
		$columns_array		= array('im.actualitemcode','im.itemshortdescription','im.unitspercase',
				...
				'FORMAT(promoamount,'.$this->decimalplaces.') AS discount','FORMAT((Salesitemgsttax+salesitemexcisetax),'.$this->decimalplaces.')  as salestax','FORMAT((returnitemgsttax+damageditemgsttax+returnitemexcisetax+damageditemexcisetax),'.$this->decimalplaces.')  as rtntax','FORMAT((((FLOOR(salesqty/im.unitspercase)*id.salescaseprice)+((salesqty%im.unitspercase)*id.salesprice))-((FLOOR(returnqty/im.unitspercase)*id.returncaseprice)+((returnqty%im.unitspercase)*id.returnprice))
				-((FLOOR(damagedqty/im.unitspercase)*id.returncaseprice)+((damagedqty%im.unitspercase)*id.returnprice))-promoamount)+(Salesitemgsttax+salesitemexcisetax+returnitemgsttax+damageditemgsttax+returnitemexcisetax+damageditemexcisetax),'.$this->decimalplaces.') AS total_amount','1 AS sett_tran_type','id.transactionkey as edit_del_primary_id'
				);
		
		$columns_show  = array($item_code,$item_name,$upc,$sales_case_price,$sales_pcs_price,$return_case_price,$return_pcs_price,$case_sales_qty,$pcs_sales_qty,$case_return_qty,$pcs_return_qty,$case_damage_qty,$pcs_damage_qty,$case_free_qty,$pcs_free_qty,$case_free_qty,$pcs_free_qty,$discount,'Sales','Return',$total_amount);
	...
	$pagingparams = array(
			...
			"show_top_columns_value" => array(array("3",""),array("4","Sales"),array("4","Return"),array("2","Damage"),array("2","Free"),array("3","Promotion"),array("2","Tax"),array("1",""),array("1","")),
			...
			"show_columns_right_side" =>array('salescaseprice','salesprice','returncaseprice','returnprice',"case_sales_qty","pcs_sales_qty","case_return_qty","pcs_return_qty","case_damage_qty","pcs_damage_qty","case_manualfreeqty_qty","pcs_manualfreeqty_qty","discount","salestax","rtntax","total_amount",'casediscount','pcsdiscount'),
			"show_header_right_side"=>array($this->translate->_('Case Price'),$this->translate->_('Pcs Price'),$this->translate->_('CAS'),$this->translate->_('PCS'),$this->translate->_('Discount'),$this->translate->_('Sales'),$this->translate->_('Return'),$this->translate->_('Total Amount')),
    ...
    public function invoicedetailoldAction()
    {
	$params 		= $this->getRequest()->getParams();
	...
	if(isset($params["key"]) && $params["key"]>0)
		...
		$columns_array		= array('im.actualitemcode','im.itemshortdescription','im.unitspercase',
				...
				'FLOOR((manualfreeqty/im.unitspercase)) AS case_manualfreeqty_qty','(manualfreeqty%im.unitspercase) AS pcs_manualfreeqty_qty',
				'FLOOR((freesampleqty/im.unitspercase)) AS casediscount','(freesampleqty%im.unitspercase) AS pcsdiscount',
				'FORMAT(promoamount,'.$this->decimalplaces.') AS discount','FLOOR((returnfreeqty/im.unitspercase)) AS case_returnfree_qty','(returnfreeqty%im.unitspercase) AS pcs_returnfree_qty',
				'FORMAT((((FLOOR(salesqty/im.unitspercase)*id.salescaseprice)+((salesqty%im.unitspercase)*id.salesprice))-((FLOOR(returnqty/im.unitspercase)*id.returncaseprice)+((returnqty%im.unitspercase)*id.returnprice))
	...
	$pagingparams = array(
			...
			"show_columns" => $columns_show,
			"show_top_columns" => true,
			"show_top_columns_value" => array(array("3",""),array("4","Sales"),array("4","Return"),array("2","Damage"),array("2","Free"),array("3","Promotion"),array("2","Buy Back"),array("1",""),array("1","")),
			"show_total_columns"=>true,			
			"show_total_columns_value"=>array("case_sales_qty"=>"0","pcs_sales_qty"=>"0","case_return_qty"=>"0","pcs_return_qty"=>"0","case_damage_qty"=>"0","pcs_damage_qty"=>"0","case_manualfreeqty_qty"=>"0","pcs_manualfreeqty_qty"=>"0","total_amount"=>"1","casediscount"=>"0","pcsdiscount"=>"0"),
			"show_total_columns_msg"=>array("itemshortdescription","Total",$this->decimalplaces),
    ...
    */
	...
	if(isset($params["key"]) && $params["key"]>0)
		...
		if($altcode_status)
		{
			...
			$columns_array 	=  array('im.alternatecode','im.itemshortdescription','im.unitspercase',
						...
						'FORMAT((salesordervat+salesorderexcisetax),'.$this->decimalplaces.') AS salestax','FORMAT((returnvat+returnexcisetax+damagedvat+damagedexcisetax),'.$this->decimalplaces.') AS returntax',
						'FORMAT((((FLOOR(salesqty/im.unitspercase)*id.salescaseprice)+
						((salesqty%im.unitspercase)*id.salesprice))-((FLOOR(returnqty/im.unitspercase)*id.returncaseprice)
						+((returnqty%im.unitspercase)*id.returnprice))-((FLOOR(damagedqty/im.unitspercase)*id.returncaseprice)
						+((damagedqty%im.unitspercase)*id.returnprice))-promoamount)+(returnvat+returnexcisetax+damagedvat+damagedexcisetax+salesordervat+salesorderexcisetax),'.$this->decimalplaces.') AS total_amount'
						);
			// column header to be displayed
			$columns_show  = array($alt_code,$item_name,$upc,$sales_case_price,$sales_pcs_price,$return_case_price,$return_pcs_price,$case_sales_qty,$pcs_sales_qty,$case_return_qty,$pcs_return_qty,$case_damage_qty,$pcs_damage_qty,$case_free_qty,$pcs_free_qty,$case_free_qty,$pcs_free_qty,$discount,'Sales','Return',$total_amount);
		}
		else
		{
			// column to be fetched	
	...
	$pagingparams = array(
			...
			"show_columns" => $columns_show,
			"show_top_columns" => true,
			"show_top_columns_value" => array(array("3",""),array("4","Order"),array("4","Return"),array("2"," Damage"),array("2","Free"),array("3","Promotion"),array("2","Tax"),array("1","")),
			"show_total_columns"=>true,			
    ...
    public function salesorderdetailoldAction()
    {
	$params 		= $this->getRequest()->getParams();
