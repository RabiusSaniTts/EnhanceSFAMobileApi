...
class Hhctransaction_LoadstockController extends Hhctransaction_Library_Controller_Action_Abstract
{
    ...
    */
    public function loadrequestAction()
    {
		...
		$additional_where_condition = array();
		$additional_where_condition[] = "  ith.transactiontype = 4 and ith.EG_LR_APPROVED = 1";
		$additional_where_condition[] = " (ith.transactiondate BETWEEN \'".date("Y-m-d 00:00:00",strtotime(str_replace('/', '-', $Common_NameSpace->tdate)))."\' AND \'".date("Y-m-d 23:59:59",strtotime(str_replace('/', '-', $Common_NameSpace->tdate)))."\' )";
    ...
    */
    public function viewloadrequestgridAction()
    {
		...
		$columns_show  	= array(
									$this->translate->_('Item Code'),$this->translate->_('Item Name'),$this->translate->_('UPC'),
									$this->translate->_('Case Price'),$this->translate->_('Pcs Price'),
									 $this->translate->_('Van Stock'),
									$this->translate->_('Case'),
		...
		$columns_array 	= array('itd.itemcode','im.itemdescription','im.unitspercase','FORMAT(itd.itemcaseprice,'.$this->decimalplaces.') as itemcaseprice','FORMAT(itd.itemprice,'.$this->decimalplaces.') as itemprice',
'CONCAT( IFNULL(FLOOR(isd.vanstock/im.unitspercase),0), "/" ,IFNULL((isd.vanstock%im.unitspercase),0)) AS vanstock',

			'FLOOR(itd.quantity/im.unitspercase) AS cases',

			'(itd.quantity%im.unitspercase) AS pieces','FORMAT(((itd.itemcaseprice)*FLOOR(itd.quantity/im.unitspercase))+(itd.itemprice*(itd.quantity%im.unitspercase)),'.$this->decimalplaces.') AS total_amt','1 AS sett_tran_type','primary_key AS edit_del_primary_id');
		...
		$pagingparams = array(
					 ...
					 "show_deleteall" => false,
					 "primaryid" => "transactiontypecode",
					 "noeditfields" => array('alternatecode','itemcode','itemdescription','unitspercase','itemcaseprice','itemprice','total_amt','vanstock'),
					 "currentlink" => array("/".$params['module']."/".$params['controller']."/".$params['action'].$ex_param),
					 "deletelink" => array("/id/#pattern#/delete/yes/msg/curr","#pattern#"),
					 "editlink" => array("/id/#pattern#/edit/yes","#pattern#"),
					 "nodata_message" => $this->translate->_('No Record(s) Found'),
    ...
    */
	...
	/**
    * @name       loadrequestApproveAction
    * @since      01-02-2012
    * @version    Release: 1
    * @author     M@M <jyothish.t@wjtowell.com>
    * @copyright  Elan Technologies
    * @param
    *
    * This action is for display loadrequest
    *
    */
    public function loadrequestapproveAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		...
		$additional_where_condition = array();
		$additional_where_condition[] = "  ith.transactiontype = 4 and ith.EG_LR_APPROVED = 0";
		$additional_where_condition[] = " (ith.transactiondate BETWEEN \'".date("Y-m-d 00:00:00",strtotime(str_replace('/', '-', $Common_NameSpace->tdate)))."\' AND \'".date("Y-m-d 23:59:59",strtotime(str_replace('/', '-', $Common_NameSpace->tdate)))."\' )";
		...
		$pagingparams = array(
				...
				"primaryid" => "detailkey",
				"show_extralink" => true,
				"extralink" => array(array("View","/".$params['module']."/".$params['controller']."/viewloadrequestapprove/id/#pattern#","#pattern#")),
				"nodata_message" => $this->translate->_('No Record(s) Found'),
				"fetch_columns_inquery" => $cols_array,
				"show_columns" => $columns_show,
				"additional_where" => $additional_where_condition
    ...
    * @version    Release: 1
    * @author     HD <hiren.d@elantechnologies.com>
    * @copyright  TOWELL TAKE SOLUTIONS
    * @param
    *
    * This action is for view load
    *
    */
    public function viewloadrequestapproveAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		...
		$param_array_header[3]  = $formdata['ddldepotroute'];
		$param_array_header[4]  = $reqdate;	
		$param_array_header[5]  = $this->currentUser->username;	
		$return_header = $this->SFA_Comman->executequery('CALL sp_APPROVE_loadstock_inventorytransactionheader(?,?,?,?,?)',$param_array_header,'');
		SFA_Message::setMsg($this->translate->_('Update Record'));
		$this->_helper->redirector("loadrequestapprove", "loadstock", "hhctransaction");
		}
