...
*/
class Promotions_CustomerController extends Promotions_Library_Controller_Action_Abstract
{
    /**
    * @name       init
    * @since      6-02-2012
    ...
    */
	...
	if($formdata["hdDelete"]==1)
	{
		...
		$result 	= $this->SFA_Comman->executequery('CALL sp_delete_promotions_customer_freegoods(?,?)',$param_array,'');
	...
	$this->view->title 	= $this->translate->_('Customer Free Contract');

	$id				= $this->translate->_('Contract ID');
	$code			= $this->translate->_('Code');
	$custoemr		= $this->translate->_('Customer');
	...
	$cols_array    = array('contractid','cf.customercode','cm.customername AS customername','DATE_FORMAT(cf.creationdate,"%d-%m-%Y") as creationdate','DATE_FORMAT(cf.enddate,"%d-%m-%Y") AS enddate','CASE WHEN CURDATE() < DATE(cf.startdate) THEN "Pending" WHEN DATE(cf.startdate) <= CURDATE() AND DATE(cf.enddate) >= CURDATE() THEN "Running" WHEN CURDATE() >  DATE(cf.enddate) THEN "Ended"  END AS `status`');
	...
	$pagingparams = array(
				 ...
				 "pagename" => $this->translate->_('Customer Free Contract'),
			     ...
			     "deletelink" => array("/promotions/customer/freegoods/id/#pattern#/delete/yes/","#pattern#"),
			     "show_deleteall" => false,
			     "primaryid" => "contractid",
			     "editlink" => array("/promotions/customer/addfreegoods/edit/yes/id/#pattern#","#pattern#"),
	...
	$rowcount = $this->SFA_Comman->executequery('CALL sp_get_promotions_customer_freegoods(?,?,?,?,?,?,?,?)',$param_array,'',$downloadCSV,$printData,2,$pagingparams);
    ...
    */
    public function addfreegoodsAction()
    {
		...
		if($formdata['txtstartdate']!="" && $formdata['txtenddate']!="")
		{
			...
			$result = $this->SFA_Comman->executequery('CALL sp_edit_promotions_customer_addfreegoods(?,?,?,?,?,?,?,?)',$param_array,'');
			...
			$result = $this->SFA_Comman->executequery('CALL sp_add_promotions_customer_addfreegoods(?,?,?,?,?,?,?,?)',$param_array,'');
			...
			$this->_helper->redirector('freegoods', 'customer', 'promotions'); 
		   
		  
		}
		elseif($params['id'] > 0)
		{
			$result  			= $this->SFA_Comman->executequery('CALL sp_get_promotions_customer_addfreegoods(?)',$params['id'],'');
			
			$res['contractid'] 		= $result[0][0]['contractid'];
			$res['customercode'] 	= $result[0][0]['customercode'];
		...
		{
			$result  			= $this->SFA_Comman->executequery('CALL sp_get_promotions_customer_addfreegoods(?)','0','');
    ...
    */
    public function freegoodsgridAction()
    {
		...
		if($params["update"]=="yes") {
			...
			{
			$r_add = $this->SFA_Comman->executequery('CALL sp_edit_promotions_customer_freegoodsgrid(?,?,?,?)',$paramarry,'');
		...
		if($params["add"]=="yes") {
		  ...
		  if($formdata['ddl_itemcode'] && $formdata['txt_freequantity']){
			if($formdata['hdnid'] > 0 || $params['key'] > 0)
			{
				...
				$r_add = $this->SFA_Comman->executequery('CALL sp_add_promotions_customer_freegoodsgrid(?,?,?,?)',$paramarry,'');
			...
			else
			{
				...
				$paramarry[5]  = $formdata['chk_active'];
				$paramarry[6]  = $formdata['ddl_depotcode'];
				...
				$r_add = $this->SFA_Comman->executequery('CALL sp_add_promotions_customer_addfreegoodsgriddetail(?,?,?,?,?,?,?,?,?,?)',$paramarry,'');
	...
	if($params["delete"]=="yes"){
	    ...
	    $r_delete = $this->SFA_Comman->executequery('CALL sp_delete_promotions_customer_freegoodsgrid(?,?)',$paramarry,'');
	...
	// called stored procedure for counter
	$rowcount = $this->SFA_Comman->executequery('CALL sp_get_promotions_customer_freegoodsgrid(?,?,?,?,?,?,?)',$param_array,'');
	...
	$data_arr["data"] = $this->SFA_Comman->executequery('CALL sp_get_promotions_customer_freegoodsgrid(?,?,?,?,?,?,?)',$param_array,'');
