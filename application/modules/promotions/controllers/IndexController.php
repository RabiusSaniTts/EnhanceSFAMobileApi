...
class Promotions_IndexController extends Promotions_Library_Controller_Action_Abstract
{
    ...
    */
    public function addqualificationgrpAction()
    {
		...
		if($params['id'] > 0){
			...
			$this->view->divisoncode = $result[3];
    ...
    */
    public function qualificationgrpitemgridAction()
    {
		...
		//print_r($params['key']); exit();
		$param_array[7] = ' AND pgh.grouptype = 1 AND pgh.groupnumber = "'.$params['key'].'" ';
		//$param_array[7] = ($params['key'] <> "" ? ' AND pgh.grouptype = 1  AND pgh.groupnumber ='.$params['key'].' ': ' AND pgh.groupnumber = 0 AND pgh.grouptype = 1 ');
    ...
    */
    public function assignmentitemgridAction()
    {
		...
		if($formdata["add"]=="yes"){
			...
			$param_array[8] = $formdata['txtpromo_case_price'];	//promocaseprice
			//$param_array[9] = trim($formdata["txtquantity"]); //promocaseprice
			$param_array[9] = (trim($formdata['txtquantity']) <> ""?trim($formdata["txtquantity"]):'0');
		...
		$param_array[5] = (int)$get_return_vals['show_records_per_page'];
		$param_array[6] = implode(", ",$columns_array);
		$param_array[7] = ($params['key'] <> "" ? ' AND pgh.grouptype = 2  AND pgh.groupnumber ='.$params['key'].' ': ' and pgh.groupnumber = 0 AND pgh.grouptype = 2 ');
		//$param_array[7] = ' AND pgh.grouptype = 2 AND pgh.groupnumber ='.$params['key'].' ';
		
		//var_dump($params['key']); exit;
    ...
    */
	...
	$promo_amt		= $this->translate->_('Promotion Amount');
		...
		//Jyothish added new function to filter by divison code 2025-10-19
public function getitemsbyfilterajaxAction()
{
    $this->_helper->viewRenderer->setNoRender(true);

    try {
        $filterBy = $this->_request->getPost('filterby');

        $params = $filterBy;

        $result = $this->SFA_Comman->executequery('CALL sp_ajax_getitemsbydivision(?)', $params, '');
