    ...
    function XLSImport($table, $fields, $csv_fieldname='csv')
    {
       ...
       $filename = BASE_PATH . "/data/excel/".$table."-" . date( "m-d-Y" ) . ".xls";
        if(move_uploaded_file($_FILES[$csv_fieldname]['tmp_name'], $filename))
        {
			...
			//echo  $import_status;exit;
            if($import_status)
                if($import_status == "true" || $import_status == 1)
                return $sucsMsg = "Your file has been imported successfully!";
				else
				return $sucsMsg = $import_status;//"ERROR: while importing into table occured data mismatch!!";
            else
				if($status)
				return $errMsg = "ERROR: File is not readable! Its Bad file";
				else
                return $errMsg = "ERROR: while importing the file!";
     
        }else{
		 return $errMsg = "ERROR: while uploading the file!";
		}
        //exit();
    }
}