...
class Organization_RouteController extends Organization_Library_Controller_Action_Abstract
{
    ...
    */
    public function addrouteAction()
    {
		...
		$items = $this->SFA_Comman->executequery('CALL sp_combo_itemmust()','','');
		
		//var_dump($items);
		$this->view->itemmust_key_data 	= $items[0];		
		...
		$route_type = array(array('id'=>0,'val'=>'0 - Enable Order And Sales Process (History & Goal For Order Only)'),
				...
				array('id'=>4,'val'=>'4 - Merchandizing Only'));
		...
		if($formdata['hdnid'] > 0){
			...
			$param_array[38] = $formdata['device_assigned'];
			$param_array[39] = $formdata['ddlitemmustkey'];
			$param_array[40] = $formdata['chkenable_fss']; //enable price edit 
			...
			if($result[0][0]['result'] == 'duplicate'){
				...
				//echo "tesddddddd";
					...
					$param_setting1_array[39]= $formdata['chkenablefreegood'];
					//echo "tssdf0550";
				...
				//	echo "tssdfddddd056";
					...
					if($formdata['chkenableautopostingaccount'] == 1) {
						...
						/* for edit New param added by nilesh on 22Mar2016*/
					
					$param_setting2_array[35] = $formdata['tx_forcesettlementdays'];
					$param_setting2_array[36] = $formdata['chk_routecredit'];
					$param_setting2_array[37] = $formdata['routecreditlimitdays'];
					
					/*for edit New param added by nilesh on 31Mar2016*/
					$param_setting2_array[38]=$formdata['chkupd_gps'];			//updategps
					$param_setting2_array[39]=$formdata['chkenforce_gps'];		//enforcegps
					$param_setting2_array[40]=$formdata['chkenable_gps'];       //enablegps
					$param_setting2_array[41]=$formdata['ddlenabledamagedrtn']; // enabledamagedtrxn
					//print_r($param_setting2_array);exit;
					/**/
					
					$param_setting2_array[42]= $formdata['saleslimitflag']; // saleslimitflag				
					$param_setting2_array[43]= $formdata['minlimit']; // minlimit				
					$param_setting2_array[44]= $formdata['maxlimit']; // maxlimit
					$param_setting2_array[45]= $formdata['tx_orderholddays']; // max order hold days
					$param_setting2_array[46]= $formdata['custchkupd_gps']; // enforecegeocheckout
		...
		elseif(!empty($formdata['txtroute_name']) && !empty($formdata['txtaltcode'])  && !empty($formdata['txtcode']))
		{
			...
			$param_array[39] = $formdata['txtcode'];		//routecode
			$param_array[40] = $formdata['device_assigned'];		//routecode
			$param_array[41] = $formdata['ddlitemmustkey'];
			$param_array[42] = $formdata['chkenable_fss']; //enable price edit
			//die;
		...
		//	$result = $this->SFA_Comman->executequery('CALL sp_add_organization_route_addroute(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',$param_array,'');
			if($result[0][0]['result'] == 'duplicate'){
				SFA_Message::setErrorMsg($this->translate->_('Record(s) already Exist'));
			}else if($result[0][0]['result'] == '')
				{
				SFA_Message::setErrorMsg($this->translate->_('Record(s) already Exist'));
				}
			else{
					echo "sss".$routecode = $result[0][0]['result'];
					...
					/*New param added by nilesh on 22Mar2016*/
					//var_dump($param_setting2_array);
					$param_setting2_array[35] = $formdata['tx_forcesettlementdays'];
					$param_setting2_array[36] = $formdata['chk_routecredit'];
					$param_setting2_array[37] = $formdata['routecreditlimitdays'];
					...
					$param_setting2_array[44]= $formdata['maxlimit']; // maxlimit
					$param_setting2_array[45]= $formdata['tx_orderholddays']; // max order hold days
					$param_setting2_array[46]= $formdata['custchkupd_gps']; //enforcegeocheckout
		...
		elseif($params['id'] > 0)
		{
			...
			$viewformdata['active_status']	= $res['activestatus'];	// activestatus
			...
			$viewformdata['device_assigned_id'] = $res['device_assigned_id'];
			//var_dump($res);
			$viewformdata['ddlitemmustkey'] = $res['itemmustkey'];
			$viewformdata['enablefss'] = $res['enablefss'];
			...
			/**/
			$new_param = array();
			//var_dump($viewformdata['active_status']);
			if($params['tmpl'] == 1){
			
			$new_param[1] = $res['device_assigned_id'];			
			$edit_device_id = $this->SFA_Comman->executequery('CALL sp_get_not_assigned_devices(?)',$new_param,'');	
			$this->view->deviceinfo = $edit_device_id[0];
			$this->view->itemmust_key_data = $result[12];
			$this->view->status				    = $result[13];
			}
			else
			{
			...
			$edit_device_id = $this->SFA_Comman->executequery('CALL sp_get_not_assigned_devices(?)',$new_param,'');
			$this->view->deviceinfo = $edit_device_id[0];
			$this->view->itemmust_key_data = $result[11];
		...
		else
		{
			...
			$add_param = array();
			$add_param[1] = "";
			$edit_device_id = $this->SFA_Comman->executequery('CALL sp_get_not_assigned_devices(?)',$add_param,'');
			$this->view->deviceinfo = $edit_device_id[0];
			$this->view->itemmust_key_data = $result[12];
			$this->view->status				    = $result[13];
			/**/
    ...
    */
    public function setting2values(){
		...
		$ddlenabledamagedrtn = array();
		$ddlenabledamagedrtn = array(array('id'=>0,'val'=>'Disable'),				
				array('id'=>1,'val'=>'Enable'));
		$this->view->ddlenabledamagedrtn	= $ddlenabledamagedrtn;
		...
		$load_req_roll_up = array(array('id'=>0,'val'=>'Add Load Request'),
								  array('id'=>1,'val'=>'Prefill with average sales'),
								  array('id'=>2,'val'=>'Prefill with order quantities'),
								  array('id'=>3,'val'=>'Request only MSL Items'));
    ...
    */
    public function addroutetmplAction()
    {
		...
		if(count($formdata) > 0) {
			...
			$param_array[27]= $formdata['txttemplatename'];
			$param_array[28]= $formdata['ddlitemmustkey'];				//presalesorder
			
			
			if($formdata['hdnid'] > 0){
				$param_array[29]	= $formdata['hdnid'];
			...
			if($lastid > 0)
			{
				...
				if($formdata['chkenableautopostingaccount'] == 1) {
					...
					$param_setting2_array[44]= $formdata['maxlimit']; // maxlimit
					$param_setting2_array[45]= $formdata['tx_orderholddays']; // max order hold days
					$param_setting2_array[46]= $formdata['custchkupd_gps'];
		...
		elseif($params['id'] > 0)
		{
			...
			$this->view->itemmust_key_data 	= $result[5];
			$this->view->status				    = $result[6];
			...
			$viewformdata['ddlitemmustkey'] =$result[0][0]['itemmustkey'];
