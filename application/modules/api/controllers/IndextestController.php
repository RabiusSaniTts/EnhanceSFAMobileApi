...
class Api_IndextestController extends Api_Library_Controller_Action_Abstract
{
    ...
    */
	...
	public function salesmanverchkAction(){
         //Request Parameter
        $this->view->params = $params = $this->getRequest()->getParams();
	...
	//Route id
		...
		//for Version
        $param_array[2] = $params['verno'];

	$result = $this->SFA_Comman->executequery('CALL sp_ws_version_check(?,?)',$param_array,'');
	$resultdata = (count($result[0]) > 0) ? $result[0]:array();
         //json output
    ...
    */
	public function getsyncdataAction(){
		...
		if($x==1){ // setting
			...
			$result['saletarget']               = (count($resultdata[8]) > 0) ? $resultdata[8]:array();
			...
			$result['nrprule'] 	= (count($resultdata[4]) > 0) ? $resultdata[4]:array();																	   
			...
			//echo $params['routeid'];exit;
			
			// if($params['routeid']=='1316')
			// { echo "dsds";
			// $resultdata= $this->SFA_Comman->executequery('CALL sp_ws_syncicsdata_customers1(?,?,?,?)',$param_array,'');
			// echo "--- res:":;rint_r($resultdataw);exit;
			// }
			// else
			// {
			...
			// {
				...
				// $resultdata = $this->SFA_Comman->executequery('CALL sp_ws_syncicsdata_customers1(?,?,?,?)',$param_array,'');
				// // echo "--- res:";print_r($resultdata);exit;
			// }
			// else
			// {
				// $resultdata = $this->SFA_Comman->executequery('CALL sp_ws_syncicsdata_customers(?,?,?,?)',$param_array,'');
			...
			//$result['customeritemgrpcodes'] 	=         (count($resultdata[9]) > 0) ? $resultdata[9]:array();
			//$result['customeritemmapcode'] 	= (count($resultdata[10]) > 0) ? $resultdata[10]:array();
			...
			$result['customeritemgrpcode'] 	= (count($resultdata[0]) > 0) ? $resultdata[0]:array();
			$result['customeritemmapcode'] 	= (count($resultdata[1]) > 0) ? $resultdata[1]:array();*/
			...
			$result['itemnrp'] 	= (count($resultdata[2]) > 0) ? $resultdata[2]:array();	
			$result['custnrp'] 	= (count($resultdata[3]) > 0) ? $resultdata[3]:array();																		   
			/*$result['customeritemgrpcode'] 	=         (count($resultdata[2]) > 0) ? $resultdata[2]:array();*/
		}
	...
	public function getsyncdata1Action()
	{
		$this->view->params = $params = $this->getRequest()->getParams();
		$param_array    = array();	
        $param_array[1] = $params['userid'];
