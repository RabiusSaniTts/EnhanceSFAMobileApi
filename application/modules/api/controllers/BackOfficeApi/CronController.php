...
//        foreach($result[0][0] as $tourID)
	...
	public function sendalertAction()
	{
		...
		/*
		$this->SFA_Comman->executequery('CALL sp_update_customer_GPS("")','','');		
		$subject  = 'RoutePro GPS Updated.';
		$message  = '';
		$message  = '<html><body>';
		$message .= '<h4>RoutePro Customers GPS Locations Updated.</h4>';					
		$message .= "</body></html>";
		$headers  = 'From: support@mirnah.com' . "\r\n" .
			'MIME-Version: 1.0' . "\r\n" .
			'Content-type: text/html; charset=utf-8';
		
		$to       = 'hinshinm@enhanceuae.com,a.nair@mirnah.com';
		...
		*/
		...
		if($result[0]>0)
		{
					$subject  = 'RoutePro Password Changed.';
					$message  = '';
					$message  = '<html><body>';
					$message .= '<h4>Please Find Below Password Changes.</h4>';
					$message .= '<table rules="all" style="border-color: #666;" cellpadding="10">';
					$message .= "<tr style='background: #eee;'>
						<td><strong>Password Type</strong> </td>
						<td><strong>New Password</strong></td>
						<td><strong>Old Password</strong></td></tr>";
					 foreach($result[0] as $row){
					$message .= "<tr>
						<td>".strip_tags($row['passtype'])." </td>
						<td>".strip_tags($row['newpassword'])."</td>
						<td>".strip_tags($row['oldpassword'])."</td></tr>";
					 }
					$message .= "</table>";
					$message .= "</body></html>";
					$headers  = 'From: support@mirnah.com' . "\r\n" .
					...
					$to       = 'enhvancashier@enhanceuae.com';
					$success = mail($to, $subject, $message, $headers);		
