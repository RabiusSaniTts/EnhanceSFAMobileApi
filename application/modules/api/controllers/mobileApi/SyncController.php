...
//
   //Function for decode signaturedata added by vinay
     public function senddata1Action()
  { //echo "----test---";exit;
       // print_r($this->_getParam('salesorderheader'));exit;
		...
		$filename = 'C:/wamp/www/sfa/enhance_live/log/salesorder_log_'.date('Ymd').'.txt';
		$trtype ="";
  ...
  { //echo "----test---";exit;
	 ...
	 /*
		...
		$filename = 'C:/wamp/www/sfa/enhance_live/log/sendata_log_'.date('Ymd').'.txt';
		$trtype ="";
    ...
    $resultreturn = array();    
		...
		$path   = str_replace('//','/',$_SERVER['DOCUMENT_ROOT'].'/sfa/enhancetest/');		                         
	...
	$withstrip=(stripslashes($pararminvoice));
		...
		/*
		$filename = 'C:/wamp/www/sfa/enhance_live/log/custopera_log_'.date('Ymd').'.txt';
		$trtype ="";
	...
	if(count($params)>0)
	{
	    for($i=0;$i<count($params);$i++)
	    {
		    ...
		    $param_array[19] = $params[$i]['log_id'];
	...
	//For customervisitlog
    if(!is_null($this->_getParam('customervisitlog')))
    {
		$pararminvoice=$this->_getParam('customervisitlog');    
		$withstrip=(stripslashes($pararminvoice));
        $params = json_decode($withstrip,true);   
		$ar=array();
		$arrcust=array();
		...
		if(count($params)>0)
		{
			...
			{
				$param_array     = array();         
				$param_array[1] = $params[$i]['log_id'];
				$param_array[2] = $params[$i]['routekey'];
				$param_array[3] = $params[$i]['routecode'];
				$param_array[4] = $params[$i]['salesmancode'];
				$param_array[5] = $params[$i]['customercode'];
				$param_array[6] = $params[$i]['log_startdate'];
				$param_array[7] = $params[$i]['log_starttime'];
				$param_array[8] = $params[$i]['log_enddate'];
				$param_array[9] = $params[$i]['log_endtime'];
				$resultdata = $this->SFA_Comman->executequery('CALL sp_ws_getdata_from_customervisitlog(?,?,?,?,?,?,?,?,?)',$param_array,'');     

				if($resultdata[0][0]['lastid']>0)
				{
					// $arr19[]=array("itemcode"=>$param_array[1]);
					$arrcust[]=array("routekey"=>$param_array[2],"log_id"=>$param_array[1],"customercode"=>$param_array[5]);
					//$arr22[]=array();
				}
			}
			$resultreturn['customervisitlog'] = $arrcust;
		}
		else
		{
			$resultreturn['customervisitlog'] = array();
		}

	}
	//end of Customer visit log
	...
	if(count($params)>0)
	{
		...
		$param_array[16] = $params[$i]['orderdeliverydate'];
		...
		$param_array[56] = $params[$i]['lineitemdiscount'];
		$param_array[57] = $params[$i]['totalreturnpromoamount'];
		$param_array[58] = $params[$i]['returnlineitemdiscount'];
		...
		$param_array[62] = $params[$i]['headerdiscounttaxamt'];
	...
	if(count($params)>0)
	{
		...
		$param_array[50] = $params[$i]['sales_amount'];
		$param_array[51] = $params[$i]['returnpromovalue'];
		$param_array[52] = $params[$i]['returnpromoamount'];
		$param_array[53] = $params[$i]['return_amount'];
		$param_array[54] = $params[$i]['returnfreesampleqty'];
   ...
   // if($value1[0] == 'invoiceheader')
	...
	if(count($params)>0)
	{
	    for($i=0;$i<count($params);$i++)
	    {
		   ...
		   $param_array[1] = $params[$i]['transactionkey'];
		    ...
		    $param_array[42] = $params[$i]['totalpromoamount'];
			$param_array[43] = $params[$i]['totallineitemtax'];
	...
	$withstrip=(stripslashes($pararminvoice));
		...
		/*
		$filename = 'C:/wamp/www/sfa/enhance_live/log/salesorder_detail_log_'.date('Ymd').'.txt';
		$trtype ="";
	...
	if(count($params)>0)
	{
	    for($i=0;$i<count($params);$i++)
	    {
		  ...
		  $param_array[8] = $params[$i]['freesampleqty'];
			...
			$param_array[29] = $params[$i]['promoamount']; // sujee added 01/02/2019
			//$param_array[28] = "0";
			$param_array[30] = $params[$i]['salesordervat'];
			$param_array[31] = $params[$i]['salesorderexcisetax'];
			$param_array[32] = $params[$i]['returnvat'];
			$param_array[33] = $params[$i]['returnexcisetax'];
			$param_array[34] = $params[$i]['damagedvat'];
			$param_array[35] = $params[$i]['damagedexcisetax'];
			$param_array[36] = $params[$i]['promovat'];
			$param_array[37] = $params[$i]['promoexcisetax'];
			$param_array[38] = $params[$i]['fgvat'];
			$param_array[39] = $params[$i]['fgexcisetax'];
	  ...
	  $resultreturn['salesorderdetail'] = $arr11;
	   //print_r($resultreturn['salesorderdetail'] );exit;
   ...
   //For batchdetail
	...
	if(count($params)>0)
	{
	    for($i=0;$i<count($params);$i++)
	    {
			...
			$param_array[28] = $params[$i]['deductionflag'];
	...
	if(count($params)>0)
	{
	    for($i=0;$i<count($params);$i++)
	    {
			...
			$param_array[17] = $params[$i]['deductionamount'];
  ...
  //  if($value11[0] == 'inventorytransactionheader')
	...
	$arr20=array();
	//if(count($params)>0)
		// commented not to upload sig as delay in seerver 19/02/2020 need to check 
	if(1==0)
	{
	...
	if(count($params)>0)
	{
	    for($i=0;$i<count($params);$i++)
	    {
	        ...
	        $param_array[18] = $params[$i]['expirydate'];			    	    
    ...
    //Sync log
		...
		//exit;
			...
			//$result_route_close = $this->SFA_Comman->executequery('CALL sp_check_isroute_closed(?)',1,'');
	...
	exit;
	  ...
	  /* $routeKeyArr= array($this->_getParam('routekey'));
	    ...
	    $this->OracleDBSync->onlineexportdata($routeKeyArr);
...
//Newly added for Sync issue for Detail lines for Invoce and AR on 7-May-2025, Arun, Aswin Worked

public function invtxndetailAction()
	{
		$reqval= $this->getRequest()->getParams();
		$pararminvoice=$reqval['invoicedetail'];	
		$withstrip=(stripslashes($pararminvoice));
		...
		$filename = $path.'log/upload_inv_'.date('YmdH').'.txt';
		if (!file_exists($filename)) {
			fopen($filename,'a');
		}
		chmod($filename,0777);		
		...
		for($i=0;$i<count($params);$i++)
	    {
			...
			$resultdata = $this->SFA_Comman->executequery('CALL sp_ws_upload_inv_detl(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',$param_array,'');
			if($resultdata[0][0]['lastid']>0)
			{
				$arr[]=array("routekey"=>$param_array[1],"visitkey"=>$param_array[2],"itemcode"=>$param_array[4]);
			}			
	...
	public function artxndetailAction()
	{
		$reqval= $this->getRequest()->getParams();
		$pararminvoice=$reqval['ardetail'];	
		$withstrip=(stripslashes($pararminvoice));
		...
		$arr=array();
		$filename = $path.'log/upload_ar_'.date('YmdH').'.txt';
		if (!file_exists($filename)) {
			fopen($filename,'a');
		}
		chmod($filename,0777);		
		...
		for($i=0;$i<count($params);$i++)
	    {
			...
			$param_array[1] = $params[$i]['routekey'];
		    $param_array[2] = $params[$i]['visitkey'];
		    $param_array[3] = $params[$i]['transactionkey'];//$arr_artrn[number_format($params[$i]['transactionkey'],0)][0];
		    $param_array[4] = $params[$i]['invoicenumber'];
		    $param_array[5] = $params[$i]['invoicedate'];
		    $param_array[6] = $params[$i]['totalinvoiceamount'];
		    $param_array[7] = $params[$i]['onacctreasoncode'];
			...
			$resultdata = $this->SFA_Comman->executequery('CALL sp_ws_upload_ar_detl(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',$param_array,'');
      
			if($resultdata[0][0]['lastid']>0)//if(1==1)
			{
				$arr[]=array("routekey"=>$param_array[1],"visitkey"=>$param_array[2],"transactionkey"=>$param_array[3]);
			}						
		}
		
		$resultreturn['ardetail'] = $arr;
		echo json_encode($resultreturn);
	}
...
//End of Newly added
	...
	public function custseqAction()
	{
		...
		$filename = $path.'log/visit_'.date('YmdH').'.txt';
		if (!file_exists($filename)) {
			fopen($filename,'a');
		}
		chmod($filename,0777);		
		...
		{
			$param_array 	= array();		   
		    $param_array[1] = $j['routekey'];
		    $param_array[2] = $j['seqweeknumber'];
		    $param_array[3] = $j['seqweekday'];
		    $param_array[4] = $j['routecode'];
		    $param_array[5] = $j['customercode'];
		    $param_array[6] = $j['sequencenumber'];
		    $param_array[7] = $j['schelduledflag'];
		    $param_array[8] = $j['servicedflag'];
		    $param_array[9] = $j['scannedflag'];
			//file_put_contents($filename, $param_array);
			//print_r($param_array);	    
		$resultdata = $this->SFA_Comman->executequery('CALL sp_ws_get_from_routesequencecustomerstatus(?,?,?,?,?,?,?,?,?)',$param_array,'');
