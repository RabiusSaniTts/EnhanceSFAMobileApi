...
class Account_CustomerController extends Account_Library_Controller_Action_Abstract
{
    ...
    */
    public function customerAction()
    {
		...
		$additional_where_condition[] = " um.userid = ".$this->currentUser->userid;
		// prepare the configuration for grid
		$pagingparams = array(
				"show_grid_heading" => true,
				"grid_heading_message" => $this->translate->_('Overview'),
    ...
    */
    public function addcustomerAction()
    {
...

		{
			...
			$param_array[15] = str_replace(',','',$formdata['txtcr_limit']);	
//print_r($param_array[15]);
//exit;			//creditlimit
			//$param_array[15] = $formdata['txtcr_limit'];			//creditlimit
			...
			/*Following param commented by nilesh on 14March2016*/
                ...
                $param_array[48] = $formdata['hdnchangeroutestatus'];		//it's for update route id in route sequence.
                /*instead of 46 param, 47 param added by nilesh on 14Mar2016*/

				$param_array[49] = $formdata[itemmapkey];
				$param_array[50] = ($formdata['ddlcust_mslcat'] > 0) ? $formdata['ddlcust_mslcat'] : 'NULL';			//MSL Category
			...
			{
			$param_array[47] = ($formdata['ddlcust_mslcat'] > 0) ? $formdata['ddlcust_mslcat'] : 'NULL';			//MSL Category
		...
		elseif($params['id'] > 0)
		{
			...
			if($result[4][0]['activecustomer'] ==1) 
			{
				$res['ddlstatus'] 	= 'Active';
			}
			else
			{
				$res['ddlstatus'] 	= 'Inactive';
			}
		...
		//	exit;
			...
			$res['chkrentcontrol']  = $result[4][0]['rentcontrol'];
			...
			$res['anpflag']		= $result[4][0]['anpflag'];
			
		//for sales amount limit
			$res['saleslimitflag']		= $result[4][0]['saleslimitflag'];
			$res['saleslimitmin']		= $result[4][0]['minsaleslimit'];
			$res['saleslimitmax']		= $result[4][0]['maxsaleslimit'];
		//end of sales limit
		//Rebate Message
			$res['txtrbtmsg']		= $result[4][0]['rebateprintyn'];
		//End of Rebate Message
 ...
 $res['itemmapkey'] 	= $result[4][0]['itemmapkey'];
$res['ddlcust_mslcat'] 	= $result[4][0]['mslcategory'];
$res['chkenforce_msl'] 	= $result[4][0]['enforcemsl'];

	
			//$res['txtlangname'] 	= $result[4][0][''];
			...
			$this->view->itemmapkey	    	= $result[21];
			$this->view->customer_mslcategory	    = $result[22];//print_r($result[18]);exit;
		...
		else
		{
			...
			$this->view->advertising_data	    = $result[19];
			$this->view->loyalitydata	    	= $result[20];
			$this->view->customer_mslcategory	= $result[18];    
			//print_r($result[18]);exit;
		}
    }
    /**
    ...
    */   
	...
	$param_setting1_array[1]  = $formdata['ddldis_msg_key1'];		//messagekey1
	$param_setting1_array[2]  = $formdata['ddldis_msg_key2'];		//messagekey2
	$param_setting1_array[3]  = $formdata['ddlena_sugg_sales'];		//enablesuggestsales
	$param_setting1_array[4]  = $formdata['ddlena_auto_fill_ret'];		//enableautofillreturns
	$param_setting1_array[5]  = $formdata['ddlena_auto_fill_dmg'];		//enableautofilldamaged
	$param_setting1_array[6]  = $formdata['ddlena_auto_fill_cap'];		//enablesigcapture
	$param_setting1_array[7]  = $formdata['ddlenable_ret_trxn'];		//enablereturnstrxn
	$param_setting1_array[8]  = $formdata['ddlena_ar_coll'];		//enablearcollection
	$param_setting1_array[9]  = $formdata['ddlena_promo_trxn'];		//enablepromotrxn
	$param_setting1_array[10] = $formdata['ddlena_sales_trxn'];		//enablesalestrxn
	$param_setting1_array[11] = $formdata['ddlena_invo_copy'];		//enableinvoicecopy
	$param_setting1_array[12] = $formdata['ddlinvo_price_prnt'];		//invoicepriceprint
	$param_setting1_array[13] = $formdata['ddlprnt_seq'];			//printsequence
	$param_setting1_array[14] = $formdata['ddlena_edit_price_invs'];	//enablepriceeditinvs
	$param_setting1_array[15] = $formdata['ddlena_sell_prev'];		//enablesellprevious
	$param_setting1_array[16] = $formdata['ddlena_survey_audit'];		//enablesurveyaudit
	$param_setting1_array[17] = $formdata['ddlena_del_instru'];		//enabledelivinstruct
	$param_setting1_array[18] = $formdata['ddlena_invo_comment'];		//enableinvoicecomment
	$param_setting1_array[19] = $formdata['ddlinvo_dtl_entry'];		//invoicedetailentry
	$param_setting1_array[20] = $formdata['ddlord_dtl_entry'];		//orderdetailentry
	$param_setting1_array[21] = $formdata['ddlforce_stock_capture'];	//forcestockcapture
	$param_setting1_array[22] = $formdata['ddlauto_set_coll'];		//autosettlecollection
	$param_setting1_array[23] = $formdata['ddlorder_format'];		//orderformat
	$param_setting1_array[24] = $formdata['ddlena_delay_prnt'];		//enabledelayprint
	$param_setting1_array[25] = $formdata['ddlena_damage_ret'];		//enabledamagedreturns
	$param_setting1_array[26] = $formdata['ddlautho_itm_list'];		//authorizeditemlistctl
	$param_setting1_array[27] = $formdata['hdnid'];				//customercode
	$param_setting1_array[28] = $formdata['ddlprnt_msg_key1'];		//messagekey3
	$param_setting1_array[29] = $formdata['ddlprnt_msg_key2'];		//messagekey4
	$param_setting1_array[30] = $formdata['ddlinvo_header_msg_key'];	//messagekey5
	$param_setting1_array[31] = $formdata['ddlinvo_trailor_key'];		//messagekey6
	$param_setting1_array[32] = $formdata['ddlena_itm_barcode'];		//enableupcprint
	$param_setting1_array[33] = $formdata['chkenablepos'];			//enableposequipment
	$param_setting1_array[34] = $formdata['chkenableadvpay'];		//enableadvancepayment
	$param_setting1_array[35] = $formdata['chkenablebuybackfree'];		//enablebuybackfree
	$param_setting1_array[36] = $formdata['chkenablerental'];		//enablerental	
	$param_setting1_array[37] = $formdata['chkenableautofillsales'];	//EnableAutoFillSales 
	$param_setting1_array[38] = $formdata['chkenableadvbatchsel'];		//enablebatchselection
	...
	$param_setting1_array[42] = $formdata['chkenableexchangetrxn'];
	$param_setting1_array[43] = $formdata['chkenablepassrtntranxn'];
	$param_setting1_array[44] = $formdata['chkrentcontrol'];
    ...
    */   
	...
	$param_setting2_array[3]   = $formdata['ddlcust_tax_key1']; 	//custtaxkey1
	$param_setting2_array[4]   = $formdata['ddlcust_tax_key2']; 	//custtaxkey2
	$param_setting2_array[5]   = $formdata['ddlcust_tax_key3']; 	//custtaxkey3
	$param_setting2_array[6]   = $formdata['txtlost_placement']; 	//lostplacementdelivs
	$param_setting2_array[7]   = $formdata['txtnew_plac_deli']; 	//newplacementdelivs
	$param_setting2_array[8]   = $formdata['txtinvo_copies']; 	//invoicecopies
	$param_setting2_array[9]   = $formdata['txttax_id']; 		//customertaxid
	$param_setting2_array[10]  = $formdata['ddlena_promo_edit_ord']; //enablepromoeditords
	$param_setting2_array[11]  = $formdata['ddlena_promo_edit_invs']; //enablepromoeditinvs
	$param_setting2_array[12]  = $formdata['txtzip']; 		//customerzip
	$param_setting2_array[13]  = $formdata['ddladd_addl_invo']; 	//enableaddlpromoords
	$param_setting2_array[14]  = $formdata['ddlena_promo_invoice'];	//enableaddlpromoinvoices
	$param_setting2_array[15]  = $formdata['chkroundnetamt']; 	//roundnetamount
	$param_setting2_array[16]  = $formdata['chkenforce_promo']; 	//enforcepromotion
	$param_setting2_array[17]  = $formdata['chkdraftcpy']; 		//enabledraftcopy
	$param_setting2_array[18]  = $formdata['txtforward_factor']; 	//forwardcoverfactor	
	...
	$param_setting2_array[23]  = $formdata['chkallow_cash']; 	//allowcashoncreditexceed
	$param_setting2_array[24]  = $formdata['chkprint_outlet']; 	//printoutletitemcode
	$param_setting2_array[25]  = $formdata['hdnid'];		//customercode		
	$param_setting2_array[26]  = $formdata['ddlsales'];		//invoiceformatoption
	$param_setting2_array[27]  = $formdata['ddltax_option'];	//customertaxidoptions  
	$param_setting2_array[28]  = $formdata['txtfixlati'];		//fixedlatitude
	$param_setting2_array[29]  = $formdata['txtfixlong'];		//fixedlongitude	
	$param_setting2_array[30]  = $formdata['chkgraceperiod'];	//graceperiod
    $param_setting2_array[31]  = $formdata['ddlroundingtype'];//$formdata['txtroundingoff'];	//roundingoffvalue 
	...
	$param_setting2_array[34]   = $formdata['ddlcust_visual'];
	...
	$param_setting2_array[38]  = $formdata['chkenforce_msl']; 
	...
	$param_setting2_array[42]  = $formdata['ddltax_option'];	//customertaxidoptions
	$param_setting2_array[43]  = $formdata['ddlapplytax_option'];
	$param_setting2_array[44]  = $formdata['anpflag'];
	$param_setting2_array[45]  = $formdata['saleslimitflag'];
	$param_setting2_array[46]  = $formdata['minlimit'];
	$param_setting2_array[47]  = $formdata['maxlimit'];
	$param_setting2_array[48]  = $formdata['txtrbtmsg'];
    ...
    */
    ...
    public function setting2values()
    {
		...
		$tax_data[2]['id']    	= '2';
		$tax_data[3]['id']    	= '3';	
		$tax_data[0]['val']    	= 'Do not Prompt or Print (Default)';
		$tax_data[1]['val']    	= 'Prompt, do not Print';
		$tax_data[2]['val']    	= 'Print, do not Prompt';
		$tax_data[3]['val']    	= 'Prompt and Print';
    ...
    */
    public function authorizegroupitemgridAction()
    {
		...
		/**
    * @name       getitemlistforitemmustlist
    * @since      22-02-2012
    * @version    Release: 1
    * @author     sree <sree@tts.com>
    * @copyright  tts Technologies
    * @param
    *
    * This action is use for getitemlist from item group code
    */
    public function getitemlistforitemmustlistAction()
    {
		$params = $this->getRequest()->getParams();
		$param_array = array();
		$param_array[1] = ($params['agency'] <> "" && $params['agency'] <> "null" )? $params['agency']:0;
		$param_array[2] = ($params['brand'] <> "" && $params['brand'] <> "null" )? $params['brand']:0;
		$param_array[3] = ($params['category'] <> "" && $params['category'] <> "null" )? $params['category']:0;
		$param_array[4] = ($params['subcategory'] <> ""  && $params['subcategory'] <> "null" )? $params['subcategory']:0;
		
		$result = $this->SFA_Comman->executequery('CALL sp_combo_itemmaster_itemmustlist(?)',$param_array,'');	
...
gn     * @since      22-02-2012
    ...
    */
    public function itemmustcustomerAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
//print_r($params);
// print_r($formdata);      
        ...
        $this->view->title = $this->translate->_('Customer Item Must List');
    
        $columns_array     = array('DivisionCode','customercategory','alternatecode','customername','itemmustkey','itemmustdescription','enforcemsl',' customercode AS edit_del_primary_id' );
        $columns_show   = array($this->translate->_('Division'),$this->translate->_('Customer Classification'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('MSL Code'),$this->translate->_('MSL Description'),$this->translate->_('Enforce MSL'));
        if($formdata["grid_search_text"] != "")
		 $srch='and (DivisionCode LIKE "'.$formdata["grid_search_text"].'%" or alternatecode LIKE "'.$formdata["grid_search_text"].'%" or customername LIKE "'.$formdata["grid_search_text"].'%" or itemmustkey LIKE "'.$formdata["grid_search_text"].'%" or itemmustdescription LIKE "'.$formdata["grid_search_text"].'%" or enforcemsl LIKE "'.$formdata["grid_search_text"].'%")';
	...
	$pagingparams = array(
			...
			"show_deleteall" => false,
			"editmode"=>true,
			"noeditfields" => array("selectbox",'DivisionCode','customercategory','alternatecode','customername','itemmustkey','itemmustdescription'),
			"primaryid" => "primary_key ",
			"currentlink" => array("/hhctransaction/invoice/salesorderdetail".$ex_param),
			"editlink" => array("/sid/#pattern#/edit/yes","#pattern#"),
			"deletelink" => array("/sid/#pattern#/delete/yes","#pattern#"),
			"nodata_message" => $this->translate->_('No Record(s) Found'),
			"fetch_columns_inquery" => $columns_array,
			"show_columns" => $columns_show,
	...
	// called stored procedure for counter
	$result = $this->SFA_Comman->executequery('CALL sp_get_account_customer_itemmust_enforcemsl(?,?,?,?,?,?,?)',$param_array,'');
	...
	public function saveitemmustcustomerAction()
    {
        $this->view->formdata = $formdata = $this->_request->getPost();
	    $this->view->params = $params = $this->getRequest()->getParams();
		$param_array = array();
			$param_array[2]	= substr($params['mslids'],0,strlen($params['mslids'])-1);
			$param_array[1]	= $params['enforce'];
		$this->SFA_Comman->executequery('CALL sp_update_account_customer_customeritemmust(?,?)',$param_array,'');
		echo "Data Updated Successfully";
		exit;
	}
	/*public function additemmustcustomerAction()
    {
        $this->view->formdata = $formdata = $this->_request->getPost();
	    $this->view->params = $params = $this->getRequest()->getParams();
	//	print_r($params);
		...
		if(count($formdata) > 0) {
			...
			if($formdata['enforce_msl'])
			$param_array[7]	= 1;
		else
			$param_array[7]	= 0;
			// print_r($param_array);exit;
			$this->SFA_Comman->executequery('CALL sp_add_link_customer_customeritemmustlink(?,?,?,?,?,?,?)',$param_array,'');
			
			SFA_Message::setMsg($this->translate->_('Update Record'));	 
			$this->_helper->redirector('itemmustcustomer', 'customer', 'account');
		}
		else
		{ 
			$result = $this->SFA_Comman->executequery('CALL sp_get_links_customer_itemmustmsl()','','');
	 ...
	 public function getcustomerfromitemmustenforceAction()
    {
	...
	$result = $this->SFA_Comman->executequery('CALL sp_get_customer_from_customeritemmustlist(?,?)',$param_array,'');
	echo Zend_Json::encode($result);
	exit;
    }
	/**
    ...
    */
    public function additemmustAction()
    {
        ...
        if($params['id'] > 0 || $formdata['hdnid']){
            ...
            $res['txtitemmust_id'] 	= $result[4][0]['itemmustcode'];
            $res['txt_desc'] 		= $result[4][0]['itemmustdescription'];
            $res['txtdesc_arb'] 	= $result[4][0]['arbitemmustdescription'];
			$res['txt_qty'] 	= 0;//$result[1][0]['quantity'];
    
            $this->view->formdata = $res;
			$this->view->agency 	= $result[0];  
			$this->view->brand 		= $result[1];  
			$this->view->mcategory  = $result[2];  
			$this->view->item_group = $result[3];    
	    } else {		
			...
			$this->view->mcategory  = $result[2];  
			$this->view->item_group = $result[3]; 
			$this->view->formdata['txtitemmust_id']= ($result[4][0]['AUTO_INCREMENT'] == '') ? '1' : $result[4][0]['AUTO_INCREMENT'];
	    }
    }
	/**
    ...
    */
    public function itemmustitemgridAction()
    {
        ...
        $columns_array = array('im.actualitemcode','im.itemshortdescription AS itemshortdescription','itd.quantity','itd.max_quantity','CONCAT(primary_key,"_",ith.itemmustcode) AS edit_del_primary_id');   //'CONCAT(primary_key,"_",ith.itemmustcode) AS edit_del_primary_id'     
         $columns_show =  array($this->translate->_('Item Code'),$this->translate->_('Item Description'),$this->translate->_('Min Quantity'),$this->translate->_('Max Quantity'));
        if($this->css == 'ar_') {
			$columns_array[1]	= 'im.arbitemshortdescription AS itemshortdescription';
		}
		...
		/*if($params["deleteall"]=="yes") {
            ...
            $ids = implode(',',$formdata['chk']);
			$idss = str_replace("_".$params['hdnitemmustno'],'',$ids);
			$paramarry = array();
			$paramarry[1] = $idss;//$params['hdnitemmustno'].'_$';
		...
		if (
				isset($formdata['deleteall']) &&
				$formdata['deleteall'] === 'yes'
			) {

				$ids = trim($formdata['hdndeleteall'], ',');

				$paramarry = array();
				$paramarry[1] = $ids;

				$r_delete = $this->SFA_Comman->executequery(
					'CALL sp_delete_account_customer_itemmustgrid(?)',
					$paramarry,
					''
				);

				$params['key'] = $formdata['hdnitemmustno'];

				SFA_Message::setMsg(
					$this->translate->_('Delete Record')
				);
			}
		// UPDATE THE RECORD
			...
			{		
				...
				$updateData["4"] =$params['max_quantity'];					
		...
		if($formdata["add"]=="yes") {
			...
			if(!empty($formdata['txtitemmust_id']) && !empty($formdata['txt_desc']) && !empty($formdata['ddlitem_code'])){
                ...
                $param_array[7] = trim($formdata['txt_qty']);
				$param_array[8] = trim($formdata['mxtxt_qty']);// print_r($param_array);exit;
		...
		$pagingparams = array(
				...
				//	 "mastervalues" => $mastervalues,
					 ...
					 "editmodevalue"=>"primary_key",
					 "noeditfields" => array('selectbox','alternatecode','actualitemcode','itemshortdescription','edit_del_primary_id'), 
					 "fetch_columns_inquery" => $columns_array,
					 "show_columns" => $columns_show,			     
					 "nodata_message" => $this->translate->_('No Record(s) Found')
					 );
		...
		{
			$keyparam_temp=explode("_",$params['id']);
	$keyparam=$keyparam_temp[1];
	 $pr_key=$keyparam_temp[0];
		}
	else{
	$keyparam=$params['key'];
	 $pr_key=0;
	    ...
	    $srch="";
		 // print_r($_REQUEST['searchval']);exit;
		$searchedon=$_REQUEST["searchval"];    //echo $searchedon ;exit;
		  //$searchedon="117";
		if($searchedon!="")
		 $srch='and (im.alternatecode LIKE "'.$searchedon.'%" or im.itemdescription LIKE "'.$searchedon.'%" or im.itemshortdescription LIKE "'.$searchedon.'%")';
		//$srch=" and 1=1"; 
 
 if($params["edit"]=="yes"){
$condition='AND primary_key="'.$pr_key.'" AND itd.itemmustcode ="'.$keyparam.'"';
}
else{
	$condition='AND itd.itemmustcode ="'.$keyparam.'"';
}
		$param_array = array();
		$param_array[1] = '';
		$param_array[2] = $get_return_vals['order_columns_name'];
    ...
    */
	...
	$this->render("ajaxgrid");
	 ...
	 public function mslcategoryAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        if($formdata["hdDelete"]==1)
        {
            ...
            $result 	= $this->SFA_Comman->executequery('CALL sp_delete_account_customer_mslcat(?,?)',$param_array,'');
        ...
        $this->view->title	= $this->translate->_('MSL Category');	
        ...
        $pagingparams = array(
                ...
                "editlink" => array("/account/customer/addmslcategory/id/#pattern#/edit/yes/","#pattern#"),
                "nodata_message" => $this->translate->_('No Record(s) Found'),
                "fetch_columns_inquery" => $cols_array,
                "show_columns" => $columns_show
                );
        ...
        $printData = (isset($formdata['printData'])) ? $formdata['printData'] : $params['printData'];
        // called stored procedure for counter
        $result 	= $this->SFA_Comman->executequery('CALL sp_get_account_customer_mslcat(?,?,?,?,?,?,?,?)',$param_array,'',$downloadCSV,$printData,2,$pagingparams);
	    ...
	    public function addmslcategoryAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		...
		if($formdata['txtcode'] && $formdata['txtname'])
		{
			...
			{	    
				...
				$last_id = $this->SFA_Comman->executequery('CALL sp_edit_account_customer_addmslcat(?,?,?,?,?,?)',$param_array,'');
			...
			else
			{
				...
				$last_id = $this->SFA_Comman->executequery('CALL sp_add_account_customer_addmslcat(?,?,?,?,?)',$param_array,'');
				
				SFA_Message::setMsg($this->translate->_('New Record'));
			}
		   $this->_helper->redirector('mslcategory', 'customer', 'account');
		}
		elseif($params['id'] > 0)
		{
			$result  		= $this->SFA_Comman->executequery('CALL sp_get_account_customer_addmslcat(?)',$params['id'],'');
			$res['txtcode'] 	= $result[0][0]['categoryid'];
			$res['txtname'] 	= $result[0][0]['categoryname'];
			$res['txtname_arb'] = $result[0][0]['arbcategoryname'];
			$res['ddlstatus'] 	= $result[0][0]['activestatus'];
