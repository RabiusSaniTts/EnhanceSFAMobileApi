...
class Inventory_TargetController extends Inventory_Library_Controller_Action_Abstract
{
    ...
    */
    public function itempackageAction(){
       ...
       $this->view->params = $params = $this->getRequest()->getParams();
		...
		$description	= $this->translate->_('Item Package Description');
		...
		if($Settings_NameSpace->cpanel["Use Alternate Code"]['status']) {
			...
			$columns_show 	= array($this->translate->_('Package ID'),$this->translate->_('Alternate Code'),$this->translate->_('Description'),$this->translate->_('Status'));
		}
		else
		{
			$cols_array 	= array('packagecode','packagedescription','activestatus');
			$columns_show 	= array($this->translate->_('Package ID'),$this->translate->_('Description'),$this->translate->_('Status'));
		}
    ...
    */
    public function salesmantargetAction()
    {
		...
		/*Following code added by Nilesh on 6Mar2016*/
		$From	 		= $this->translate->_('From');
		$To	 			= $this->translate->_('To');
		
		$cols_array 	= array('sales.salesmancode','salesmanname1','routename','fromdate', 'todate','primary_key as edit_del_primary_id' );
		$columns_show 	= array($salesmancode,$salesmanname,$routename,$From,$To);
    ...
    */
    public function addsalesmantargetAction() {
		...
		if(count($formdata) > 0)
		{
			...
			$param_array[11] = $formdata['txtcomval2'];
    ...
    */
    ...
    {	
		...
		$noeditfield = array('packagedescription','packagenumber');
		
		//varible declaration for grid title	
		$code	 	= $this->translate->_('Package Code');
		$name       = $this->translate->_('Description');
		$from_date	= $this->translate->_('From Date');
		$to_date	= $this->translate->_('To Date');
		$quantity	= $this->translate->_('Qty/Amount');
		$com1	= $this->translate->_('Comm.No');
		$com2	= $this->translate->_('Comm.Per');		
		
		// column to be fetched
		$columns_array 	=  array('rg.packagenumber','packagedescription','DATE_FORMAT(fromdate,"%d-%m-%Y") as fromdate','DATE_FORMAT(todate,"%d-%m-%Y") as todate','quantity','comval1','comval2','primary_key as edit_del_primary_id');
		$columns_show  	= array($code,$name,$from_date,$to_date,$quantity,$com1,$com2);
		...
		if($params["update"]=="yes") {			
			...
			$updateData["6"] = $params["comval1"];
			$updateData["7"] = $params["comval2"];
