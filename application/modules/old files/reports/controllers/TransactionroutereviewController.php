...
*/
class Reports_TransactionroutereviewController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function routereviewAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Route Visit Review";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/transactionroutereview/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/transactionroutereview/exportcsv";
    }
    /**
      ...
      *
      */
    public function routevisitAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_transaction_route_review(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $total_callexceptions += $row['callexceptions'];
                $total_nontargetvisits += $row['nontargetvisits'];
                $total_totalvisits += $row['totalvisits'];
                $total_schedulesale += $row['schedulesale'];
                $total_schedulenosale += $row['schedulenosale'];
                $total_unschedsale += $row['unschedsale'];
                $total_unschedulenosale += $row['unschedulenosale'];
                $total_effectivevisit += $row['effectivevisit'];
                $total_startkms += $row['startkms'];
                ...
                $responce->rows[$i]['cell']=array($routename,$row['routestartdate1'],$row['daytxt'],$row['routeenddate1'],$row['targettovisit'],$row['targetvisits'],$row['callexceptions'],$row['nontargetvisits'],$row['totalvisits'],$row['schedulesale'],$row['schedulenosale'],$row['unschedsale'],$row['unschedulenosale'],$row['effectivevisit'],$row['startkms'],$row['endkms'],$row['kmscovered']);
                $i++;
        ...
        $responce->userdata['startkms'] = $total_startkms;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Route Start Date'),$this->translate->_('Day'),$this->translate->_('Route End Date'),$this->translate->_('Scheduled Targets To Visit'),$this->translate->_('Scheduled Targets Visited'),$this->translate->_('Scheduled Targets Not Visited'),$this->translate->_('Un Scheduled Targets Visited'),$this->translate->_('Total Visits'),$this->translate->_('Shedule Sale'),$this->translate->_('Shedule No Sale'),$this->translate->_('Un Shedule Sale'),$this->translate->_('Un Shedule No Sale'),$this->translate->_('Effective Visit'),$this->translate->_('Starting Kms'),$this->translate->_('Ending Kms'),$this->translate->_('Total Kms Covered'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$routename][] = array($result_arr[0][$i]['routestartdate1'],$result_arr[0][$i]['daytxt'],$result_arr[0][$i]['routeenddate1'],$result_arr[0][$i]['targettovisit'],$result_arr[0][$i]['targetvisits'],$result_arr[0][$i]['callexceptions'],$result_arr[0][$i]['nontargetvisits'],$result_arr[0][$i]['totalvisits'],$result_arr[0][$i]['schedulesale'],$result_arr[0][$i]['schedulenosale'],$result_arr[0][$i]['unschedsale'],$result_arr[0][$i]['unschedulenosale'],$result_arr[0][$i]['effectivevisit'],$result_arr[0][$i]['startkms'],$result_arr[0][$i]['endkms'],$result_arr[0][$i]['kmscovered']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          		= $column_model_arr;
        $data_arr["config"]["report_title"] 		= $this->translate->_('Route Visit Review').$report_title;
        $data_arr["config"]["report_title_height"] 	= $report_title_height;
        $data_arr["config"]["file_name"]    		= "RouteVisitReview";
