...
class Reports_MerchandizingposController extends Reports_Library_Controller_Action_Abstract
{
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "POS Tracking";
    ...
    public function postrackAction()
    {
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routecode']." - ".$route,$row['salesmancode']." - ".$$salesman,$row['customercode'],$row['customername'],$row['date'],$row['time'],$itemdescription,$row['quantity'],$row['serialnumber'],$posinstructionname,strtoupper($row['posstatus']));
                $i++;
            }
        }
        else
