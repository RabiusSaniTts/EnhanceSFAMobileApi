...
*/
class Reports_MerchandizingsurveyController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function init()
    {
        ...
        $this->report_session		= new Zend_Session_Namespace('Re_merchandizingsurvey');
    ...
    */
    ...
    public function surveytrackingAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Survey Tracking";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/merchandizingsurvey/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/merchandizingsurvey/exportcsv";
    ...
    /**
      ...
      *
      */
    public function surveytrackAction()
    {
        $this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_merchandizing_survey(?,?,?,?,?)',$param_array,'');
       ...
       // $count  = !empty($result_arr[0]) ? count($result_arr[0]) : 0;
        ...
        foreach($result_arr[1] as $row) {
            $customername = ($this->css == 'ar_') ? $row['arbcustomername'] : $row['customer'];
            $salesman = ($this->css == 'ar_') ? $row['arbsalesmanname1'] : $row['salesman'];
            $route = ($this->css == 'ar_') ? $row['arbroutename'] : $row['route'];
            $surveydescription = ($this->css == 'ar_') ? $row['arbsurveyprompt'] : $row['surveydescription'];
            
            $responce->rows[$i]['id']=$i;
            
            $responce->rows[$i]['cell']=array($row['routecode']." - ".$route,$row['salesmancode']." - ".$salesman,$row['customercode'],$customername,$row['date'],$row['time'],$surveydescription,$row['surveyresponse']);
            $i++;
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Salesman'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Date'),$this->translate->_('Time'),$this->translate->_('Survey Description'),$this->translate->_('Survey Response'));
        ...
        for($i = 0; $i < count($result_arr[1]); $i++)
        {
            ...
            $surveydescription = ($this->css == 'ar_') ? $result_arr[1][$i]['arbsurveyprompt'] : $result_arr[1][$i]['surveydescription'];
            
            $column_model_arr[$result_arr[1][$i]['routecode']." - ".$route][$result_arr[1][$i]['salesmancode']." - ".$salesman][] = array($result_arr[1][$i]['customercode'],$customername,$result_arr[1][$i]['date'],$result_arr[1][$i]['time'],$surveydescription,$result_arr[1][$i]['surveyresponse']);
        }
        //pr($column_model_arr,1);
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_("Survey Tracking").$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "SurveyTracking";
