...
class Reports_TransactiondepositsummaryController extends Reports_Library_Controller_Action_Abstract
{
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Deposit Summary";
    ...
    public function depositsumAction()
    {
        ...
        if(!empty($result_arr[0])){
            foreach($result_arr[0] as $row) {
                ...
                $responce->rows[$i]['cell'] = array($row['routecode']." - ".$routename,$row['salesmancode']." - ".$salesmanname,$row['transactiondate'],$row['trantype'],$row['immediatecash'],$row['immediatecheck'],$row['total']);
                $i++;
            }
        }
        else
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Salesman'),$this->translate->_('Transaction Date'),$this->translate->_('Type'),$this->translate->_('Immediate Cash'),$this->translate->_('Immediate Check'),$this->translate->_('Total'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][$result_arr[0][$i]['salesmancode']." - ".$salesmanname][$result_arr[0][$i]['trantype']][] = array($result_arr[0][$i]['transactiondate'],$result_arr[0][$i]['immediatecash'],$result_arr[0][$i]['immediatecheck'],$result_arr[0][$i]['total']);
        }
