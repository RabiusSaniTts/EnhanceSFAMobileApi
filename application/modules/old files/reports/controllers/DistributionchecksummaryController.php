...
class Reports_DistributionchecksummaryController extends Reports_Library_Controller_Action_Abstract
{
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Distribution Check Summary";
     ...
     public function distributioncheckdataAction()
     {
	  ...
	  if(!empty($result_arr[1])){
	       foreach($result_arr[1] as $row) {
		    ...
		    $responce->rows[$i]['cell'] = array($row['routecode'],$row['actualitemcode'],$row['itemdescription'],$row['caseprice'],$row['defaultsalesprice'],$row['cutoff_qty'],$row['max_qty'],$row['location1'],$row['location2'],$row['expiry_date']);
		    $i++;
