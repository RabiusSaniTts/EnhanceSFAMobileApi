...
class Reports_RoutedepositsummaryController extends Reports_Library_Controller_Action_Abstract
{
    ...
    */
        ...
        if(!empty($result_arr[0])) {
            for($j = $start; $j < count($result_arr[0]); $j++,$i++)
            {
                ...
                if($result_arr[0][$j]['paytype'] == "cash")
                    $cashval = $result_arr[0][$j]['cashamt'];
                else
                    $cashval = $result_arr[0][$j]['checkamt'];
                ...
                $responce->rows[$i]['cell'] = array($result_arr[0][$j]['routeenddate'],$result_arr[0][$j]['routecode']." - ".$routename,$result_arr[0][$j]['type'],
                                                    $result_arr[0][$j]['paytype'],$result_arr[0][$j]['invoicenumber'],date("H:i",strtotime($result_arr[0][$j]['transactiontime'])),
                                                    $result_arr[0][$j]['againstinv'],$result_arr[0][$j]['saleinvoiceddate'],$result_arr[0][$j]['salesmancode'],
                                                    $result_arr[0][$j]['customercode'],$customername,$bankname,$result_arr[0][$j]['checknumber'],'',$cashval);
    ...
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route End Date'),$this->translate->_('Route Code'),$this->translate->_('Invoice Type'),$this->translate->_('Payment Type'),$this->translate->_('Invoice Number'),$this->translate->_('Transaction Time'),$this->translate->_('Against Invoice'),$this->translate->_('Invoiced Date'),$this->translate->_('Invoiced Salesman'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Bank Name'),$this->translate->_('Check Number'),$this->translate->_('Draw Date'),$this->translate->_('Amount'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routeenddate']][$result_arr[0][$i]['routecode']." - ".$result_arr[0][$i]['routename']][$result_arr[0][$i]['type']][$result_arr[0][$i]['paytype']][] = array($result_arr[0][$i]['invoicenumber'],
                                                    ...
                                                    $result_arr[0][$i]['customername'],$result_arr[0][$i]['bankname'],$result_arr[0][$i]['checknumber'],'',$cashval);
        }
