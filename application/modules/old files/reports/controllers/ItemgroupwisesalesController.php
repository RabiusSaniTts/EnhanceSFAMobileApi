...
class Reports_ItemgroupwisesalesController extends Reports_Library_Controller_Action_Abstract
{
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Item Group Wise Sales";
    ...
    public function itemgroupwisesalesdataAction()
    {
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $total = $row['sales_invprice'] - ($row['goodretun_invprice']+$row['damagereturn_invprice']+$row['expiryreturn_invprice']+$row['itempromodiscount']);
                $salesqty += $row['salesqty'];
                $sales_invprice += $row['sales_invprice'];
                $returnqty += $row['returnqty'];
                $goodretun_invprice += $row['goodretun_invprice'];
                $damagedqty += $row['damagedqty'];
                $damagereturn_invprice += $row['damagereturn_invprice'];
                $expiryqty += $row['expiryqty'];
                $expiryreturn_invprice += $row['expiryreturn_invprice'];
                ...
                $responce->rows[$i]['cell'] = array($row['routecode'].' - '.$routename, $row['salesmancode'].' - '.$salesmanname,
                                                    $row['majorcategorycode'].' - '.$majorcategory, $row['itemcode'],$itemdescription,
                                                    $row['unitspercase'],$row['salesqty'], $row['sales_invprice'],$row['returnqty'], $row['goodretun_invprice'],
                                                    $row['damagedqty'], $row['damagereturn_invprice'], $row['expiryqty'], $row['expiryreturn_invprice'],
                                                    $row['freesampleqty'], $row['freegods_stdprice'], $row['itempromodiscount'],$total);
                $i++;
            }
        }
        else
    ...
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Salesman'),$this->translate->_('Item Category'),$this->translate->_('Item Code'),$this->translate->_('Description'),$this->translate->_('UPC'),$this->translate->_('Sales Qty'),$this->translate->_('Sales @  Inv. Price'),$this->translate->_('G. Return Qty'),$this->translate->_(
                                     'G.Return @ Inv. Price'),$this->translate->_('Damage Qty'),$this->translate->_('Damaged @ Inv. Price'),$this->translate->_('Expired Qty'),$this->translate->_('Expired @ Inv. Price'),$this->translate->_('Free Qty'),$this->translate->_(
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $total = $result_arr[0][$i]['sales_invprice'] - ($result_arr[0][$i]['goodretun_invprice']+$result_arr[0][$i]['damagereturn_invprice']+$result_arr[0][$i]['expiryreturn_invprice']+$result_arr[0][$i]['itempromodiscount']);
            
            $column_model_arr[$result_arr[0][$i]['routecode'].' - '.$routename][$result_arr[0][$i]['salesmancode'].' - '.$salesmanname][$result_arr[0][$i]['majorcategorycode'].' - '.$majorcategory][]
                = array(
                        $result_arr[0][$i]['itemcode'], $itemdescription,
                        $result_arr[0][$i]['unitspercase'],$result_arr[0][$i]['salesqty'], $result_arr[0][$i]['sales_invprice'],$result_arr[0][$i]['returnqty'], $result_arr[0][$i]['goodretun_invprice'],
                        $result_arr[0][$i]['damagedqty'], $result_arr[0][$i]['damagereturn_invprice'], $result_arr[0][$i]['expiryqty'], $result_arr[0][$i]['expiryreturn_invprice'],
                        $result_arr[0][$i]['freesampleqty'], $result_arr[0][$i]['freegods_stdprice'], $result_arr[0][$i]['itempromodiscount'],$total);
        }
