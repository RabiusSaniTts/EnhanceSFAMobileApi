...
class Reports_ReturnsummaryController extends Reports_Library_Controller_Action_Abstract
{
    ...
    */
    ...
    public function returnsummarydataAction()
    {
        ...
        if(!empty($result_arr[1])){
            foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']=array($row['routecode']." - ".$routename,$row['hocode']." - ".$row['honame'],$row['description'],
                                                $row['majorcategorycode']." - ".$row['majorcategroy'],$row['itemcode'],$row['itemdescription'],
                                                $row['unitspercase'], $row['returnprice'],$row['quantity'],$row['value']
                                                );
                $i++;
            }
        }
        ...
        $responce->userdata['unitspercase'] = $total_upc;
    ...
    public function exportAction()
    {
        ...
        $param_array = array();
        $param_array[1] = $extra_where;
        $param_array[2] = "routecode,hocode,description,majorcategorycode,".$sidx;
        $param_array[3] = $sord;
        $param_array[4] = $limit;
        $param_array[5] = $page;
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('HO'),$this->translate->_('Return Reason'),$this->translate->_('Item Group'),$this->translate->_( 'Item Code'),$this->translate->_('Item Description'),$this->translate->_('UPC'),$this->translate->_('Return Price (Pcs)'),$this->translate->_('Qty in PCS'),$this->translate->_('Value'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][$result_arr[0][$i]['hocode']." - ".$result_arr[0][$i]['honame']][$result_arr[0][$i]['description']][$result_arr[0][$i]['majorcategorycode']." - ".$result_arr[0][$i]['majorcategroy']][] =
            array($result_arr[0][$i]['itemcode'],$result_arr[0][$i]['itemdescription'],$result_arr[0][$i]['unitspercase'], $result_arr[0][$i]['returnprice'],$result_arr[0][$i]['quantity'],$result_arr[0][$i]['value']);
        }
