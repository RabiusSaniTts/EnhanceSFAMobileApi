...
*/
class Reports_TabletfreesummaryController extends Reports_Library_Controller_Action_Abstract
{
     /**
    * @name       init
    * @since      01-10-2012
    ...
    */
    ...
    public function init()
    {
		...
		$this->report_session		= new Zend_Session_Namespace('Re_freesummary');
    }
    ...
    */
    ...
    public function freesummaryAction()
    {
    ...
    */
    public function indexAction()
    {
        ...
        $this->view->ReportTitle = "Free Summary";
        ...
        $this->view->xlsexport_link = $this->view->baseUrl()."/reports/tabletfreesummary/export";
        $this->view->cvsexport_link = $this->view->baseUrl()."/reports/tabletfreesummary/exportcsv";
    }
    /**
    ...
    public function freesumAction()
    {
        ...
        $result_arr = $this->SFA_Comman->executequery('CALL sp_report_tablet_free_summary(?,?,?,?,?)',$param_array,'');
        ...
        if(!empty($result_arr[1])){
        	foreach($result_arr[1] as $row) {
                ...
                $responce->rows[$i]['cell']= array($row['routecode']." - ".$routename,$row['customercode']." - ".$customername
                                                    , $row['itemcode'], $itemdescription, $row['cases']." / ".$row['units'],
                                                    number_format($row['value'],$this->decimalplaces));
                $i++;
            }
        }
        else
        ...
        //$responce->userdata['EmployeeID'] = $row['routecode']." - ".$row['routename'];
    ...
    */
    public function exportAction()
    {
        ...
        $data_arr["columns"] = array($this->translate->_('Route'),$this->translate->_('Customer Code'),$this->translate->_('Item Code'),$this->translate->_('Item Description'),$this->translate->_('Quantity'),$this->translate->_('Amount'));
        ...
        for($i = 0; $i < count($result_arr[0]); $i++)
        {
            ...
            $column_model_arr[$result_arr[0][$i]['routecode']." - ".$routename][$result_arr[0][$i]['customercode']." - ".$customername][] =
                                        array($result_arr[0][$i]['itemcode'], $itemdescription, $result_arr[0][$i]['cases']." / ".$result_arr[0][$i]['units'],
                                        number_format($result_arr[0][$i]['value'],$this->decimalplaces));
        }
        
        $data_arr["columns_model"]          = $column_model_arr;
        $data_arr["config"]["report_title"] = $this->translate->_("Tablet Free Summary").$report_title;
        $data_arr["config"]["report_title_height"] = $report_title_height;
        $data_arr["config"]["file_name"]    = "FreeGoodsDetail";
