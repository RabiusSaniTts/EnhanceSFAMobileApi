...
class Hhctransaction_CloudtranController extends Hhctransaction_Library_Controller_Action_Abstract
{
    /**
    * @name       init
    * @since      01-02-2012
    ...
    */
    ...
    public function invoiceAction()
    {
		...
		if($params['delid'] > 0)
		{
			...
			if($result[0][0]['dontdelete'] != 'Not Found') {
				SFA_Message::setErrorMsg($this->translate->_('Already Receipt Is Generated.'));
				$ids		= explode(',',$result[0][0]['dontdelete']);
				$checked 	= $ids;
			}
			
			$this->_helper->redirector("invoice", "cloudtran", "hhctransaction");
		...
		if($formdata["hdDelete"]==1)
		{
			...
			if($result[0][0]['dontdelete'] != 'Not Found') {
				SFA_Message::setErrorMsg($this->translate->_('Already Receipt Is Generated.'));
				$dontdeleteids		= explode(',',$result[0][0]['dontdelete']);				
			}
			
			if($result[0][0]['deleted_id'] =='')
			{
				$ids		= explode(',',$ids);
				$checked 	= array_merge($ids,$dontdelete);
			...
			else
			{
				...
				if(count($ids) != count($deleted_id)){
					$checked 	= array_merge($checked,$dontdelete);
					SFA_Message::setErrorMsg($this->translate->_('Record Already In use'));
				}			
					SFA_Message::setMsg($this->translate->_('Delete Record'));
		...
		if($altcode_status) {
			$cols_array = array('cm.alternatecode','customername','invoicenumber','routename','IF(voidflag = 0,"NORMAL","VOIDED") AS voidflag','FORMAT(totalinvoiceamount,'.$this->decimalplaces.') AS totalinvoiceamount','transactionkey as edit_del_primary_id' );
		}
		else {
			$cols_array = array('cm.customercode','customername','invoicenumber','routename','IF(voidflag = 0,"NORMAL","VOIDED") AS voidflag','FORMAT(totalinvoiceamount,'.$this->decimalplaces.') AS totalinvoiceamount','transactionkey as edit_del_primary_id' );
		}
		...
		$Common_NameSpace = new Zend_Session_Namespace('Cloud_Invoice');
		...
		if($Common_NameSpace->tdate)
			$additional_where_condition[] = " (transactiondate BETWEEN \'".date("Y-m-d 00:00:00",strtotime(str_replace('/', '-', $Common_NameSpace->tdate)))."\' AND \'".date("Y-m-d 23:59:59",strtotime(str_replace('/', '-', $Common_NameSpace->tdate)))."\' )  AND record_flag = \'0\' ";
    ...
    * @param
    *
    * This action is for Add cloudtran information
    ...
    public function addinvoiceinfoAction()
    {
		...
		// create combo for route,customer, and item
		 ...
		 if(count($formdata) > 0)
		 {
			...
			$this->_helper->redirector('addinvoiceinfo', 'cloudtran', 'hhctransaction');
		}
    }
    /**
    ...
    * @param
    *
    * This action is for add add cloudtran
    *
    */
    public function addinvoiceAction()
    {
		...
		if(isset($params["id"]) && $params["id"]>0)
			...
			$this->view->invoiceitemgrid    = $this->view->BaseUrl("/hhctransaction/cloudtran/invoicedetail".$ex_param);
    ...
    * @param
    *
    * This action is for display ajax item grid in add cloudtran page
    ...
    */
	...
	if(isset($params["key"]) && $params["key"]>0)
		...
		$sales_case_price	= $this->translate->_('OUT');
		...
		$columns_array		= array('im.actualitemcode','im.itemshortdescription','im.unitspercase',
				...
				'FORMAT(id.goodreturncaseprice,'.$this->decimalplaces.') AS returncaseprice','FORMAT(id.goodreturnprice,'.$this->decimalplaces.') AS returnprice',
				...
				'FORMAT(promoamount,'.$this->decimalplaces.') AS discount','FORMAT((((FLOOR(salesqty/im.unitspercase)*id.salescaseprice)+((salesqty%im.unitspercase)*id.salesprice))-((FLOOR(returnqty/im.unitspercase)*id.goodreturncaseprice)+((returnqty%im.unitspercase)*id.goodreturnprice))
				-((FLOOR(damagedqty/im.unitspercase)*id.returncaseprice)+((damagedqty%im.unitspercase)*id.returnprice))-promoamount),'.$this->decimalplaces.') AS total_amount','1 AS sett_tran_type','CONCAT(id.transactionkey,"_",im.actualitemcode) as edit_del_primary_id'
				);
		$columns_show  = array($item_code,$item_name,$upc,$sales_case_price,$sales_pcs_price,$return_case_price,$return_pcs_price,$case_sales_qty,$pcs_sales_qty,$case_return_qty,$pcs_return_qty,$case_damage_qty,$pcs_damage_qty,$case_free_qty,$pcs_free_qty,$case_buyback_qty,$pcs_buyback_qty,$case_rental_qty,$pcs_rental_qty,$case_free_qty,$pcs_free_qty,$discount,$total_amount);
	...
	$pagingparams = array(
			...
			"show_deletelink" => false,			
			"show_deleteall" => false,
			"currentlink" => array("/hhctransaction/cloudtran/detailinvoice".$ex_param),
			...
			"show_top_columns_value" => array(array("3",""),array("4","Sales"),array("4","Return"),array("2","Damage"),array("2","Free"),array("2","BuyBack"),array("2","Rental"),array("3","Promotion"),array("1",""),array("1","")),
			"show_total_columns"=>true,			
			"show_total_columns_value"=>array("case_sales_qty"=>"0","pcs_sales_qty"=>"0","case_return_qty"=>"0","pcs_return_qty"=>"0","case_damage_qty"=>"0","pcs_damage_qty"=>"0","case_manualfreeqty_qty"=>"0","pcs_manualfreeqty_qty"=>"0","case_returnfreeqty_qty"=>"0","pcs_returnfreeqty_qty"=>"0","case_fixedrentqty_qty"=>"0","pcs_fixedrentqty_qty"=>"0","total_amount"=>"1","casediscount"=>"0","pcsdiscount"=>"0"),
			"show_total_columns_msg"=>array("itemshortdescription","Total",$this->decimalplaces),
			"show_columns_right_side" =>array('salescaseprice','salesprice','returncaseprice','returnprice',"case_sales_qty","pcs_sales_qty","case_return_qty","pcs_return_qty","case_damage_qty","pcs_damage_qty","case_manualfreeqty_qty","pcs_manualfreeqty_qty","case_returnfreeqty_qty","pcs_returnfreeqty_qty","case_fixedrentqty_qty","pcs_fixedrentqty_qty","discount","total_amount",'casediscount','pcsdiscount'),
    ...
    */
    public function viewbatchesgridAction()
    {
		...
		if(isset($params["key"]) && $params["key"]!= "") {
            $idarr = explode("_",$params["key"]);
			$ex_param = "/key/".$params["key"]."/key1/".$params["key1"];
			$additional_where_condition[] = ' id.transactionkey = "'.$idarr[0].'" ';
			$additional_where_condition[] = ' transactiontypecode = "'.$params["key1"].'" ';
            $additional_where_condition[] = ' bd.itemcode = "'.$idarr[1].'" ';
		}
    ...
    */ 
    ...
    {	
		...
		if($params['delid'] > 0)
		{
			...
			$this->_helper->redirector("salesorder", "cloudtran", "hhctransaction");
		}
		...
		if($altcode_status) {
			$cols_array = array('cm.alternatecode','customername','invoicenumber','routename','IF(voidflag = 0,"NORMAL","VOIDED") AS voidflag','DATE_FORMAT(orderdeliverydate,"%d-%m-%Y") AS orderdeliverydate','FORMAT(totalinvoiceamount,'.$this->decimalplaces.') AS totalinvoiceamount','transactionkey as edit_del_primary_id' );
		}
		else {
			$cols_array = array('cm.customercode','customername','invoicenumber','routename','IF(voidflag = 0,"NORMAL","VOIDED") AS voidflag','DATE_FORMAT(orderdeliverydate,"%d-%m-%Y") AS orderdeliverydate','FORMAT(totalinvoiceamount,'.$this->decimalplaces.') AS totalinvoiceamount','transactionkey as edit_del_primary_id' );			
		}
    ...
    */
	...
	if(isset($params["id"]) && $params["id"]>0)
        ...
        $this->view->invoiceitemgrid    = $this->view->BaseUrl("/hhctransaction/cloudtran/salesorderdetail".$ex_param);
    }
    ...
    */
	...
	if(isset($params["key"]) && $params["key"]>0)
		...
		if($altcode_status)
		{
			...
			$columns_array 	=  array('im.alternatecode','im.itemshortdescription','im.unitspercase',
						'FLOOR((salesqty/im.unitspercase)) AS case_sales_qty','(salesqty%im.unitspercase) AS pcs_sales_qty',
						'FORMAT(id.salescaseprice,'.$this->decimalplaces.') AS salescaseprice','FORMAT(id.salesprice,'.$this->decimalplaces.') AS salesprice',
						'FLOOR((returnqty/im.unitspercase)) AS case_return_qty','(returnqty%im.unitspercase) AS pcs_return_qty',
						'FORMAT(id.goodreturncaseprice,'.$this->decimalplaces.') AS returncaseprice','FORMAT(id.goodreturnprice,'.$this->decimalplaces.') AS returnprice',				
						'FLOOR((damagedqty/im.unitspercase)) AS case_damage_qty','(damagedqty%im.unitspercase) AS pcs_damage_qty',
						'IFNULL(FLOOR((manualfreeqty/im.unitspercase)),0) AS case_manualfreeqty_qty','IFNULL((manualfreeqty%im.unitspercase),0) AS pcs_manualfreeqty_qty',
						'FLOOR((freesampleqty/im.unitspercase)) AS casediscount','(freesampleqty%im.unitspercase) AS pcsdiscount',				
						'FORMAT(promoamount,'.$this->decimalplaces.') AS discount',
						'FORMAT(((((salesqty/im.unitspercase)*id.salescaseprice)+
						((salesqty%im.unitspercase)*id.salesprice))-(((returnqty/im.unitspercase)*id.goodreturncaseprice)
						+((returnqty%im.unitspercase)*id.goodreturnprice))-(((damagedqty/im.unitspercase)*id.returncaseprice)
						+((damagedqty%im.unitspercase)*id.returnprice))-promoamount),'.$this->decimalplaces.') AS total_amount'
						);
			// column header to be displayed
			$columns_show  = array($alt_code,$item_name,$upc,$sales_case_price,$sales_pcs_price,$return_case_price,$return_pcs_price,$case_sales_qty,$pcs_sales_qty,$case_return_qty,$pcs_return_qty,$case_damage_qty,$pcs_damage_qty,$case_free_qty,$pcs_free_qty,$case_free_qty,$pcs_free_qty,$discount,$total_amount);
		}
		else
		{
			// column to be fetched	
	...
	$pagingparams = array(
			...
			"currentlink" => array("/hhctransaction/cloudtran/salesorderdetail".$ex_param),
			"editlink" => array("/id/#pattern#/edit/yes","#pattern#"),
			"deletelink" => array("/id/#pattern#/delete/yes","#pattern#"),
			"nodata_message" => $this->translate->_('No Record(s) Found'),
			"fetch_columns_inquery" => $columns_array,
			"show_columns" => $columns_show,
			"show_top_columns" => true,
			"show_top_columns_value" => array(array("3",""),array("4",$this->translate->_('Order')),array("4",$this->translate->_("Return")),array("2",$this->translate->_("Damage")),array("2",$this->translate->_("Free")),array("3",$this->translate->_("Promotion")),array("1","")),
			"show_total_columns"=>true,			
    ...
    */
    ...
    public function arcollectionAction()
    {
		...
		if($params['delid'] > 0)
		{
			...
			$this->_helper->redirector("arcollection", "cloudtran", "hhctransaction");	
		}
		...
		$cols_array = array('cm.customercode','customername','invoicenumber','routename','FORMAT(amountpaid,'.$this->decimalplaces.')  AS amountpaid','transactionkey as edit_del_primary_id' );
		
		if($altcode_status)
			$cols_array[0] = 'cm.alternatecode';
	...
	$not_in_search[] 	= 'transactiondate';
	 ...
	 // CREATE A SESSION NAMESPACE
	$Common_NameSpace = new Zend_Session_Namespace('Cloud_ARCollection');
    ...
    */
	...
	if(isset($params["id"]) && $params["id"]>0)
        ...
        $this->view->invoiceitemgrid    = $this->view->BaseUrl("/hhctransaction/cloudtran/arheaderdetail".$ex_param);
    }
    /**
    ...
    */
    ...
    public function advpaymentAction()
    {
		...
		if($params['delid'] > 0)
		{
			...
			$this->_helper->redirector("advpayment", "cloudtran", "hhctransaction");
		}
	...
	if($altcode_status) {
		$cols_array = array('cm.alternatecode','customername','invoicenumber','routename','FORMAT(amountpaid,'.$this->decimalplaces.') AS totalinvoiceamount ','transactionkey as edit_del_primary_id' );
	}
	else {
		$cols_array = array('cm.customercode','customername','invoicenumber','routename','FORMAT(amountpaid,'.$this->decimalplaces.') AS totalinvoiceamount ','transactionkey as edit_del_primary_id' );
	}
	$columns_show =  array( $this->translate->_('Customer Code'),
							$this->translate->_('Customer'),
							$this->translate->_('Receipt No'),
	...
	// CREATE A SESSION NAMESPACE
	$Common_NameSpace = new Zend_Session_Namespace('Cloud_AdvPayment');
    ...
    * @param
    *
    * This action is for getting loadperiod number and salesman name
    *
    */
    public function getloadperiodAction()
    {
		$params = $this->getRequest()->getParams();
		$result = $this->SFA_Comman->executequery('CALL sp_combo_loadperiod_routecode(?)',$params['routeid'],'');
