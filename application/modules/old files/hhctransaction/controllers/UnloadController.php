...
class Hhctransaction_UnloadController extends Hhctransaction_Library_Controller_Action_Abstract
{
      /**
    * @name       init
    * @since      01-02-2012
    ...
    *
    */
    public function unloadretAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		
		$cols_array 	= array('ith.routecode','sm.salesmanname1','rm.routename','sm.salesmanname1','ith.documentnumber','ith.transactiontime','ith.detailkey AS edit_del_primary_id');
		...
		$Common_NameSpace = new Zend_Session_Namespace('UnloadReturn');
		
		$last_url 		= htmlspecialchars($_SERVER['HTTP_REFERER']);
		$end_last_url 	= explode('/',$last_url);
		
		if(end($end_last_url) == 'unloadret' || strpos($last_url,'viewunloadret') || strpos($last_url,'/unloadret/' )) {
		...
		$additional_where_condition[] = "  ith.transactiontype = 3 ";
		...
		$pagingparams = array(
				...
				"primaryid" => "detailkey",
				"show_extralink" => true,
				"extralink" => array(array("View","/".$params['module']."/".$params['controller']."/viewunloadret/id/#pattern#","#pattern#")),
		...
		$param_array[9] = " AND ( itd.transactiontypecode = 11 OR itd.transactiontypecode = 14 OR itd.transactiontypecode = 5 )";
    ...
    *
    */
    public function viewunloadretAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		...
		$this->view->itemgrid    = $this->view->BaseUrl("/".$params['module']."/".$params['controller']."/viewunloadretgrid".$ex_param);
    ...
    */
    public function viewunloadretgridAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
    ...
    * @param
    *
    * This action is for display damage return in stock
    *
    */
    public function unloadvarAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		...
		$Common_NameSpace = new Zend_Session_Namespace('UnloadVariance');
		
		$last_url 		= htmlspecialchars($_SERVER['HTTP_REFERER']);
		$end_last_url 	= explode('/',$last_url);
		
		if(end($end_last_url) == 'unloadvar' || strpos($last_url,'viewunloadvar') || strpos($last_url,'/unloadvar/' )) {
		...
		$pagingparams = array(
				...
				"primaryid" => "detailkey",
				"show_extralink" => true,
				"extralink" => array(array("View","/".$params['module']."/".$params['controller']."/viewunloadvar/id/#pattern#","#pattern#")),
				"nodata_message" => $this->translate->_('No Record(s) Found'),
				"fetch_columns_inquery" => $cols_array,
				"show_columns" => $columns_show,
				"additional_where" => $additional_where_condition
		...
		$param_array[7] = implode(", ",$pagingparams["fetch_columns_inquery"]);
		$param_array[8] = strlen($get_return_vals['where_condition'])>0?$get_return_vals['where_condition']:'';
		$param_array[9] = " AND itd.transactiontypecode = 9";
		// called stored procedure for counter
		$result = $this->SFA_Comman->executequery('CALL sp_get_hhctransaction_stock_beginstock(?,?,?,?,?,?,?,?)',$param_array,'');
    ...
    *
    */
    public function viewunloadvarAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
        $this->view->formdata = $formdata = $this->_request->getPost();
		...
		$this->view->itemgrid    = $this->view->BaseUrl("/".$params['module']."/".$params['controller']."/viewunloadvargrid".$ex_param);
    ...
    * This action is use for view item grid in viewbeginstockAction
    */
    public function viewunloadvargridAction()
    {
		$this->view->params = $params = $this->getRequest()->getParams();
		...
		if(isset($params["key"]) && $params["key"]>0){
			$ex_param = "/key/".$params["key"];			
			$additional_where_condition[] = ' ( detailkey = "'.$params["key"].'" AND itd.transactiontypecode = 9 )';
		}
