...
class Links_RouteController extends Links_Library_Controller_Action_Abstract
{
    ...
    * This action is use for the assign category to selected route
    */
    public function routemandatorylinkAction()
    {
        $this->view->formdata = $formdata = $this->_request->getPost();
	
        $fill_by = array();
            $fill_by[0]['id']  = "Mandatory";
        $fill_by[0]['val'] = 'Mandatory';
        $fill_by[1]['id']  = "Optional";
        $fill_by[1]['val'] = 'Optional';
        $fill_by[2]['id']  = "Disable";
        $fill_by[2]['val'] = 'Disable';        
        $this->view->fill_by = $fill_by;
        
		$features = array();
        $features[0]['id']  = "Shelf Stock";
        $features[0]['val'] = 'Shelf stock';
        $features[1]['id']  = "Survey";
        $features[1]['val'] = 'Survey';
        ...
        if(count($formdata) > 0)
        {
            ...
            $param_array = array();
            $param_array[1]	= $formdata['ddlfeature'];
            $param_array[2]	= $formdata['ddlfillby'];
            $param_array[3]	= substr($formdata['hdnselectedroute'], 0, -1);	//selected question
            $param_array[4]	= $formdata['hdncount_route'];			// get count for add record in child table
            
            $this->SFA_Comman->executequery('CALL sp_add_link_route_routemandatory(?,?,?,?,?)',$param_array,'');
            
            SFA_Message::setMsg($this->translate->_('Update Record'));	 
            $this->_helper->redirector('routemandatorylink', 'route', 'links');
        }
        else
        {
           /* $result = $this->SFA_Comman->executequery('CALL sp_get_links_route_routeitemlink()','','');
	...
	/**
    * @name       getrouteformandatoryAction
    * @since      10-07-2012
    * @version    Release: 1
    * @author     HD <hiren.d@elantechnologies.com>
    * @copyright  Elan Technologies
    ...
    * This action is use for the get customer list from selected routecode.
    */
    public function getrouteformandatoryAction()
    {
		//view variable declaration
		$params = $this->getRequest()->getParams();
		...
		$param_array 	= array();
		$param_array[1]	= $params['fillby'];		
		$param_array[2]	= $params['feature'];
		
		
		$result = $this->SFA_Comman->executequery('CALL sp_get_route_for_mandatory(?,?)',$param_array,'');
		echo Zend_Json::encode($result);
		exit;
    }
    /**
