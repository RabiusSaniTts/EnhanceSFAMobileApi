...
class Links_CustomerController extends Links_Library_Controller_Action_Abstract
{
    ...
    */
    public function itemmustlinkAction()
    {
		...
		else
		{
			...
			$this->view->route_info 		= $result[4];
			$this->view->customercat_info 	= $result[5];
			$this->view->channel_info 	= $result[6];
		}
    }
    /**
    ...
    */
	...
	exit;
	    ...
	    public function customermappingkeyAction()
    {
	...
	{	    
	  ...
	  // echo $param_array[1];exit;
	    $this->SFA_Comman->executequery('CALL sp_customer_mapping_key_update(?,?,?,?)',$param_array,'');
	    
	    SFA_Message::setMsg($this->translate->_('Update Record'));	 
	    $this->_helper->redirector('customermappingkey', 'customer', 'links');
	}
	else
	{
	    $result = $this->SFA_Comman->executequery('CALL sp_combo_customermappingcall()','','');
...
// print_r($param_array[3]);exit;
	...
	else
	{
	    ...
	    $this->view->route_info 		= $result[1];
		    ...
		    public function getcustomerfrommapkeyAction()
    {
	//view variable declaration
	$params = $this->getRequest()->getParams();
	...
	$result = $this->SFA_Comman->executequery('CALL sp_get_customer_from_mapkey(?,?)',$param_array,'');
	echo Zend_Json::encode($result);
	exit;
    }
