...
const ROUTE_MASTER_RESPONSE_FIELDS = [
  'routecode',
  'routename',
  'arbroutename',
  'subareacode',
  ...
  'reqeododomlogreport',
  'inventoryvalueprint',
  'loadaccessafterunload',
  'routebalance',
  'depotrouteflag',
  'routeinventoryvariance',
  'allowpopulateload',
  'enableaddcustomer',
  'allowgctocash',
  'usesalesdateflag',
...
async function getRouteMaster(
  ...
  return normalizeRouteMasterRows(rows);
}

function normalizeRouteMasterRows(rows: GenericRow[]): GenericRow[] {
  return rows.map((row) => {
    const normalized: GenericRow = {} as GenericRow;

    for (const field of ROUTE_MASTER_RESPONSE_FIELDS) {
