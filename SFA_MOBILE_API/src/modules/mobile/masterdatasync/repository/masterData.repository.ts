...
const CUSTOMER_MASTER_RESPONSE_FIELDS = [
  "customercode",
  "type",
  "headofficecode",
  "routecode",
...
function normalizeCustomerMasterRows(rows: GenericRow[]): GenericRow[] {
  return rows.map((row) => {
    const normalized: GenericRow = {} as GenericRow;

    for (const field of CUSTOMER_MASTER_RESPONSE_FIELDS) {
...
async function getCustomerMaster(
  ...
  return normalizeCustomerMasterRows(rows);
}
