	...
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper.select(Qry, function(result) {
			if (result.array != undefined) {
				result = $.map(result.array, function(item, index) {
					return [ [ item.customercode, item.customername,
							...
							item.invoicepaymentterms,item.outletsubtype,item.headerAmount,
							item.customeraddress2,item.customeraddress3,item.printoutletitemcode,item.alternatecode,item.splitfree,
							item.arbcustomeraddress1,item.arbcustomeraddress2,item.arbcustomeraddress3,item.tsalesamt,item.treturnsamt,item.tdamagesamt,item.invoicepriceprint,item.totalfreesampleamount]];
				});
			...
			//	printstatus =  sessionStorage.getItem("printstatus");
				...
				totalfinalamount=eval(result[0][24])-eval(result[0][25])-eval(result[0][26]);
               ...
               checkOrderHeader('SalesOrder', '1',totalfreesampleamount,splitfree);
...
function GetOrderData(trans, val,isFree)
{
	...
	if (val == '1') {
		...
		var Header = ["SL#","ITEM#","OUTLET CODE", "DESCRIPTION", "UPO", "QTY CAS/PCS",
				"TOTAL PCS", "CASE PRICE", "UNIT PRICE","DISCOUNT", "AMOUNT"];
		//alert(Header);
	} else {
		var total = {
			"QTY CAS/PCS" : "0/0",
		...
		var HeaderFree = ["SL#", "ITEM#","OUTLET CODE", "DESCRIPTION", "UPO", "QTY CAS/PCS",
		      			"TOTAL PCS", "CASE PRICE", "UNIT PRICE","DISCOUNT", "AMOUNT"]
	...
	//		+ sessionStorage.getItem("CheckCode")
	//		+ "' = 'alternatecode' THEN im.alternatecode ELSE im.actualitemcode END AS 'itemcode',IFNULL(ocode.outletitemcode,'-') AS 'outletcode',itemdescription AS 'description',unitspercase as upc,CASE WHEN unitspercase ='1' THEN  '0/' || IFNULL(CAST((salesqty/unitspercase) AS INT),0) ELSE (IFNULL(CAST((salesqty/unitspercase) AS INT),0) ||  '/' || IFNULL(CAST((salesqty%unitspercase) AS INT),0)) END AS 'quantity',salesqty AS 'tqty',IFNULL(salescaseprice,0) AS caseprice,IFNULL(salesprice,0) AS unitprice,IFNULL(promoamount,0),0 AS 'discount',(((IFNULL(CAST((salesqty/unitspercase) AS INT),0) * IFNULL(salescaseprice,0)) + (IFNULL(CAST((salesqty%unitspercase) AS INT),0)) * IFNULL(salesprice,0)) - IFNULL(promoamount,0)),arbitemshortdescription AS arbdescription FROM salesorderdetail sod JOIN itemmaster im ON im.actualitemcode = sod.itemcode and sod.salesqty > 0 JOIN salesorderheader soh ON soh.visitkey = sod.visitkey AND soh.customercode = "
	//		+ sessionStorage.getItem("customerid")
	//		+ " LEFT JOIN outletitemcodes ocode on ocode.itemcode=sod.itemcode and ocode.groupcode= '" +customerOutlet+ " '  WHERE sod.routekey="
				...
				if (trans == 'free' || trans == 'promofree') {
					result = $.map(result.array, function(item, index) {
						return [ [item.sl, item.itemcode,item.outletcode, item.description, item.upc,
								...
								item.unitprice,item.discount, item.amount,item.arbdescription ] ];
					});
				} else {
					result = $.map(result.array,
							function(item, index) {
...
function SetOrderTransaction(trans, result, Header, Total,isFree) { //alert("SetOrderTransaction"); alert('trans' +trans);
	for (i = 0; i < result.length; i++) { 
		for (j = 0; j < result[i].length; j++) {
...
//				decimalplace).toString();
		...
		/* data["SUB TOTAL"] = eval(totalfinalamount).toFixed(
		 ...
		 data["SUB TOTAL"] = (eval(totalfinalamount) - eval(itempromoamount)).toFixed(decimalplace).toString();
...
function getInvoiceInfo() {
	...
	var Qry = "SELECT cm.customercode,customername  || '-' || arbcustomername  AS 'customername',customeraddress1  AS 'address',cm.roundnetamount,inv.paymenttype,IFNULL(totalpromoamount,0) totalpromoamount,(CAST(IFNULL(totalsalesamount,0) - IFNULL(totalreturnamount,0)-IFNULL(totaldamagedamount,0) AS VARCHAR)+(select IFNULL(sum(diffround),0) from invoicedetail where visitkey="+sessionStorage.getItem("VisitKey")+")) totalinvoiceamount,documentnumber,invoicenumber,comments,printstatus,(SELECT CASE cm.messagekey5 WHEN 0 THEN '' ELSE messageline1 || messageline2 || messageline3 || messageline4 END FROM customermessages WHERE messagekey = messagekey5) AS invheadermsg,(SELECT CASE cm.messagekey6 WHEN 0 THEN '' ELSE messageline1 || messageline2 || messageline3 || messageline4 END FROM customermessages WHERE messagekey = messagekey6) AS invtrailormsg,(SELECT IFNULL(sum(IFNULL(promoamount,0)),0) from invoicedetail invt where invt.visitkey="
			+ sessionStorage.getItem("VisitKey")
			+ " AND invt.routekey="
			+ sessionStorage.getItem("RouteKey")
			...
			if (result.array != undefined) {
				result = $.map(result.array, function(item, index) {
					return [ [ item.customercode, item.customername,
							...
							item.tdamagesamt,item.diffround,item.invoicepriceprint,item.roundnetamount,
							/* 37*/ item.itemlinetaxamount,item.totaltaxesamount,
                            item.printtax,item.applytax,item.taxregistrationnumber,item.salestax,item.returntax,item.damagetax,item.freetax,
                            item.totExcTax,item.totVatTax,item.cashamt,item.chequeamt,item.invdate,item.creditdays,item.creditterms,item.headerdiscounttaxamt,item.taxcard,item.crno]];
				});
				//alert(JSON.stringify(result));
				customercode = result[0][0];
				customername = result[0][1];
				...
				if(result[0][39]==null || result[0][39]=='' || result[0][39]==undefined){
	                printtax=0;
                }
				...
				data["SALESTAX"]=eval(result[0][42]).toFixed(decimalplace);
				data["RETURNTAX"]=eval(result[0][43]).toFixed(decimalplace);
				data["DAMAGEDTAX"]=eval(result[0][44]).toFixed(decimalplace);
				data["FREETAX"]=eval(result[0][45]).toFixed(decimalplace);
				
				data["TOTEXC"]=eval(result[0][46]).toFixed(decimalplace);
				//data["TOTVAT"]=eval(result[0][47]).toFixed(decimalplace);
				
				data["TOTVAT"]=eval(result[0][47] - result[0][53] ).toFixed(decimalplace);
				
				  var cashamt=eval(result[0][48]).toFixed(decimalplace);
				
				data["Cash"] = {"Amount" : eval(result[0][15]).toFixed(decimalplace)};
				//alert('data["Cash"]' +data["Cash"]);
			
				 var chequeamt=eval(result[0][49]).toFixed(decimalplace);
                   if(cashamt==0){
                  ...
                  // var paymentduedate=addDaysToDate(getTabletDate(),eval(result[0][51]));
                   ...
                   var paymentduedate=addDaysToDate(getTabletDate(),caldays);
                    ...
                    headerdiscounttaxamt = eval(result[0][53]).toFixed(decimalplace);
	...
	if (val == '1') {
		...
		var Header = ["SL#","ITEM#","OUTLET CODE", "DESCRIPTION", "UPC", "QTY CAS/PCS",
		   ...
		   var total = {"QTY CAS/PCS" : "0/0","TOTAL PCS" : "0","DISCOUNT" : "0","TAX" : "0","AMOUNT" : "0","AMOUNT AFTER VAT" : "0" }; 
		    var Header = ["SL#","ITEM#","OUTLET CODE","DESCRIPTION","UPC","QTY CAS/PCS","TOTAL PCS","CASE PRICE","UNIT PRICE","DISCOUNT","AMOUNT","EXC TAX","VAT","AMOUNT AFTER VAT","GROSS AMT","BARCODE"];
...
*/
		 var total = {"QTY CAS/PC" : "0/0","DISCOUNT" : "0","TAX" : "0","AMOUNT" : "0","AMOUNT AFTER VAT" : "0"}; 
	        var Header = ["SL#","ITEM#","OUTLET CODE","DESCRIPTION","QTY CAS/PCS","TOTAL PCS","CASE PRICE","UNIT PRICE","DISCOUNT","AMOUNT","EXC TAX","VAT","AMOUNT AFTER VAT","GROSS AMT","BARCODE"];
	}
	total["EXC TAX"]="0.00";
	total["VAT"]="0.00";
	...
	var QrySales = "SELECT DISTINCT 0 as sl,CASE WHEN '"
		+ sessionStorage.getItem("CheckCode")
		+ "' = 'alternatecode' THEN im.alternatecode ELSE im.actualitemcode END AS 'itemcode',IFNULL(ocode.outletitemcode,'-') AS 'outletcode',itemdescription AS 'description',unitspercase as upc,CASE WHEN unitspercase ='1' THEN  '0/' || IFNULL(CAST((salesqty/unitspercase) AS INT),0) ELSE (IFNULL(CAST((salesqty/unitspercase) AS INT),0) ||  '/' || IFNULL(CAST((salesqty%unitspercase) AS INT),0)) END AS 'quantity',salesqty AS 'tqty',IFNULL(salescaseprice,0) AS caseprice,IFNULL(salesprice,0) AS unitprice,IFNULL(promoamount,0) as discount,(((IFNULL(CAST((salesqty/unitspercase) AS INT),0) * IFNULL(salescaseprice,0)) + (IFNULL(CAST((salesqty%unitspercase) AS INT),0)) * IFNULL(salesprice,0)) - IFNULL(promoamount,0)) +  IFNULL(inv.diffround,0) AS 'amount',arbitemshortdescription AS arbdescription,barcode1 as barcode,COALESCE(salesitemexcisetax,0) as excisetax, COALESCE(salesitemgsttax,0) || '(' || CASE WHEN tm.taxpercentage IS NULL THEN 0 ELSE  CAST(tm.taxpercentage  AS INT)   END || '%)' vat,(((IFNULL(CAST((salesqty/unitspercase) AS INT),0) * IFNULL(salescaseprice,0)) + (IFNULL(CAST((salesqty%unitspercase) AS INT),0)) * IFNULL(salesprice,0)) - IFNULL(promoamount,0) +IFNULL(inv.diffround,0) + IFNULL(inv.salesitemgsttax,0)) AS 'amountav',(((IFNULL(CAST((salesqty/unitspercase) AS INT),0) * IFNULL(salescaseprice,0)) + (IFNULL(CAST((salesqty%unitspercase) AS INT),0)) * IFNULL(salesprice,0)) +IFNULL(inv.diffround,0)) AS 'grossamount' FROM invoicedetail inv JOIN itemmaster im ON im.actualitemcode = inv.itemcode and inv.salesqty > 0 JOIN invoiceheader invh ON invh.visitkey = inv.visitkey AND invh.customercode = "
		+ sessionStorage.getItem("customerid")
				...
				if (trans == 'free' || trans == 'promofree') {
					...
					if(eval(printbarcode)==1){ 
						result = $.map(result.array, function(item, index) {
							return [ [item.sl, item.itemcode,item.outletcode, item.description, item.upc,
									item.quantity, item.tqty, item.caseprice,
									item.unitprice,item.discount,item.excisetax,item.vat,item.amount,item.amountav,item.grossamount,item.barcode] ];
						});
					}else{
						result = $.map(result.array, function(item, index) {
							return [ [item.sl, item.itemcode,item.outletcode, item.description, item.upc,
									item.quantity, item.tqty, item.caseprice,
									item.unitprice,item.discount,item.excisetax,item.vat, item.amount,item.amountav,item.grossamount,item.arbdescription,item.barcode] ];
						});
					}
								...
								function(item, index) {
									...
									if (val == '1') {
										return [ [item.sl, item.itemcode,item.outletcode, item.description,
												...
												item.amount,item.amountav,item.grossamount,item.barcode ] ];
									} else {
										return [ [item.sl,item.itemcode,item.outletcode,item.description,
												item.quantity, item.tqty,
												item.caseprice, item.unitprice,
												item.discount,item.excisetax,item.vat, item.amount,item.amountav,item.grossamount,item.barcode ] ];
									}
								...
								function(item, index) {
									...
									if (val == '1') {
										return [ [item.sl, item.itemcode,item.outletcode, item.description,
												item.upc, item.quantity, item.tqty,
												item.caseprice, item.unitprice,item.discount,item.excisetax,item.vat,
												item.amount,item.amountav,item.grossamount,item.arbdescription,item.barcode ] ];
									} else {
										return [ [item.sl,item.itemcode,item.outletcode, item.description,
												item.quantity, item.tqty,
												item.caseprice,item.unitprice,
												item.discount, item.excisetax,item.vat,item.amount,item.amountav,item.grossamount,item.arbdescription,item.barcode ] ];
									}
...
//			invoicetype = "CREDIT INVOICE: " + invoicenumber;
	    ...
	    if(companyTaxStng==1){
	    	if(eval(headerAmount)>=0){
				...
				invoicetype = "TAX CREDIT INVOICE ";
			}
	    } else {
...
//				decimalplace).toString();
		...
		data["INVOICE DISCOUNT"] = (eval(itempromoamount) + eval(manualfreediscount)).toFixed(decimalplace).toString();
		
		
		 // data["INVOICE HEADER DISCOUNT"] =  (eval(totalpromoamount)).toFixed(decimalplace).toString();
		  data["INVOICE HEADER DISCOUNT"] =  (eval(totalpromoamount) - eval(itempromoamount)).toFixed(decimalplace).toString();
		...
		//data["NET SALES"] =eval(headerAmount).toFixed(decimalplace).toString();
		var netsales= eval(headerAmount)- eval(headerdiscounttaxamt);  //eval(totalfinalamount)-eval(itempromoamount);
			...
			if (result.array != undefined) {
				result = $.map(result.array, function(item, index) {
					return [ [ item.customercode, item.customername,
							...
							item.excesspayment,item.alternatecode,item.custbalance ] ];
				});
				customercode = (result[0][0]).toString();
				customername = (result[0][1]).toString();
				customeraddress = (result[0][2]).toString();
				...
				data["CustBalance"]= result[0][14]; // sujee added 27/06/2018
				getCollection();
			}
		}, function() {
			console.warn("Error calling plugin");
...
/*	var Qry = "SELECT CASE WHEN '" + localStorage.getItem("alternatepending") + "' = '1' THEN ard.alternateinvoicenumber ELSE ard.invoicenumber END as invoicenumber,(SELECT STRFTIME('%d/%m/%Y',transactiondate) FROM customerinvoice WHERE invoicenumber=ard.invoicenumber)  AS invoicedate,ard.totalinvoiceamount,ard.amountpaid,(ard.invoicebalance) AS invoicebalance FROM ardetail ard JOIN arheader arh ON ard.transactionkey = arh.transactionkey WHERE arh.invoicenumber ="
	...
	*/
	...
	var Qry = "SELECT CASE WHEN '" + localStorage.getItem("alternatepending") + "' = '1' THEN ard.alternateinvoicenumber ELSE ard.invoicenumber END as invoicenumber,(SELECT STRFTIME('%d/%m/%Y',transactiondate) FROM customerinvoice WHERE invoicenumber=ard.invoicenumber)  AS invoicedate,ard.totalinvoiceamount,(ard.invoicebalance+ard.amountpaid) as previousbalance,ard.amountpaid,(ard.invoicebalance) AS invoicebalance FROM ardetail ard JOIN arheader arh ON ard.transactionkey = arh.transactionkey WHERE arh.invoicenumber ="
	+ invoicenumber;
	console.log(Qry);
	if (platform == 'iPad') {
		Cordova.exec(function(result) {
			...
			if (result.array != undefined) {
				result = $.map(result.array, function(item, index) {
					...
					return [ [ item.itemcode, item.description, item.loadedqty,
							...
							item.vanstock,item.total,item.arbitemdescription]];
...
function getOrderInfo() {
	...
	"routekey="+routeKey+" and visitkey="+visitKey+"  and typecode=1),0) chequeamt FROM  customermaster cm JOIN "+
	...
	*/
	...
	"cm.invoicepriceprint as invoicepriceprint,ifnull((select amount from cashcheckdetail where  routekey="+routeKey+
	" and visitkey="+visitKey+"  and typecode=0),0) cashamt,ifnull((select amount from cashcheckdetail where "+
	"routekey="+routeKey+" and visitkey="+visitKey+"  and typecode=1),0) chequeamt,(select sig.signaturedata from sigcapturedata sig where sig.routekey=inv.routekey and sig.visitkey=inv.visitkey) as sigdata  FROM  customermaster cm JOIN "+
	"salesorderheader inv ON cm.customercode = inv.customercode AND inv.visitkey="+visitKey+"  AND inv.routekey="+routeKey+
	" LEFT JOIN cashcheckdetail ccd ON ccd.visitkey = inv.visitkey  AND ccd.routekey = inv.routekey";
	console.log(Qry+" -------------");
	 if (platform == 'Android') {
...
function GetOrderData(trans, val,isFree) {
	...
	var QryOrder= "SELECT 0 as sl,CASE WHEN '"
			...
			+customerOutlet+"  WHERE inv.routekey="
			+ sessionStorage.getItem("RouteKey")
			+ " AND inv.visitkey = "
			+ sessionStorage.getItem("VisitKey")
			+ " and im.itemtype=1  order by CASE printsequencecust WHEN 0 THEN 100000 ELSE printsequencecust END";
				...
				if (trans == 'free' || trans == 'promofree') {
					if(eval(printbarcode)==1){
						result = $.map(result.array, function(item, index) {
							return [ [item.sl, item.itemcode,item.outletcode, item.description, item.upc,
									item.quantity, item.tqty, item.caseprice,
									item.unitprice,item.discount, item.amount,item.barcode] ];
						});
					}else{
						result = $.map(result.array, function(item, index) {
