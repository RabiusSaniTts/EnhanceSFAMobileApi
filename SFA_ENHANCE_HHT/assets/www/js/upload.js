...
function SimulateFunction(rkey, tkey, vkey, flag) {
//alert("SimulateFunction");
	splittranKey = tkey;
	splitVisitKey = vkey;
	sendmanualdata(rkey, splittranKey, splitVisitKey, flag);
...
function sendmanualdata(rkey, tkey, vkey, flag) {
	...
	routekey = rkey;
	transactionkey = splittranKey.shift();
	//alert(JSON.stringify(transactionkey));
	visitkey = splitVisitKey.shift();
...
function getinvoicedetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,itemcode,salesqty,returnqty,damagedqty,freesampleqty,salesprice,returnprice,stdsalesprice,stdreturnprice,promoqty,salesitemexcisetax,salesitemgsttax,returnitemexcisetax,returnitemgsttax,damageditemexcisetax,damageditemgsttax,fgitemexcisetax,fgitemgsttax,promoitemexcisetax,promoitemgsttax,coopid,batchdetailkey,salescaseprice,returncaseprice,stdsalescaseprice,stdreturncaseprice,goodreturnprice,goodreturncaseprice,stdgoodreturncaseprice,stdgoodreturnprice,expiryqty,currencycode,returnfreeqty,manualfreeqty,limitedfreeqty,rebaterentqty,fixedrentqty,pricechgindicator,discountamount,discountpercentage,promoamount,replacementqty,replacementprice,replacementcaseprice,promovalue,mdat,returnpromovalue,returnpromoamount,amount,diffround,roundsalesamount,amount from invoicedetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
	...
	if (platform == 'iPad') {
		Cordova
				.exec(
						...
						"PluginClass",
						"GetfielddataMethod",
						[ "select routekey,visitkey,transactionkey,itemtransactiontype,itemcode,itemtranstypeseq,reasoncode,quantity,catchweightqty,weighted,instructioncode,currencycode,expirydate from invoicerxddetail where issync=0 and istemp='false'" ]);
	} else if (platform == 'Android') {
		window.plugins.DataBaseHelper
				.select(
...
function getsalesorderdetail() { //alert("getsalesorderdetail");
	if (platform == 'iPad') {
		Cordova
				.exec(
						function(result) {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,itemcode,salesqty,returnqty,damagedqty,freesampleqty,manualfreeqty,salesprice,returnprice,stdsalesprice,stdreturnprice,coopid,salescaseprice,returncaseprice,stdsalescaseprice,stdreturncaseprice,currencycode,allocated,freegoodcases,freegoodpcs,salespcs,allocatedcases,salescases,allocatedpcs,returncases,returnpcs from salesorderdetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")",
						function(result) {
	...
	if (platform == 'iPad') {
		Cordova
				.exec(
						...
						"PluginClass",
						"GetfielddataMethod",
						[ "select routekey,visitkey,transactionkey,itemtransactiontype,itemcode,itemtranstypeseq,reasoncode,quantity,catchweightqty,weighted,instructioncode,currencycode,expirydate from orderrxddetail where issync=0 and istemp='false'" ]);
	} else if (platform == 'Android') {
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,itemtransactiontype,itemcode,itemtranstypeseq,reasoncode,quantity,catchweightqty,weighted,instructioncode,substr(expirydate, 7, 4) || '-' || substr(expirydate, 4, 2) || '-' || substr(expirydate, 1, 2) as expirydate from orderrxddetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")",
						function(result) {
...
function getsalesorderheader() { //alert("getsalesorderheader");
	if (platform == 'iPad') {
		Cordova
				.exec(
						function(result) {
		...
		var cc="select transactionkey,routekey,visitkey,documentnumber,invoicenumber,transactiondate,transactiontime,dsdnumber,ponumber,customercode,routecode,CAST(salesmancode AS VARCHAR) salesmancode,orderdeliveryroutecode,orderdeliverydate,CAST(totalinvoiceamount AS VARCHAR) as totalinvoiceamount,CAST(totalsalesamount AS VARCHAR) as totalsalesamount,totalreturnamount,totaldamagedamount,dexflag,splittransaction,voidflag,transmitindicator,hhcinvoicenumber,paymenttype,hhcdocumentnumber,voidreasoncode,advanceused,paymentstatus,advancebalance,mdat,advancereceived,currencycode,status,refnumber,totalfreesampleamount,deliverystatus,data,comments,actualtransactiondate,comments2,hhctransactionkey,totalpromoamount,totalbuybackfreeamount from salesorderheader where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")";
		console.log(cc);
		...
		window.plugins.DataBaseHelper
				.select(
						"select transactionkey,routekey,visitkey,documentnumber,invoicenumber,transactiondate,transactiontime,dsdnumber,ponumber,customercode,routecode,CAST(salesmancode AS VARCHAR) salesmancode,orderdeliveryroutecode,orderdeliverydate,CAST(totalinvoiceamount AS VARCHAR) as totalinvoiceamount,CAST(totalsalesamount AS VARCHAR) as totalsalesamount,totalreturnamount,totaldamagedamount,dexflag,splittransaction,voidflag,transmitindicator,hhcinvoicenumber,paymenttype,hhcdocumentnumber,voidreasoncode,advanceused,paymentstatus,advancebalance,mdat,advancereceived,currencycode,status,refnumber,totalfreesampleamount,deliverystatus,data,comments,actualtransactiondate,comments2,hhctransactionkey,totalpromoamount,totalbuybackfreeamount,totallineitemtax from salesorderheader where issync=0 and istemp='false' and confirmorder = 1 and dexflag=0 and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")",
						function(result) {
...
function getbatchexpirydetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,batchdetailkey,batchnumber,itemcode,expirydate,quantity,transactiontypecode,visitkey from batchexpirydetail where issync=0 and istemp='false and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")'", function(result) {
...
function getardetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,invoicenumber,invoicedate,CAST(totalinvoiceamount AS VARCHAR) as totalinvoiceamount,onacctreasoncode,CAST(amountpaid AS VARCHAR) as amountpaid,CAST(invoicebalance AS VARCHAR) as invoicebalance,arcollectiontype,chequestatusindicator,sapchequestatusindicator,currencycode,pdcbalance,alternateinvoicenumber,deductionamount from ardetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")", function(result) {
							if (!$.isEmptyObject(result)) {
...
function getcashcheckdetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,typecode,checknumber,CAST(amount AS VARCHAR) as amount,updateindicator,checkdate,bankcode,checkstatus,branchcode,drawercode,chequestatusindicator,sapchequestatusindicator,currencycode,hhctransactionkey,paymenttype,checktype,transactiontype from cashcheckdetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey + ")", function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
	...
	if (platform == 'iPad') {
		Cordova
				.exec(
						...
						"PluginClass",
						"GetfielddataMethod",
						[ "select inventorykey,detailkey,routekey,transactiontype,routecode,salesmancode,transactiondate,transactiontime,documentnumber,odometerreading,transferlocationcode,referencenumber,requestdate,securitycode,transmitindicator,voidflag,hhcdocumentnumber,loadnumber,refdocumentnumber,currencycode,actualtransactiondate,inventorynumber,data,isurgent from inventorytransactionheader where issync=0 and istemp='false'" ]);
	} else if (platform == 'Android') {

		window.plugins.DataBaseHelper
				.select("select inventorykey,detailkey,routekey,transactiontype,routecode,CAST(salesmancode AS VARCHAR) salesmancode,transactiondate,transactiontime,documentnumber,odometerreading,transferlocationcode,referencenumber,requestdate,securitycode,transmitindicator,voidflag,CAST(hhcdocumentnumber AS VARCHAR),loadnumber,refdocumentnumber,currencycode,actualtransactiondate,inventorynumber,data,isurgent from inventorytransactionheader where issync=0 and istemp='false' and detailkey in ("
								+ visitkey + ")",
						function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
...
function getinventorytransactiondetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,detailkey,transactiontypecode,itemcode,quantity,weighted,itemprice,batchdetailkey,itemcaseprice,currencycode,reasoncode,expirydate from inventorytransactiondetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and detailkey in ("
								+ visitkey + ") ",
						function(result) {
...
function getnonservicedcustomer() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,customercode,reasoncode from nonservicedcustomer where issync=0 and routekey in ("
								+ routekey
								+ ") and customercode in ("
								+ transactionkey + ")", function(result) {
							if (!$.isEmptyObject(result)) {
...
function getsurveyauditdetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,surveydefkey,surveypage,surveyindex,surveyrectype,lookuptype,surveyresponse from surveyauditdetail where issync=0 and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")", function(result) {
...
function getposequipmentchangedetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,posaction,itemcode,quantity,serialnumber,instructioncode from posequipmentchangedetail where issync=0 and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")",
...
function getsigcapturedata() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,customercode,documentnumber,transactiondate,transactiontime,balancedueamount,signaturedata,transaction_type from sigcapturedata where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")", function(result) {
...
function getcustomervisitlog() {
	if (platform == 'Android') {
		window.plugins.DataBaseHelper
				.select(
						"SELECT log_id, routekey, routecode, salesmancode, customercode, log_startdate, log_starttime, log_enddate, log_endtime FROM customervisitlog ",
						function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
									customervisitlog[i] = result.array[i];
								}

							}
							customervisitlog = JSON.stringify(customervisitlog);
							getcustomeroperationcontrol();
		...
		window.plugins.DataBaseHelper
				.select(
						...
						function(result) {
							...
							customeroperationscontrol = JSON
									.stringify(customeroperationscontrol);
							//console.log(customeroperationscontrol);
...
//Second sigcapture function for sales
...
function getroutesequencecustomerstatus() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,seqweeknumber,seqweekday,routecode,customercode,sequencenumber,schelduledflag,servicedflag,scannedflag from routesequencecustomerstatus where issync=0 and routekey in ("
								+ routekey + ")",
						function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
...
function getnosalesheader() {
	...
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select transactionkey,routekey,visitkey,documentnumber,invoicenumber,routecode,salesmancode,transactiondate,transactiontime,nosalereasoncode,voidflag,transmitindicator,customercode,hhcdocumentnumber,hhcinvoicenumber,data from nosalesheader where issync=0  and routekey in ("
								+ routekey + ")", function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
		...
		window.plugins.DataBaseHelper
				.select(
						...
						function(result) {
							...
							enddaydetail = JSON.stringify(enddaydetail);
							/*
							 * if(clrlog=='clr') { uploaddata(clrlog); }else {
							 */
							uploaddata();
							// }
						}, function() {
...
function UploadImages(fileName, filePath) {
	...
	params.value1 = "test";
	params.value2 = "param";
	var url1=wsurl+"image/upload";
	// options.params = params;
	options.chunkedMode = false;
	var ft = new FileTransfer();

	
	/* org line sujee commented 01/03/2018
	 * ft.upload(filePath,
			"http://enomsfa01.westeurope.cloudapp.azure.com:8095/sfa/enhance_live/api/image/upload", win,
			fail, options);*/
...
function uploaddata() {
	...
	console.log("customer visit log : " + customervisitlog);
	var routecode = sessionStorage.getItem("RouteCode");
	var userid = sessionStorage.getItem("SalesmanCode");
	var deviceid = sessionStorage.getItem("DeviceID");
	var routekey = sessionStorage.getItem("RouteKey");
...
//	alert(invoicedetail);
	...
	$.ajax({
				...
				success : function(data) {
					...
					var orderrxddetaildata = data.orderrxddetaildata;
					...
					var customervisitlog = data.customervisitlog;
					...
					if (orderrxddetaildata != null
							&& orderrxddetaildata.length != 0) {
					...
					if (customervisitlog != null && customervisitlog.length != 0) {
						console.log("customervisitlog length : " + customervisitlog.length);
						for (i = 0; i < customervisitlog.length; i++) {
							
							updatecustomervisitlog(customervisitlog, i);
						}
					}
					...
					if (splitVisitKey.length > 0) {
						sendmanualdata(routekey, splittranKey, splitVisitKey,
								syncflag);
					} else {
	...
	if (platform == 'iPad') {
		return abc = Cordova.exec(function(result) {
			...
			alert(error);
				...
				[ "update invoicerxddetail set issync=1 where routekey='"
						+ orderrxddetaildata[i].routekey + "' and visitkey='"
						+ orderrxddetaildata[i].visitkey + "' and itemcode='"
						+ orderrxddetaildata[i].itemcode + "'" ]);
	} else if (platform == 'Android') {
		window.plugins.DataBaseHelper.insert(
				"update orderrxddetail set issync=1 where routekey='"
						+ orderrxddetaildata[i].routekey + "' and visitkey='"
						+ orderrxddetaildata[i].visitkey + "' and itemcode='"
						+ orderrxddetaildata[i].itemcode + "'", function(
						result) {
					return true;
				}, function() {
					console.warn("Error calling plugin");
...
function updatecustomervisitlog(customervisitlog, i) {
	if (platform == 'Android') {
		window.plugins.DataBaseHelper.insert(
				"update customervisitlog set issync=1 where routekey='"
						+ customervisitlog[i].routekey + "' and visitkey='"
						+ customervisitlog[i].visitkey + "'", function(result) {
					return true;
				}, function() {
					console.warn("Error calling plugin");
				});
