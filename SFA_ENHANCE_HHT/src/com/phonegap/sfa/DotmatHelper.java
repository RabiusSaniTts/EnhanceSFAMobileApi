...
public class DotmatHelper extends Plugin {
			...
			for (int j = 0; j < jArr.length(); j++) {
				...
				for (int i = 0; i < jInner.length(); i++) {
					...
					if (request.equalsIgnoreCase("Transfer_In")) {
						...
						printSalesOrderReport(jsnData, address);
		...
		try {
			...
			for (int i = 0; i < jData.length(); i++) {
				...
				for (int j = 0; j < jArr.length(); j++) {
					...
					/*
					 * else if (j == 8) { itemDescrion = "                 *" + jArr.getString(j) +
					 * "!";
					 * 
					 * }
					 */
	...
	void parseUnloadResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			// sujee commented no need to print total 08/06/2021
			/*
			 * for (int i = 0; i < jTotal.length(); i++) { JSONObject jTOBject =
			 * jTotal.getJSONObject(0); String strTotal = ""; for (int j = 0; j <
			 * headers.length(); j++) { if (i != 3 || i != 4 || i != 5 || i != 7 || i != 8)
			 * { if (jTOBject.has(headers.getString(j))) { strTotal = strTotal +
			 * getAccurateText(jTOBject.getString(headers.getString(j).toString()),
			 * hashValues.get(headers.getString(j).toString()) + MAXLEngth,
			...
			// no need to print variance 08/06/2021
			/*
			 * printlines2( (getAccurateText("UNLOADED DAMAGE VARIANCE", 60, 2) +
			 * getAccurateText( object.has("damagevariance") ?
			 * object.getString("damagevariance") : "0", 16, 1)), 1, object, 1, args[0], 7,
			 * 7);
			 */
	...
	void printLoadRequestReport(final JSONObject object, final String... args) {
		...
		try {
			...
			hashValues = new HashMap<String, Integer>();
			hashValues.put("Sl#", 4);
			hashValues.put("Item#", 12); // 7
			hashValues.put("Description", 36); // 41
			hashValues.put("UPC", 4);
	...
	void printSalesTaxReport(JSONObject object, String... args) {
		...
		try {
			...
			if (object.getString("displayupc").equals("1")) {
				hashValues = new HashMap<String, Integer>();
				hashValues.put("SL#", 0);
				hashValues.put("ITEM#", 5);

				if (printoultlet != 0) {
					hashValues.put("OUTLET CODE", 8);
					hashValues.put("BARCODE", 15);
					hashValues.put("DESCRIPTION", 25); // 25
					hashValues.put("DISCOUNT", 10);
				} else { // working

					hashValues.put("OUTLET CODE", 0);
					hashValues.put("BARCODE", 15);
					hashValues.put("DESCRIPTION", 40); // 15
				...
				hashValues.put("OUTLET CODE", 8);
				hashValues.put("BARCODE", 15);
				hashValues.put("DESCRIPTION", 40); // 32
			...
			for (int i = 0; i < jData.length(); i++) {
						...
						if (!isoutlet) {
							...
							if (j == 14) {
								strheader = strheader + getAccurateText(
										(HeaderVal.indexOf(" ") == -1) ? HeaderVal
												: HeaderVal.substring(0, HeaderVal.indexOf(" ")),
										8 + MAXLEngth, hashPositions.get(headers.getString(j).toString()));

								strHeaderBottom = strHeaderBottom + getAccurateText(
										(HeaderVal.indexOf(" ") == -1) ? ""
												: HeaderVal.substring(HeaderVal.indexOf(" "), HeaderVal.length())
														.trim(),
										9 + MAXLEngth, hashPositions.get(headers.getString(j).toString()));
								...
								if (j < 9) {
									...
									strTotal = strTotal
											+ getAccurateText(jTotal.getString(headers.getString(14).toString()),
													80 + MAXLEngth, hashPositions.get(headers.getString(j).toString()));
								}
							} else {
				...
				if (jInnerData.length() > 0) {
					...
					// if (!object.getString("LANG").equals("en")) {
					/*
					 * printlines2(ArabicTEXT.headerbottomrevereseArabic, 1, object, 1, args[0], 1,
					 * 1); printlines2(ArabicTEXT.headerrevereseArabic, 1, object, 1, args[0], 1,
					 * 1);
					 */

					if (strHeaderBottom.length() > 0) {

						// printlines2(ArabicTEXT.headerDotmatbottomrevereArab, 1, object, 1, args[0],
						// 1, 1);
					}
					// printlines2(ArabicTEXT.headerDotmatrevereseArabic.trim(), 1, object, 1,
					// args[0], 1, 1);
					...
					for (int m = 0; m < jArr.length(); m++) {
						if (m != 0 && m != 7 && m != 12) {
							...
							if (!isoutletdata) {
								...
								if (m == 3) {
									itemDescrion = "" + jArr.getString(15) + ""; // BC
								}

								else if (m == 4) {
									itemDescrion = "" + jArr.getString(3) + ""; // DES
								}
								...
								strData = strData + getAccurateText(itemDescrion,
										m == 15 ? 8 : hashValues.get(headers.getString(m).toString()) + MAXLEngth,
										hashPositions.get(m == 15 ? "DESCRIPTION" : headers.getString(m).toString()));
								/*
								 * if(m != 16) { strData = strData + getAccurateText(itemDescrion, m == 15 ? 60
								 * : hashValues.get(headers.getString(m).toString()) + MAXLEngth,
								 * hashPositions.get(m == 15 ? "DESCRIPTION" :
								 * headers.getString(m).toString()));
								 * 
								 * }
								 */
								/*
								 * else {
								 * 
								 * strData = strData + getAccurateText(itemDescrion, m == 15 ?2 :
								 * hashValues.get(headers.getString(m).toString()) + MAXLEngth,
								 * hashPositions.get(m == 15 ? "DESCRIPTION" :
								 * headers.getString(m).toString())); }
								 */
			...
			if (taxSetting == 1) {
				...
				if (object.has("TOTVAT")) {
					...
					int companyTaxStng = Integer.parseInt(object.getString("enabletax"));
					if (companyTaxStng == 1 && taxSetting != 1) {
						printlines2((getAccurateText("VAT AMOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
								+ getAccurateText(object.getString("TOTVAT"), 12, 0) + getAccurateText(" : ", 3, 0)
								+ "*" + getAccurateText(ArabicTEXT.TotalVatTax, 20, 2) + "!"), 1, object, 1, args[0], 1,
								1);
					}
					// }
			...
			if (object.has("GROSS AMOUNT") && object.getString("GROSS AMOUNT").toString().length() > 0) {
				double invoice = Double.parseDouble(object.getString("GROSS AMOUNT"));

				if (invoice != 0) {

					printlines2(
							(getAccurateText("GROSS AMOUNT", 25, 0) + getAccurateText(" : ", 3, 0)
									+ getAccurateText(object.getString("GROSS AMOUNT") + " OMR", 12, 0)
									+ getAccurateText(" : ", 3, 0) + "*"
									+ getAccurateText(ArabicTEXT.grossAmount, 15, 2) + "!"),
							1, object, 1, args[0], 1, 1);
				}
			}
			...
			if (object.has("INVOICE DISCOUNT") && object.getString("INVOICE DISCOUNT").toString().length() > 0) {
				...
				if (invoice != 0) {
					...
					printlines2(
							(getAccurateText("INVOICE DISCOUNT", 25, 0) + getAccurateText(" : ", 3, 0)
			...
			if (object.has("INVOICE HEADER DISCOUNT")
				...
				if (invoiceheader != 0) {
					...
					printlines2(
							(getAccurateText("INVOICE HEADER DISCOUNT", 25, 0) + getAccurateText(" : ", 3, 0)
									+ getAccurateText(object.getString("INVOICE HEADER DISCOUNT") + " OMR", 12, 0)
									+ getAccurateText(" : ", 3, 0) + "*"
									+ getAccurateText(ArabicTEXT.InvoiceDiscount, 15, 2) + "!"),
							1, object, 1, args[0], 1, 1);
				}
			...
			printlines2((getAccurateText("AMOUNT BEFORE VAT", 25, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(S1 + " OMR", 12, 0) + getAccurateText(" : ", 3, 0) + "*"
			...
			if (object.has("VAT CASH DISCOUNT") && object.getString("VAT CASH DISCOUNT").toString().length() > 0) {
				...
				if (cashdiscount != 0) {
					...
					printlines2(
							(getAccurateText("VAT CASH DISCOUNT", 25, 0) + getAccurateText(" : ", 3, 0)
									+ getAccurateText(object.getString("VAT CASH DISCOUNT") + " OMR", 12, 0)
									+ getAccurateText(" : ", 3, 0) + "*"
									+ getAccurateText(ArabicTEXT.InvoiceDiscount, 15, 2) + "!"),
							1, object, 1, args[0], 1, 1);
				}
			...
			if (object.has("TOTVAT")) {
				...
				int companyTaxStng = Integer.parseInt(object.getString("enabletax"));
				if (companyTaxStng == 1 && taxSetting != 1) {
					printlines2((getAccurateText("VAT AMOUNT", 25, 0) + getAccurateText(" : ", 3, 0)
							+ getAccurateText(object.getString("TOTVAT") + " OMR", 12, 0) + getAccurateText(" : ", 3, 0)
							+ "*" + getAccurateText(ArabicTEXT.TotalVatTax, 20, 2) + "!"), 1, object, 1, args[0], 1, 1);
				}
				// }
			...
			outStream.write(BoldOn);
			printlines2(
					(getAccurateText("AMOUNT AFTER VAT", 25, 0) + getAccurateText(" : ", 3, 0)
				...
				for (int l = 0; l < jInnerData.length(); l++) {
					...
					for (int m = 0; m < jArr.length(); m++) {
						...
						if (!isoutletdata) {
							...
							strData = strData + getAccurateText(itemDescrion,
									m == 11 ? 65 : hashValues.get(headers.getString(m).toString()) + MAXLEngth,
									hashPositions.get(m == 11 ? "DESCRIPTION" : headers.getString(m).toString()));
						}
					}
	...
	void parseReturnSummarysResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			hashValues.put("ARBDESCRIPTION", 0); // 25
			hashValues.put("Quantity", 20); // 10
			hashValues.put("Expiry Date", 0);
			...
			hashPositions.put("Expiry Date", 0);
	...
	void printSalesOrderReport(JSONObject object, String... args) {
		StringBuffer s1 = new StringBuffer();
		int printoultlet = 0;
		try {
			...
			for (int i = 0; i < jData.length(); i++) {
				...
				if (jInnerData.length() > 0) {
					...
					if (header.equals("order")) {
						HeadTitle = "SALES ORDER *" + ArabicTEXT.Sales + "!";
			...
			outStream.write(BoldOff);
			outStream.write(BoldOn);
			printlines2((getAccurateText("NET ORDER", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("SUB TOTAL"), 12, 0) + getAccurateText(" : ", 3, 0) + "*"
					+ getAccurateText(ArabicTEXT.NetSales, 15, 2) + "!"), 1, object, 1, args[0], 1, 1);
			outStream.write(BoldOff);
	...
	void parseCollectionResponse(final JSONObject object, final String... args) {
		...
		try {
			/*
			 * hashValues = new HashMap<String, Integer>(); hashValues.put("Invoice#", 18);
			 * hashValues.put("Due Date", 15); hashValues.put("Due Amount", 15);
			 * hashValues.put("Invoice Balance", 15); hashValues.put("Amount Paid", 15);
			 * 
			 * hashPositions = new HashMap<String, Integer>(); hashPositions.put("Invoice#",
			 * 0); hashPositions.put("Due Date", 0); hashPositions.put("Due Amount", 2);
			 * hashPositions.put("Invoice Balance", 2); hashPositions.put("Amount Paid", 2);
			 * 
			 * hashArbPositions = new HashMap<String, Integer>();
			 * hashArbPositions.put("Invoice#", 2); hashArbPositions.put("Due Date", 2);
			 * hashArbPositions.put("Due Amount", 0);
			 * hashArbPositions.put("Invoice Balance", 0);
			 * hashArbPositions.put("Amount Paid", 0);
			 * 
			 * hashArabVales = new HashMap<String, String>(); hashArabVales.put("Invoice#",
			 * ArabicTEXT.Invoice); hashArabVales.put("Due Date", ArabicTEXT.InvoiceDate);
			...
			hashValues.put("Invoice Amount", 10); // 15
			hashValues.put("Previous Balance", 15);

			hashValues.put("Invoice Balance", 10); // 15
			hashValues.put("Amount Paid", 10); // 15
			...
			String DedAmount = "The Deductions are ";
			String s_dedamt = "";
			int w_dedamt = 0;
			...
			if (object.getString("comments").toString().length() > 0) {
				printlines2(getAccurateText("Comments: " + object.getString("comments"), 80, 0), 1, object, 1, args[0],
						2, 2);
				if (ded_flag == true) {
					printlines2(" " + DedAmount, 1, object, 1, args[0], 3, 3);
				}

			} else {
				printlines2(" ", 2, object, 1, args[0], 2, 2);
				if (ded_flag == true) {
					printlines2("Comments: " + DedAmount, 1, object, 1, args[0], 3, 3);
				}
			}

			// sujee added 27/06/2018 to print cust bal
			printlines2(getAccurateText("CUSTOMER BALANCE: " + object.getString("CustBalance"), 80, 0), 3, object, 1,
					args[0], 2, 2);
	...
	void parseEndInventory(final JSONObject object, final String... args) {
		...
		try {
			hashValues = new HashMap<String, Integer>();
			hashValues.put("Sl#", 0);
			hashValues.put("Item#", 10);// 10
			hashValues.put("Description", 47); // 38
			...
			hashValues.put("Variance Qty", 10); // 9
			hashValues.put("Sales Qty", 10);
			hashValues.put("Return Qty", 10);
			hashValues.put("Damage Qty", 10);
			hashValues.put("FOC Qty", 10);
			...
			// ----------End
			line(startln);
			headerinvprint(object, 25);// 3

			JSONArray headers = object.getJSONArray("HEADERS");
			String strheader = "", strHeaderBottom = "";
			int MAXLEngth = 138; // 80
			for (int i = 0; i < headers.length(); i++) {
	...
	void parseDepositCashResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			String s_dedamt = "";
			int w_dedamt = 0;
			String dedflag = object.getString("DEDFLAG");
			line(startln);
			headerprint(object, 3);
	...
	void parseDepositCashDeductionResponse(final JSONObject object, final String... args) {
		...
		try {
				...
				for (int l = 0; l < jInnerData.length(); l++) {
					...
					for (int m = 0; m < jArr.length(); m++) {
						strData = strData + getAccurateText(jArr.getString(m),
								...
								hashPositions.get(headers.getString(m).toString()));
						/*
						 * if(dedflag.equals("1")) { if(m==3) { s_dedamt = jArr.getString(m); //w_dedamt
						 * = Integer.parseInt(s_dedamt); if(s_dedamt.equals("0.000")) {} else {
						 * DedAmount += "RO "+s_dedamt+", " ; }
						 * 
						 * 
						 * } }
			...
			/*
			 * if(dedflag.equals("1")) { printlines2("Comments:"+DedAmount, 1,
			 * object,1,args[0], 3, 3); }
			 */
	...
	void printVanSettlementReport(final JSONObject object, final String... args) {
		...
		try {
			...
			outStream.write("       ".getBytes());
			outStream.write(UnderlineOn);
			printlines2("TRANSACTIONS", 2, object, 1, args[0], 6, 6);
			outStream.write(UnderlineOff);
			outStream.write(BoldOff);
			outStream.write(NewLine);

			printlines2(getAccurateText("TOTAL LOAD VALUE: ", 30, 0)
					+ getAccurateText(object.getString("LOAD_TOTAL"), 10, 0), 1, object, 1, args[0], 6, 6);
			outStream.write(NewLine);
	...
	private void printlines1(String data, int ln, JSONObject object, int sts, String adr, int tp)
		...
		if (sts == 2 && count != 0) {
			...
			for (int i = 0; i < 0; i++) { // lnno changed to 0 to add ascii
				try {
					if (i % 10 == 0) {
						try {
							Thread.sleep(1000);
			...
			int ascii = 12;
			char asch = (char) ascii;
			outStream.write(asch);
		...
		if (!isEnd) {
			printArabic(data);
			for (int i = 0; i < ln; i++) { // ln chnged to zero to remove extra line between each line
				try {
			...
			if (count > pln) {
				...
				try {
					...
					char asch = (char) ascii;
					outStream.write(asch);
					line(startln); // line(cnln); changed to manage ascii
					if (tp == 4) {
	...
	private void printlines2(String data, int ln, JSONObject object, int sts, String adr, int tran, int tp)
		...
		if (sts == 2 && count != 0) {
			...
			for (int i = 0; i < 0; i++) { // lnno changed to 0 to add asccii
				try {
					if (i % 10 == 0) {
						try {
							Thread.sleep(1000);
		...
		if (!isEnd) {
			...
			if (count > pln) {
				...
				try {
					...
					char asch = (char) ascii;
					outStream.write(asch);
					line(startln);// line(21);//changed to manage ascii
					// outStream.write(CompressOff);
					if (tran == 2) {
						// suje commnted based on tax print header 21/04/2021
						headerprint(object, tp);
						/*
						 * if(companyTaxStng==1){ headerTaxprint(object, tp); }else {
						 * headerprint(object, tp); }
						 */
					} else if (tran == 1) {
						if (companyTaxStng == 1) {
							headerTaxprint(object, tp);
						} else {
							headerprint(object, tp);
						}
					} else if (tran == 5) {
	...
	private void headerTaxprint(JSONObject object, int type, String... args) throws JSONException {
		try {
			...
			try {
				...
				// printheaders(( Add[0] + ""), false, 2);
				// outStream.write(( Add[0]).getBytes());
				printheaders((getAccurateText("", 20, 1) + getAccurateText(Add[0], 40, 0)), false, 1);
				outStream.write(NewLine);
				// outStream.write((" " + Add[1]).getBytes());
				// printArabic(getAccurateText( Add[1], 40, 1));
				printheaders((getAccurateText("", 20, 1) + getAccurateText(Add[1], 40, 0)), false, 1);
				// printheaders(( Add[1] + ""), false, 2);
				// outStream.write(( Add[1]).getBytes());
				/// printArabic(getAccurateText(object.getString("companyaddress"), 80, 1));
			...
			if (object.has("companytaxregistrationnumber")) {
				 ...
				 * getAccurateText("VATIN : "+object.getString("companytaxregistrationnumber"),
				 * 80, 0) +getAccurateText("", 60, 1) +getAccurateText("*" +
				 * ArabicTEXT.taxdetail + "! ", 20, 2));
				 */

				printheaders(
						getAccurateText("", 45, 1)
								+ getAccurateText("VATIN: " + object.getString("companytaxregistrationnumber"), 30, 0),
						true, 2);
				// outStream.write(NewLine);
			...
			if (type == 1) {
				 ...
				 * printArabic(getAccurateText( "*" + ArabicTEXT.Creditinvoice + "! ", 40, 1));
				 * outStream.write(NewLine); printArabic(getAccurateText( "Invoice No. " +
				 * object.getString("invoicenumber"), 40, 1));
				 * 
				 * } else if (object.getString("invoicepaymentterms").contains("0") ||
				 * object.getString("invoicepaymentterms").contains("1")) {
				 * printArabic(getAccurateText( "*" + ArabicTEXT.Cashinvoice + "! ", 40, 1));
				 * outStream.write(NewLine); printArabic(getAccurateText( "Invoice No" +
				 * object.getString("invoicenumber"), 40, 1));
				 * 
				 * } else { outStream.write((getAccurateText(object.getString("INVOICETYPE"),
				 * 40, 1)).getBytes()); outStream.write(NewLine);
				 * outStream.write((getAccurateText( "Invoice No" +
				 * object.getString("invoicenumber"), 40, 1)).getBytes()); }
				 * 
				 * } else { outStream.write((getAccurateText(object.getString("INVOICETYPE"),
				 * 40, 1)).getBytes()); outStream.write(NewLine);
				 * outStream.write(DoubleWideOff); outStream.write(BoldOff);
				 * outStream.write(NewLine); outStream.write(BoldOn);
				 * outStream.write(DoubleWideOn); outStream.write((getAccurateText(
				 * "Invoice No. " + object.getString("invoicenumber"), 40, 1)).getBytes());
				 * outStream.write(DoubleWideOff); outStream.write(BoldOff); }
				 */
				//printheaders(getAccurateText("TAX INVOICE  " + object.getString("invoicenumber"), 50, 1), true, 2);
				printheaders(getAccurateText(object.getString("INVOICETYPE") + object.getString("invoicenumber"), 60, 1), true, 2);

				// printArabic(getAccurateText("TAX INVOICE "+"*" + ArabicTEXT.taxInv + "! ",
				// 60, 1));
				outStream.write(NewLine);
				// printheaders(getAccurateText("INVOICE
				// NUMBER:"+object.getString("invoicenumber"), 35, 1) ,true,2);
				// printheaders(getAccurateText("Invoice No
				// "+object.getString("invoicenumber")+" "+"*" + ArabicTEXT.invNo + "! ", 40, 1)
				// ,true,2);
				// printArabic(getAccurateText("Invoice No" +
				// object.getString("invoicenumber")+" "+"*" + ArabicTEXT.invNo + "! ", 80, 1));
				// outStream.write(NewLine);
				// outStream.write(NewLine);
				outStream.write(BoldOff);
				outStream.write(DoubleWideOff);
				...
				if (!object.getString("LANG").equals("en")) {
					...
					if (object.getString("invoicepaymentterms").contains("2")) {
						printArabic(getAccurateText("*" + ArabicTEXT.Creditinvoice + "! ", 40, 1));
						outStream.write(NewLine);
						printArabic(getAccurateText("Order No. " + object.getString("invoicenumber"), 40, 1));
						...
						outStream.write(NewLine);
						outStream.write(
								(getAccurateText("Order No. " + object.getString("invoicenumber"), 40, 1)).getBytes());
					}
			 ...
			 * Integer.parseInt(object.getString("ptype")); outStream.write(BoldOn);
			 * if(paymenttype==0){ printheaders(getAccurateText("(CASH)", 20, 1) ,true,2);
			 * // outStream.write((getAccurateText("(Cash)", 80, 1)).getBytes());
			 * outStream.write(NewLine); }else if(paymenttype==1){
			 * printheaders(getAccurateText("(CREDIT)", 20, 1) ,true,2);
			 * //outStream.write((getAccurateText("(Credit)", 80, 1)).getBytes());
			 * outStream.write(NewLine); }else if(paymenttype==2){
			 * //outStream.write((getAccurateText("(Cash/Credit)", 80, 1)).getBytes());
			 * printheaders(getAccurateText("(CASH/CREDIT)", 20, 1) ,true,2);
			 * outStream.write(NewLine); } outStream.write(BoldOff);
			 */
			outStream.write(NewLine);
			/*
			 * try { String[] parts = object.getString("CUSTOMER").split("\\-");
			 * outStream.write(("CUSTOMER: " + parts[0]).getBytes()); } catch (Exception e)
			 * { e.printStackTrace(); }
			 */
			outStream.write(("CUSTOMER: " + object.getString("CUSTOMER")).getBytes());
			outStream.write(NewLine);
			/*
			...
			printArabic(getAccurateText("" + object.getString("ADDRESS1"), 40, 0)
					+ getAccurateText("ROUTE:" + object.getString("ROUTE"), 40, 2));
			printArabic(getAccurateText("" + object.getString("ADDRESS2"), 40, 0) + getAccurateText(
					"SALESMAN:" + object.getString("SALESMAN") + "-" + object.getString("CONTACTNO"), 40, 2));
			printArabic(getAccurateText("" + object.getString("ADDRESS3"), 40, 0) + getAccurateText(
					"SUPERVISOR:" + object.getString("supervisorname") + "-" + object.getString("supervisorno"), 40,
					2));
			outStream.write(NewLine);
			...
			if (object.has("taxregistrationnumber")) {
				...
				/*
				 * printArabic(getAccurateText("VATIN" +
				 * object.getString("taxregistrationnumber"), 40, 0) + getAccurateText(
				 * "Trip ID" + object.getString("TourID"), 40, 2) + getAccurateText(" : ", 3,
				 * 0));
				 */
				printheaders(
						getAccurateText("VATIN: " + object.getString("taxregistrationnumber"), 40, 0) + getAccurateText(
								"INVOICE DATE:" + object.getString("DOC DATE") + " (" + object.getString("TIME") + ")",
								40, 0),
						true, 2);
				outStream.write(NewLine);
			...
			printArabic(getAccurateText("TAX CARD NO: " + object.getString("TAXCARDNO"), 40, 0) + getAccurateText(
					"SUPPLY DATE :" + object.getString("DOC DATE") + " (" + object.getString("TIME") + ")", 40, 2));
			outStream.write(NewLine);

			String paymentdueDate = "";
			try {
				paymentdueDate = object.getString("PAYMENTDUEDATE");
			} catch (Exception e) {

			}

			printArabic(getAccurateText("CR NO: " + object.getString("CRNO"), 40, 0)
					+ getAccurateText("PAYMENT DUE DATE :" + paymentdueDate, 40, 2));
			outStream.write(NewLine);

			printArabic(getAccurateText("", 40, 0) + getAccurateText("ADVANCE PAYMENT DATE: N/A", 40, 2));
			outStream.write(NewLine);

			/*
			 * printArabic(getAccurateText("ROUTE: " + object.getString("ROUTE"), 40, 0) +
			 * getAccurateText( "SALESMAN :" + object.getString("SALESMAN") + "-" +
			 * object.getString("CONTACTNO"), 40, 2)); outStream.write(NewLine);
			 * 
			 * 
			 * printArabic(getAccurateText("PAYMENT DUE DATE: " + paymentdueDate, 40, 0)
			 * +getAccurateText("SUPERVISOR : " + object.getString("supervisorname") + "-" +
			 * object.getString("supervisorno"), 40, 2));
			 */
			// outStream.write(NewLine);

			// printArabic( getAccurateText("PAYMENT DUE DATE", 57, 2) + getAccurateText(" :
			// ", 3, 0)+ getAccurateText(paymentdueDate, 10, 2));

			/*
			 * printArabic(getAccurateText("DATE:" + object.getString("DOC DATE") + " (" +
			 * object.getString("TIME") + ")", 40, 0) + getAccurateText( "PAYMENT DUE DATE:"
			 * + object.getString("PAYMENTDUEDATE"), 40, 2));
			 */
	...
	private void headerprint(JSONObject object, int type) throws JSONException {
		try {
			...
			if (type == 1) {
				...
				outStream.write(BoldOn);
				outStream.write(DoubleWideOn);
				// printheaders(getAccurateText("AGING ANALYSIS", 40, 1), false, 3); // sujee
				// commented
				printheaders(getAccurateText("CUSTOMER BALANCE", 40, 1), false, 3);
				outStream.write(DoubleWideOff);
				outStream.write(BoldOff);
				outStream.write(NewLine);
				outStream.write(NewLine);
				...
				outStream.write(BoldOn);
				outStream.write(DoubleWideOn);
				printheaders(getAccurateText("VAN SETTELMENT REPORT", 40, 1), false, 3);
				outStream.write(DoubleWideOff);
				outStream.write(BoldOff);
				outStream.write(NewLine);
				outStream.write(NewLine);
				...
				outStream.write(BoldOn);
				outStream.write(DoubleWideOn);
				printheaders(getAccurateText("DEDUCTION SUMMARY", 40, 1), false, 3);
				outStream.write(DoubleWideOff);
				outStream.write(BoldOff);
				outStream.write(NewLine);
				outStream.write(NewLine);
