...
public class PB51Helper_sigprinting extends Plugin {
  ...
  //  public static final int ImageView = null;
	...
	public class ConnectTo extends AsyncTask<String, Void, Boolean> {
		...
		protected Boolean doInBackground(String... address) {
			...
			//	 printOutput = new FileOutputStream(address[0]);
...
/*private void printimages(String data) throws IOException{
	//byte[] decodedData = Base64.getDecoder().decode(data);//decodeBase64(data);
...
//System.out.println(cordova.getActivity().getExternalFilesDir("Logo.bmp"));
Bitmap imageBit = BitmapFactory.decodeFile("/storage/emulated/0/Android/data/com.phonegap.sfa/files/Logo.bmp");
...
try{
	...
	Log.e("tag", e.toString());
		 ...
		 /*final int dataStartIndex = data.indexOf(",") + 1;
		 final String datau = data.substring(dataStartIndex);
		 byte[] decodedData = Base64.decode(datau, Base64.DEFAULT);
		System.out.println(decodedData+"decodedData"+datau);
		// byte[] decodedData = DatatypeConverter.parseBase64Binary(data.substring(data.indexOf(",") + 1));
        ByteArrayInputStream decodedString = new ByteArrayInputStream(decodedData);
		Bitmap bitMap = BitmapFactory.decodeStream(decodedString);
		 int height = 48;//bitmap.getHeight();
        int width = 48;//bitmap.getWidth();
       
        System.out.println(bitMap);
        double y = height;//Math.sqrt(IMAGE_MAX_SIZE  / (((double) width) / height));
        double x =width;// (y / height) * width;
	...
	Log.e("tag", e.toString());
       ...
       // outStream.write(bitMap);
        //String imgName = "SIGNATURE.png";
     /*   Bitmap bitmap = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length); 
        BitmapDrawable drawable = new BitmapDrawable(cordova.getActivity().getResources()/*or other way to get resource reference, bitmap);
        drawable.setBounds(0, 0, drawable.getIntrinsicWidth(),drawable.getIntrinsicHeight());*/
       // fos = new FileOutputStream("img.png");
      //  fos.write(decodedString);
	   
		//   outStream.write(scaledBitmap);
...
public int[][] convertBitmap(Bitmap inputBitmap) {

 int mWidth = inputBitmap.getWidth();
 int mHeight = inputBitmap.getHeight();

 int[][] pixels =convertArgbToGrayscale(inputBitmap, mWidth, mHeight);
 String mStatus = "ok";
 return pixels;
}

private int[][] convertArgbToGrayscale(Bitmap bmpOriginal, int width,  int height) {
int pixel;
int k = 0;
int B = 0, G = 0, R = 0;
...
try {
    for (int x = 0; x < height; x++) {
        for (int y = 0; y < width; y++) {
			...
			//result[x][y] = (int) (0.299 * R + 0.587 * G + 0.114 * B);
            if (R < 55) {                       
...
private void printimages(Bitmap bmp) throws IOException {

//File fl = new File("/storage/emulated/0/Android/data/com.phonegap.sfa/files/icn-6.png");
		//if (fl.exists()) {
		//Bitmap bmp = BitmapFactory.decodeFile("/storage/emulated/0/Android/data/com.phonegap.sfa/files/icn-6.png");
	//	byte[] decodedString = Base64.decode(encodedImage, Base64.DEFAULT);
		//Bitmap bmp = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
		System.out.println(bmp+"bmp");
		int[][] pixels = convertBitmap(bmp);
		
		outStream.write(PrinterCommands.SET_LINE_SPACING_24);
		/*	for (int y = 0; y < bmp.getHeight(); y += 24) {
			outStream.write(SELECT_BIT_IMAGE_MODE);
			System.out.println(pixels[y].length+"pixels[y].length");
			outStream.write(new byte[]{(byte)(0x00ff &  bmp.getWidth()), (byte)((0xff00 &  bmp.getWidth()) >> 8)});
		...
		//boolean v = false;
			...
			while (offset < bmp.getHeight()) {
				outStream.write(PrinterCommands.SELECT_BIT_IMAGE_MODE);
				for (int x = 0; x < bmp.getWidth(); ++x) {
			...
			outStream.write(PrinterCommands.SET_LINE_SPACING_30);
...
//System.out.println(luminance);
   result = (luminance < threshold) ? true:false;
   return result;
}
...
//					1, object, 1, args[0], 1, 1);
			...
			//if (object.has("sigcapturedata")) {
				printlines2("Signature:", 2, object, 1, args[0], 1, 1);
				String dataurl  = object.getString("sigcapture");
				String  imagedata = dataurl.substring(dataurl.indexOf(",")+1);
				byte[] imageasbytes = Base64.decode(imagedata, Base64.DEFAULT);
				Bitmap decodeimage  = BitmapFactory.decodeByteArray(imageasbytes, 0, imageasbytes.length);
				printimages(decodeimage);
				//printimages("sigcapturedata");
			//	printImage("sigcapturedata");
