...
<div></div>
	...
	<tr><td align="right">Override For :-</td><td><select name="type" id="type" align="right" onChange="hidecustomer(this);">
		<option value='-1' >------------ Select -------------</option>
		<option value='1' <?php if($_POST['type'] == "1") {echo "selected"; }  ?>>Journey Plan </option>
		<option value='2' <?php if($_POST['type'] == "2") {echo "selected"; }  ?>>GPS </option>
		...
		<?php if($_POST['type'] <> "7") {?>
	<tr id="customer"><td align="right">CustomerCode :-</td><td><input align="right" type="text" value="<?php echo $_POST['customercode'];?>" name="customercode" id="customercode" required></td></tr>
	<?php }?>
	<tr><td align="right">Access Key :- </td><td><input align="right" type="text" value="<?php echo $_POST['pass'];?>" name="pass" id="pass" required></td></tr>
	<tr><td colspan="2" align="center"><input  type="submit" value="GetData" name="submit"/></td></tr>
	</table>
...
function getCustomerMasterData($customercode,$dbcon) {
    ...
    if(mysqli_num_rows($run))  {
		...
		$headerColumn = array("CustomerCode","CustomerName","CreditLimit","CreditLimitdays","GracePeriod");
...
function updateotpgenerationdetails($customercode,$username,$type,$dbcon){
	$typearr = array("Journey Plan","GPS","Post Void","Customer Returns","Credit Limit Amount","Credit Days","Multiple Request");

	$sel_user="select rs.routecode,cm.customercode from customermaster cm  inner join routesequence rs ON rs.customercode= cm.customercode where cm.alternatecode  = '$customercode'";
    ...
    if(mysqli_num_rows($run))  {
	 ...
	 $updatecustomer ="insert into otplogdetail (routecode,customercode,username,otptype,otpdate,otptime,cdate) VALUES ($routecode,$customercode,'".$username."','".$typearr[$type-1]."','".date("Y-m-d")."',CURRENT_TIME(), NOW())";
